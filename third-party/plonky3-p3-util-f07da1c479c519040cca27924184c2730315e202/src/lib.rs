...
// Local modification notice: `apply_to_chunks` uses a Rust 1.90-compatible
// initialized-prefix slice construction, with one boundary regression test
// added for that substitution. See ../UPSTREAM.md for exact provenance.
    ...
    H: FnMut(&[I::Item]),
{
    ...
    loop {
        ...
        // `iter_next_chunk_erased` wrote exactly the first `n` elements and
        // `n <= BUFLEN`, so this view contains only initialized bytes. The
        // source array remains alive for the duration of `func`.
        let initialized_prefix =
            unsafe { core::slice::from_raw_parts(buf.as_ptr().cast::<I::Item>(), n) };
        func(initialized_prefix);
    }
}
...
mod tests {
    ...
    #[test]
    fn test_apply_to_chunks_initialized_prefix_boundaries() {
        const CHUNK_SIZE: usize = 4;

        fn chunks(input: &[u8]) -> Vec<Vec<u8>> {
            let mut output = Vec::new();
            apply_to_chunks::<CHUNK_SIZE, _, _>(input.iter().copied(), |chunk| {
        ...
        assert_eq!(chunks(&[1, 2, 3]), vec![vec![1, 2, 3]]);
        assert_eq!(chunks(&[1, 2, 3, 4]), vec![vec![1, 2, 3, 4]]);
        assert_eq!(
