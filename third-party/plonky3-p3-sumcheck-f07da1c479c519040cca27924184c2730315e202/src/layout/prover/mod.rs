...
//!
//! Local modification: layouts can bind claims at a caller-supplied point so
//! an enclosing reduction can derive that point after the commitment is fixed.
...
pub trait Layout<F: TwoAdicField, EF: ExtensionField<F>>: Sized {
    ...
    /// Records openings at a caller-supplied point that is absorbed into the
    /// transcript before the evaluations. This is the explicit-point form
    /// needed when another proof reduction has already derived the opening
    /// point after the commitment was fixed.
    fn eval_at_point<Ch>(
        &mut self,
        table_idx: usize,
        batch: &OpeningRequest,
...
mod tests {
    ...
    fn assert_explicit_point_transcript_parity<L>()
    where
        L: Layout<F, EF>,
    {
        ...
        let request = OpeningBatch::new(vec![0], vec![1]);
        let point = explicit_point(prover.num_variables_table(0), 2);
        let mut prover_challenger = challenger();
        let mut verifier_challenger = challenger();

        let evaluations = prover.eval_at_point(0, &request, point.clone(), &mut prover_challenger);
        verifier
            .add_claim_at_point(0, &request, &evaluations, point, &mut verifier_challenger)
            .unwrap();

        assert_eq!(prover.num_claims(), 2);
        let prover_next_challenge: EF = prover_challenger.sample_algebra_element();
        let verifier_next_challenge: EF = verifier_challenger.sample_algebra_element();
        assert_eq!(prover_next_challenge, verifier_next_challenge,);
    }

    fn assert_changed_explicit_point_changes_transcript<L>()
    where
        L: Layout<F, EF>,
    {
        let mut first_prover = L::from_witness(L::new_witness(build_tables(), FOLDING));
        let mut second_prover = L::from_witness(L::new_witness(build_tables(), FOLDING));
        let request = OpeningBatch::new(vec![0], vec![1]);
        let table_variables = first_prover.num_variables_table(0);
        let mut first_challenger = challenger();
        let mut second_challenger = challenger();

        first_prover.eval_at_point(
            0,
            &request,
            explicit_point(table_variables, 2),
            &mut first_challenger,
        );
        second_prover.eval_at_point(
            0,
            &request,
            explicit_point(table_variables, 3),
            &mut second_challenger,
        );

        let first_next_challenge: EF = first_challenger.sample_algebra_element();
        let second_next_challenge: EF = second_challenger.sample_algebra_element();
        assert_ne!(first_next_challenge, second_next_challenge);
    }

    #[test]
    fn explicit_point_claims_preserve_transcript_parity() {
        assert_explicit_point_transcript_parity::<PrefixProver<F, EF>>();
        assert_explicit_point_transcript_parity::<SuffixProver<F, EF>>();
    }

    #[test]
    fn changed_explicit_points_change_the_transcript() {
        assert_changed_explicit_point_changes_transcript::<PrefixProver<F, EF>>();
        assert_changed_explicit_point_changes_transcript::<SuffixProver<F, EF>>();
    }
