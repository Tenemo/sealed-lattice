...
impl<F: TwoAdicField, EF: ExtensionField<F>> Layout<F, EF> for SuffixProver<F, EF> {
        ...
        Ch: FieldChallenger<F> + GrindingChallenger<Witness = F>,
    {
        ...
        let point = SvoPoint::new_unpacked(self.claims.folding, &point, VariableOrder::Suffix);
        let (current_openings, current_evals): (Vec<_>, Vec<EF>) = batch
            .current()
            .iter()
            .copied()
            .map(|poly_idx| {
            ...
            .unzip();
        let (next_openings, next_evals): (Vec<_>, Vec<EF>) = batch
            .next()
            .iter()
            .copied()
            .map(|poly_idx| {
                let equality_partial = current_openings
                    .iter()
                    .find(|opening| opening.poly_idx() == Some(poly_idx))
                    .and_then(|opening| opening.data().rounds().last())
                    .map(|round| round.poly());
                let (evaluation, partial_evaluations) =
                    point.eval_next_suffix(table.poly(poly_idx), equality_partial);
                (
                    Opening::new_with_data(poly_idx, evaluation, partial_evaluations),
                    evaluation,
                )
