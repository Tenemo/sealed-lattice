...
//!
//! Local modification: explicit-point verifier claims are supported for the
...
impl<F: Field, EF: ExtensionField<F>> Verifier<F, EF> {
    ...
    /// Records one verifier claim at an explicit point already derived by an
    /// enclosing reduction. The point is transcript-bound before its claimed
    /// evaluations, matching [`Layout::eval_at_point`].
        ...
        Ch: p3_challenger::FieldChallenger<F> + p3_challenger::GrindingChallenger<Witness = F>,
    {
        ...
        let current_openings = batch
            ...
            .copied()
            .zip(evals.current().iter().copied())
            .map(|(poly_idx, evaluation)| VerifierOpening::new(poly_idx, evaluation))
            .collect();
...
mod tests {
    ...
    #[test]
    fn verifier_add_explicit_point_rejects_shape_mismatch_before_transcript_change() {
        let shapes = vec![TableShape::new(9, 2)];
        let mut verifier: Verifier<F, EF> = Verifier::new(&shapes, prefix_strategy());
        let mut changed_challenger = challenger();
        let mut unchanged_challenger = challenger();
        let point = Point::new(
            (0..9)
                .map(|coordinate| EF::from_u64(coordinate as u64 + 2))
                .collect(),
        );
        let request = OpeningBatch::new(vec![0, 1], Vec::new());
        let evaluations = OpeningBatch::new(vec![EF::from_u64(7)], Vec::new());

        let error = verifier
            .add_claim_at_point(0, &request, &evaluations, point, &mut changed_challenger)
            .unwrap_err();
        ...
        assert_eq!(verifier.num_claims(), 0);
        let changed_next_challenge: EF = changed_challenger.sample_algebra_element();
        let unchanged_next_challenge: EF = unchanged_challenger.sample_algebra_element();
        assert_eq!(changed_next_challenge, unchanged_next_challenge);
    }
