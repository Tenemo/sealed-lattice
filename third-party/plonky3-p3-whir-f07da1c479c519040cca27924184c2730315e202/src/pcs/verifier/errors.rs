...
//!
//! Local modification: resumable verifier transition errors are documented in
//! `../../../UPSTREAM.md`.
...
pub enum VerifierError {
    /// A resumable verifier method was called outside its protocol position.
    #[error("invalid verifier transition: expected {expected}, got {actual}")]
    InvalidVerificationPhase {
        expected: &'static str,
        actual: &'static str,
    },

    /// A query was supplied at a different ordinal than the sampled schedule.
    #[error("unexpected query ordinal in round {round_index}: expected {expected}, got {actual}")]
    UnexpectedQueryOrdinal {
        round_index: usize,
        expected: usize,
        actual: usize,
    },

    /// Internal resumable state is incomplete for the requested transition.
    #[error("incomplete verifier state: {details}")]
    IncompleteVerificationState { details: &'static str },
