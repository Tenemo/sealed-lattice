...
describe('BGV canonical stream runtime with the real WASM kernel', () => {
    ...
    it('stages exact aggregate openings as single-use canonical material', async () => {
        const runtime = openBgvCanonicalStreamRuntime({ kernel });
        const aggregateOpeningByteLength = 32_768 * 8;
        const materialRoot = '71'.repeat(64);
        const sourceBytes = makeChunk(aggregateOpeningByteLength, 71);

        await stageBgvTargetDecryptionAggregateOpeningMaterials({
            kernel,
            sources: [
                {
                    aggregateOpeningRoot: materialRoot,
                    pullChunk: pullChunks([sourceBytes]),
                    totalByteLength: aggregateOpeningByteLength,
                },
            ],
        });
        ...
        await runtime.writeMaterial({
            emitChunk: ({ bytes, chunkIndex }) => {
                emittedChunks[chunkIndex] = bytes.slice(0);
                return Promise.resolve();
            },
        ...
        expect(new Uint8Array(emittedChunks[0] ?? new ArrayBuffer(0))).toEqual(
            new Uint8Array(sourceBytes),
        );

        await expect(
            runtime.writeMaterial({
    ...
    it('refuses non-exact and trailing aggregate-opening bytes before retention', async () => {
        const aggregateOpeningByteLength = 32_768 * 8;
        for (const totalByteLength of [
            aggregateOpeningByteLength - 8,
            aggregateOpeningByteLength + 8,
        ]) {
            let pullCount = 0;
            await expect(
        ...
        const sourceBytes = makeChunk(aggregateOpeningByteLength, 74);
        await expect(
            stageBgvTargetDecryptionAggregateOpeningMaterials({
                kernel,
                sources: [
                    {
                        aggregateOpeningRoot: '74'.repeat(64),
                        pullChunk: (request) => {
                            if (request.chunkIndex === 0) {
                                return Promise.resolve(sourceBytes.slice(0));
                            }
    ...
    it('evicts prior aggregate openings when a later source fails', async () => {
        const runtime = openBgvCanonicalStreamRuntime({ kernel });
        const aggregateOpeningByteLength = 32_768 * 8;
        const firstRoot = '75'.repeat(64);
        const firstBytes = makeChunk(aggregateOpeningByteLength, 75);
        ...
        await expect(
            stageBgvTargetDecryptionAggregateOpeningMaterials({
                ...
                sources: [
                    {
                        aggregateOpeningRoot: firstRoot,
                        pullChunk: pullChunks([firstBytes]),
                        totalByteLength: aggregateOpeningByteLength,
                    },
                    {
                        aggregateOpeningRoot: '76'.repeat(64),
                        pullChunk: () =>
