...
establish proof validity, and no release transport caller consumes the
certificate gate. Its opaque
...
Neither the test-only verifier nor the other compact primitives has a WASM
export. The semantic authority, complete CFW/WHIR verification, reduced
execution fixtures, the response-tree executor, and
