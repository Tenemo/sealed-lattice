...
It does not expose raw hashing, object mutation, generic cryptography, active-static setup contribution APIs, VSS APIs, public ballot generation or casting, direct ballot proof construction, bounded-domain evaluator replay, target acceptance, decryption-share shell checks, decryption, raw BGV operations, proof witnesses, encryption randomness, raw VSS shares, legacy bridge or evaluation-proof routes, or protocol internals. Full-protocol `verifyTranscript` remains fail-closed until the later direct-path gates exist.
