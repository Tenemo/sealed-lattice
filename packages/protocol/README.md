...
The package does not currently orchestrate collective preparation, selected ballot sets, activation, certified evaluation, finality, release, or complete browser custody. Its checkpoint and storage modules are internal persistence mechanisms only. An unactivated pre-root owner samples every local seed contribution and commitment salt internally, anchors the complete raw catalog before invoking its kernel, retains the exact root and canonical local opening inventory, and checkpoints each canonical recipient plaintext before returning root bytes. Other seed-mailbox state owners persist sender source and randomness before generation, persist one recipient's exact local custody, receipt intent, and signing randomness before receipt production, and persist one participant's complete receipt-terminal endorsement intent and signing randomness before endorsement production. Each publication owner coordinates its mutations with an abstract external-recency anchor. A state-only joined-master owner now reads the exact completed source and recipient records, validates both terminal carriers through its injected kernel, and atomically replaces the two encrypted raw records with one encrypted joined record. It returns inert retention metadata only; physical reclamation remains separate. The owners are not connected to the Rust catalog, mailbox, receipt, endorsement, and join types through WebAssembly, a production anchor, or the participant worker and authorize no protocol transition. The remaining operations require Rust/WebAssembly-backed integration. TypeScript must not replace their cryptography or mint acceptance from transport or persistence status. No cryptographic suite is activated.
