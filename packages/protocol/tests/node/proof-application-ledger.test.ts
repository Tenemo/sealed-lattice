...
const item = (itemType: number, payload: Uint8Array): Uint8Array =>
    concatenate([unsigned16(itemType), unsigned32(payload.byteLength), payload]);
...
const streamDescriptor = (proofByteLength = 1n): Uint8Array => {
    const chunkCount = Number(
        (proofByteLength + 1_048_575n) / 1_048_576n,
    );
    const digests = Array.from(
        { length: chunkCount },
        (_, index) => new Uint8Array(64).fill(0x60 + index),
    );
    return tuple(0x1800, [
...
const requireVerifiedBinding = (
    ...
    if (!result.isValid) {
        throw new Error(`proof application binding refused: ${result.refusalReason}`);
    }
    return result.value;
};
