...
import {
    ...
    decodeStoredJournal,
    encodeStoredJournal,
} from '#packages/protocol/src/runtime/authenticated-checkpoint-store/records';
import {
    bytesToHex,
...
const operationLineageIdentifierFor = (seed = 1): Uint8Array =>
...
const deterministicCursorFor = (seed: number): Uint8Array =>
    Uint8Array.from(
        { length: 96 },
        (_unused, index) => (seed * 37 + index * 113) & 0xff,
    );

const emptyDeterministicCursor = (): Uint8Array<ArrayBuffer> =>
    Uint8Array.of(
        0x53,
        0x4c,
        0x43,
...
const boundaryFor = (input: {
    ...
    const sharedAttemptIdentifier =
        input.identity.randomnessAuthorityIdentifier;
    if (sharedAttemptIdentifier === undefined) {
        throw new Error('test identity is missing its stream attempt');
    }
    return {
        operationKind: 7,
        deterministicCursorBytes: deterministicCursorFor(0x61),
        randomnessAuthorityIdentifier: sharedAttemptIdentifier.slice(),
...
const deterministicBoundaryFor = (input: {
    ...
    deterministicCursorBytes: emptyDeterministicCursor(),
...
const expectedBoundary = (
    ...
    deterministicCursorBytes: boundary.deterministicCursorBytes,
...
const checkpointChainFields = (
    ...
    for (let itemIndex = 0; itemIndex < 14; itemIndex += 1) {
        const itemByteLength = view.getUint32(offset + 2, true);
        offset += 6;
        if (itemIndex === 11) {
            previousCheckpointDigest = canonicalManifestBytes.slice(
                offset,
                offset + itemByteLength,
            );
        } else if (itemIndex === 12) {
            checkpointGeneration = view.getBigUint64(offset, true);
        }
        offset += itemByteLength;
    }
    if (
        ...
        throw new Error('Checkpoint chain fields are missing.');
    }
    return { checkpointGeneration, previousCheckpointDigest };
};
...
describe('Authenticated checkpoint store', () => {
    ...
    const openStore = (input?: {
        ...
        openAuthenticatedCheckpointStore({
            ...
            rootKey: input?.rootKey ?? rootKey,
            store,
        });

    it('uses one bounded canonical binary journal and rejects hostile framing', () => {
        const lineageIdentifier = operationLineageIdentifierFor(0x31);
        const publicationIdentifier = operationLineageIdentifierFor(0x41);
        const lineageHex = bytesToHex(lineageIdentifier);
        const publicationHex = bytesToHex(publicationIdentifier);
        const chunkPrefix = `checkpoint/chunk/${lineageHex}/${publicationHex}/`;
        const record = {
            checkpointLineageIdentifier: lineageHex,
            newChunkRecordKeys: [
                `${chunkPrefix}00000000-${'11'.repeat(64)}`,
                `${chunkPrefix}00000001-${'22'.repeat(64)}`,
            ],
            obsoleteChunkRecordKeys: [],
            publicationIdentifier: publicationHex,
            recordVersion: checkpointRecordVersion,
        } as const;
        const encoded = encodeStoredJournal(record, checkpointLimits);

        expect([...encoded.slice(0, 4)]).toEqual([0x53, 0x4c, 0x43, 0x4a]);
        expect(() => {
            JSON.parse(new TextDecoder().decode(encoded));
        }).toThrow();
        expect(
            decodeStoredJournal(encoded, checkpointLimits, lineageIdentifier),
        ).toEqual(record);
        ...
        const excessiveNewChunkCount = encoded.slice();
        new DataView(excessiveNewChunkCount.buffer).setUint32(70, 3, true);
        ...
        for (const hostileJournal of [
            ...
            expect(() =>
                ...
                expect.objectContaining({ code: 'AuthenticationFailed' }),
            );
        }
    });

    it('chains advancing checkpoints by digest and preserves byte-identical republishing', async () => {
        const checkpointStore = openStore();
        const identity = await checkpointStore.beginOperation();
        const stateBytes = stateBytesFor(0x21);
        const firstBoundary = deterministicBoundaryFor({
            safeBoundaryOrdinal: 3,
            stateBytes,
        });
        const firstManifest = await checkpointStore.publish({
            boundary: firstBoundary,
            identity,
            stateChunks: chunkState(stateBytes),
        });
        expect(checkpointChainFields(firstManifest)).toEqual({
            checkpointGeneration: 1n,
            previousCheckpointDigest: new Uint8Array(64),
        });

        const secondBoundary = deterministicBoundaryFor({
            safeBoundaryOrdinal: 4,
            stateBytes,
        });
        const secondManifest = await checkpointStore.publish({
            boundary: secondBoundary,
            identity,
            stateChunks: chunkState(stateBytes),
        });
        expect(checkpointChainFields(secondManifest)).toEqual({
            checkpointGeneration: 2n,
            previousCheckpointDigest: foundationHash512(
                'sealed-lattice/runtime/checkpoint-manifest-digest/v1',
                variableBytesItem(firstManifest),
            ),
        });
    ...
    it('publishes and resumes exact multi-chunk state with opaque deterministic cursor bytes', async () => {
        const checkpointStore = openStore();
        const identity = await checkpointStore.beginOperation(
            operationLineageIdentifierFor(),
        );
        ...
        expect(identity.randomnessAuthorityIdentifier).toEqual(
            operationLineageIdentifierFor(),
        );
        const resumed = await checkpointStore.resume({
    ...
    it('retains one exact operation-lineage identifier and rejects malformed identifiers', async () => {
        const checkpointStore = openStore();
        ...
        const copiedIdentifier = identity.randomnessAuthorityIdentifier;
    ...
    it('publishes deterministic checkpoint state without a randomness authority', async () => {
        const checkpointStore = openStore();
        const identity = await checkpointStore.beginOperation();
        ...
        expect(
            resumed.operationIdentity.randomnessAuthorityIdentifier,
        ).toBeUndefined();
        expect(await restoreBytes(resumed)).toEqual(stateBytes);
    });
    ...
    it('keeps deterministic cursors opaque while binding their exact bytes', async () => {
        const checkpointStore = openStore();
        const identity = await checkpointStore.beginOperation(
            operationLineageIdentifierFor(),
        );
        ...
        const replacementBoundary: CheckpointBoundary = {
            ...
            deterministicCursorBytes:
                advancedBoundary.deterministicCursorBytes.map(
                    (byte, byteIndex) =>
                        byteIndex === 17 ? byte ^ 0x80 : byte,
                ),
        };
        await checkpointStore.publish({
        ...
        const sameBoundaryDifferentManifest: CheckpointBoundary = {
            ...replacementBoundary,
            deterministicCursorBytes:
                replacementBoundary.deterministicCursorBytes.map(
    ...
    it('refuses every changed resume manifest, source, operation, and boundary coordinate', async () => {
        ...
        const changedBoundaries: ExpectedCheckpointBoundary[] = [
            ...
            {
                ...exact,
                deterministicCursorBytes: exact.deterministicCursorBytes.map(
                    (byte, byteIndex) => (byteIndex === 0 ? byte ^ 1 : byte),
                ),
            },
            {
                ...exact,
                randomnessAuthorityIdentifier:
                    operationLineageIdentifierFor(0x7f),
            },
            {
                ...exact,
                deterministicCursorBytes: new Uint8Array(
                    exact.deterministicCursorBytes.byteLength + 1,
                ),
            },
        ];
    ...
    it('rejects forged identities, unissued operation lineages, and oversized cursors', async () => {
        const checkpointStore = openStore();
        const identity = await checkpointStore.beginOperation(
            operationLineageIdentifierFor(),
        );
        ...
        const forgedIdentity = {
            checkpointLineageIdentifier:
                identity.checkpointLineageIdentifier.slice(),
            randomnessAuthorityIdentifier:
                identity.randomnessAuthorityIdentifier?.slice(),
        ...
        const {
            randomnessAuthorityIdentifier: _omittedRandomnessAuthority,
            ...identityFreeBoundary
        } = boundary;
        await expect(
            checkpointStore.publish({
                boundary: identityFreeBoundary,
                identity,
                stateChunks: chunkState(stateBytes),
            }),
        ).rejects.toMatchObject({ code: 'InvalidInput' });
        await expect(
            checkpointStore.publish({
                boundary: {
                    ...boundary,
                    randomnessAuthorityIdentifier:
                        operationLineageIdentifierFor(0xa5),
                },
                identity,
                stateChunks: chunkState(stateBytes),
            }),
        ...
        await expect(
            checkpointStore.publish({
                boundary: {
                    ...boundary,
                    deterministicCursorBytes: new Uint8Array(
                        checkpointLimits.maximumCursorByteLength + 1,
                    ),
                },
                identity,
                stateChunks: chunkState(stateBytes),
    ...
    it('fails closed on weak entropy, tampered chunks, a wrong key, and eviction', async () => {
        ...
        await expect(
            repeatingIdentifierStore.beginOperation(
                operationLineageIdentifierFor(),
            ),
        ...
        const wrongRootKey = await generateRuntimeStorageRootKey();
        await expect(
            openStore({ rootKey: wrongRootKey }).resume({
                checkpointLineageIdentifier:
                    identity.checkpointLineageIdentifier,
                expectedBoundary: expectedBoundary(boundary),
            }),
