...
// These codes frame encrypted local checkpoint records only. They are not
// candidate object codes and cannot authorize a protocol transition.
...
export const checkpointManifestOperationDomain =
    'sealed-lattice/runtime/checkpoint-manifest-record/v1';
export const checkpointJournalOperationDomain =
    'sealed-lattice/runtime/checkpoint-journal-record/v1';
export const checkpointChunkOperationDomain =
...
const checkpointManifestDigestDomain =
    'sealed-lattice/runtime/checkpoint-manifest-digest/v1';
...
const checkpointJournalMagic = Uint8Array.of(0x53, 0x4c, 0x43, 0x4a);
const checkpointJournalHeaderByteLength =
    checkpointJournalMagic.byteLength + 2 + identifierByteLength * 2 + 4 + 4;
...
export type CheckpointOperationIdentity = Readonly<{
    ...
    checkpointLineageIdentifier: Uint8Array;
    randomnessAuthorityIdentifier?: Uint8Array;
}>;
...
export type CheckpointBoundary = Readonly<{
    ...
    deterministicCursorBytes: Uint8Array;
    randomnessAuthorityIdentifier?: Uint8Array;
...
export type AuthenticatedCheckpointStoreLimits = Readonly<{
    ...
    maximumCursorByteLength: number;
...
type DecodedCheckpointManifest = Readonly<{
    ...
    previousCheckpointDigest: Uint8Array;
    publicationIdentifier: string;
    stateStreamDescriptorBytes: Uint8Array;
}>;

export type StoredCheckpointJournal = Readonly<{
...
export type CheckpointOperationIdentityRecord = {
    checkpointLineageIdentifier: Uint8Array;
...
export function copyAndValidateBoundary(
    ...
    if (
        ...
        !(boundary.deterministicCursorBytes instanceof Uint8Array) ||
        boundary.deterministicCursorBytes.byteLength >
            limits.maximumCursorByteLength ||
    ...
    const deterministicCursorBytes = copyBoundedBytes(
        boundary.deterministicCursorBytes,
        limits.maximumCursorByteLength,
        'deterministicCursorBytes',
    );
    const randomnessAuthorityIdentifier =
        boundary.randomnessAuthorityIdentifier === undefined
            ? undefined
            : copyExactBytes(
                  boundary.randomnessAuthorityIdentifier,
                  identifierByteLength,
                  'randomnessAuthorityIdentifier',
              );
    return Object.freeze({
        operationKind: boundary.operationKind,
        orderedSourceDigests,
        deterministicCursorBytes,
        ...(randomnessAuthorityIdentifier === undefined
            ? {}
            : { randomnessAuthorityIdentifier }),
...
export const encodeCheckpointManifest = (input: {
    ...
    return encodeCanonicalTuple(checkpointManifestSchemaIdentifier, [
        fixedCanonicalItem(
            ...
            input.authorityContext.runtimeManifestHash,
        ),
        fixedCanonicalItem(
            canonicalItemTypes.hash512,
            input.authorityContext.candidateIdentity,
        ),
        fixedCanonicalItem(
            canonicalItemTypes.hash512,
        ...
        fixedCanonicalItem(
            canonicalItemTypes.rawBytes,
            input.boundary.deterministicCursorBytes,
        ),
        fixedCanonicalItem(
            canonicalItemTypes.rawBytes,
            input.boundary.randomnessAuthorityIdentifier ?? new Uint8Array(0),
        ),
        fixedCanonicalItem(
            canonicalItemTypes.hash512,
        ...
        fixedCanonicalItem(
            ...
            unsigned64LittleEndian(input.checkpointGeneration),
        ),
        fixedCanonicalItem(
...
const parseCheckpointManifestReferences = (
    ...
    const fixedItemByteLengths: readonly (number | undefined)[] = [
        hashByteLength,
        hashByteLength,
        hashByteLength,
        hashByteLength,
    ...
    let checkpointGeneration: bigint | undefined;
    let previousCheckpointDigest: Uint8Array | undefined;
    ...
    for (
        ...
        if (
            ...
            (fixedItemByteLengths[itemIndex] !== undefined &&
        ...
        if (itemIndex === 5) {
            ...
            previousCheckpointDigest = itemBytes;
        } else if (itemIndex === 12) {
            checkpointGeneration = new DataView(
                itemBytes.buffer,
    ...
    if (
        ...
        previousCheckpointDigest === undefined ||
...
export const parseStreamDescriptor = (
...
export const deriveCheckpointManifestDigest = (
    canonicalManifestBytes: Uint8Array,
): Uint8Array => {
    ...
    try {
        ...
        updateAsciiItem(hash, checkpointManifestDigestDomain);
        updateUnsigned16(hash, canonicalItemTypes.rawBytes);
        updateUnsigned32(hash, canonicalManifestBytes.byteLength + 4);
        updateUnsigned32(hash, canonicalManifestBytes.byteLength);
        hash.update(canonicalManifestBytes);
        return hash.digest();
    } finally {
        hash.destroy();
    }
...
export const deriveChunkDigest = (input: {
    chunkBytes: Uint8Array;
    chunkIndex: number;
    stateStreamDomain: string;
}): Uint8Array => {
...
export const createFullObjectDigestHasher = (input: {
    stateStreamDomain: string;
    totalByteLength: number;
...
export const authenticateFullObjectDigest = (
    hash: ReturnType<typeof shake256.create>,
...
export const manifestRecordKey = (lineageIdentifier: Uint8Array): string =>
    `checkpoint/manifest/${bytesToHex(lineageIdentifier)}`;

export const journalRecordKey = (lineageIdentifier: Uint8Array): string =>
    `checkpoint/journal/${bytesToHex(lineageIdentifier)}`;

export const chunkRecordKey = (input: {
    checkpointLineageIdentifier: Uint8Array;
...
const decodeRecordKeys = (
    ...
    for (let keyIndex = 1; keyIndex < keys.length; keyIndex += 1) {
        if (keys[keyIndex - 1] >= keys[keyIndex]) {
            throw new AuthenticatedRuntimeRecordError(
                'AuthenticationFailed',
                `${label} is not in strict canonical order.`,
            );
        }
    }
    return keys;
...
export const decodeStoredManifest = (
    bytes: Uint8Array,
    limits: AuthenticatedCheckpointStoreLimits,
    ...
    return {
        canonicalManifestBytes,
        checkpointGeneration: manifestReferences.checkpointGeneration,
        ...
        previousCheckpointDigest: manifestReferences.previousCheckpointDigest,
        stateStreamDescriptorBytes:
            manifestReferences.stateStreamDescriptorBytes,
    };
};

export const encodeStoredManifest = (input: {
    canonicalManifestBytes: Uint8Array;
    publicationIdentifier: Uint8Array;
}): Uint8Array => {
...
const maximumCheckpointChunkCount = (
    limits: AuthenticatedCheckpointStoreLimits,
): number =>
    Math.ceil(
        limits.maximumCheckpointStateByteLength /
...
const encodeJournalRecordKeys = (
    keys: readonly string[],
    expectedPrefix: string,
    maximumCount: number,
    label: string,
): Uint8Array => {
    const canonicalKeys = decodeRecordKeys(
        keys,
        expectedPrefix,
        maximumCount,
        label,
    );
    const encodedKeys = canonicalKeys.map((key) => {
        const keyBytes = textEncoder.encode(key);
        if (
            keyBytes.byteLength === 0 ||
            keyBytes.byteLength > 0xffff ||
            fatalTextDecoder.decode(keyBytes) !== key
        ) {
            throw new AuthenticatedRuntimeRecordError(
                'InvalidInput',
        ...
        return concatenateBytes([
            unsigned16LittleEndian(keyBytes.byteLength),
            keyBytes,
        ]);
    });
    return concatenateBytes([
        unsigned32LittleEndian(encodedKeys.length),
        ...encodedKeys,
    ]);
};

export const encodeStoredJournal = (
    record: StoredCheckpointJournal,
    limits: AuthenticatedCheckpointStoreLimits,
): Uint8Array => {
    if (record.recordVersion !== checkpointRecordVersion) {
        throw new AuthenticatedRuntimeRecordError(
            'InvalidInput',
            'Checkpoint journal has the wrong record version.',
        );
    }
    const lineageIdentifier = decodeExactHex(
        record.checkpointLineageIdentifier,
        identifierByteLength,
        'checkpointLineageIdentifier',
    );
    const publicationIdentifier = decodeExactHex(
        record.publicationIdentifier,
        identifierByteLength,
        'publicationIdentifier',
    );
    try {
        if (publicationIdentifier.every((byte) => byte === 0)) {
            throw new AuthenticatedRuntimeRecordError(
                'InvalidInput',
                'Checkpoint journal publication identifier must not be zero.',
            );
        }
        const lineageHex = bytesToHex(lineageIdentifier);
        const publicationHex = bytesToHex(publicationIdentifier);
        const maximumChunkCount = maximumCheckpointChunkCount(limits);
        const encodedNewChunkKeys = encodeJournalRecordKeys(
            record.newChunkRecordKeys,
            `checkpoint/chunk/${lineageHex}/${publicationHex}/`,
            maximumChunkCount,
            'newChunkRecordKeys',
        );
        const encodedObsoleteChunkKeys = encodeJournalRecordKeys(
            record.obsoleteChunkRecordKeys,
            `checkpoint/chunk/${lineageHex}/`,
            maximumChunkCount,
            'obsoleteChunkRecordKeys',
        );
        const encoded = concatenateBytes([
            checkpointJournalMagic,
            unsigned16LittleEndian(checkpointRecordVersion),
            lineageIdentifier,
            publicationIdentifier,
            encodedNewChunkKeys,
            encodedObsoleteChunkKeys,
        ]);
        if (encoded.byteLength > limits.maximumManifestByteLength) {
            encoded.fill(0);
            throw new AuthenticatedRuntimeRecordError(
                'ResourceLimit',
                'Checkpoint journal exceeds the configured manifest limit.',
            );
        }
        return encoded;
    } finally {
        lineageIdentifier.fill(0);
        publicationIdentifier.fill(0);
    }
};

export const decodeStoredJournal = (
    bytes: Uint8Array,
    limits: AuthenticatedCheckpointStoreLimits,
    lineageIdentifier: Uint8Array,
): StoredCheckpointJournal => {
    if (
        bytes.byteLength < checkpointJournalHeaderByteLength ||
        bytes.byteLength > limits.maximumManifestByteLength
    ) {
        throw new AuthenticatedRuntimeRecordError(
            'AuthenticationFailed',
            'Checkpoint journal has an invalid byte length.',
        );
    }
    const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
    let offset = 0;
    const readBytes = (byteLength: number, label: string): Uint8Array => {
        if (
            ...
            throw new AuthenticatedRuntimeRecordError(
                'AuthenticationFailed',
                `Checkpoint journal ${label} is truncated.`,
            );
        }
        const value = bytes.slice(offset, offset + byteLength);
        offset += byteLength;
    ...
    const magic = readBytes(checkpointJournalMagic.byteLength, 'magic');
    if (!bytesEqual(magic, checkpointJournalMagic)) {
        throw new AuthenticatedRuntimeRecordError(
            'AuthenticationFailed',
            'Checkpoint journal has the wrong binary type.',
        );
    }
    const recordVersion = view.getUint16(offset, true);
    offset += 2;
    const decodedLineage = readBytes(identifierByteLength, 'lineage');
    const publicationIdentifier = readBytes(
        identifierByteLength,
        'publication identifier',
    );
    if (
        recordVersion !== checkpointRecordVersion ||
        !bytesEqual(decodedLineage, lineageIdentifier) ||
        publicationIdentifier.every((byte) => byte === 0)
    ) {
        throw new AuthenticatedRuntimeRecordError(
            'AuthenticationFailed',
            'Checkpoint journal has the wrong version, lineage, or publication identifier.',
        );
    }
    const maximumChunkCount = maximumCheckpointChunkCount(limits);
    const readRecordKeys = (
        expectedPrefix: string,
        label: string,
    ): readonly string[] => {
        if (offset + 4 > bytes.byteLength) {
            throw new AuthenticatedRuntimeRecordError(
                'AuthenticationFailed',
                `Checkpoint journal ${label} count is truncated.`,
            );
        }
        const keyCount = view.getUint32(offset, true);
        offset += 4;
        if (keyCount > maximumChunkCount) {
            throw new AuthenticatedRuntimeRecordError(
                'AuthenticationFailed',
                `Checkpoint journal ${label} exceeds the chunk limit.`,
            );
        }
        const keys: string[] = [];
        for (let keyIndex = 0; keyIndex < keyCount; keyIndex += 1) {
            if (offset + 2 > bytes.byteLength) {
                throw new AuthenticatedRuntimeRecordError(
                    'AuthenticationFailed',
                    `Checkpoint journal ${label} key length is truncated.`,
                );
            }
            const keyByteLength = view.getUint16(offset, true);
            offset += 2;
            const keyBytes = readBytes(keyByteLength, `${label} key`);
            let key: string;
            try {
                key = fatalTextDecoder.decode(keyBytes);
            } catch (error) {
                throw new AuthenticatedRuntimeRecordError(
                    'AuthenticationFailed',
                    `Checkpoint journal ${label} contains invalid UTF-8.`,
                    error,
                );
            }
            if (!bytesEqual(textEncoder.encode(key), keyBytes)) {
                throw new AuthenticatedRuntimeRecordError(
                    'AuthenticationFailed',
                    `Checkpoint journal ${label} contains noncanonical UTF-8.`,
                );
            }
            keys.push(key);
        }
        return decodeRecordKeys(keys, expectedPrefix, maximumChunkCount, label);
    };
    const lineageHex = bytesToHex(lineageIdentifier);
    const publicationHex = bytesToHex(publicationIdentifier);
    const newChunkRecordKeys = readRecordKeys(
        `checkpoint/chunk/${lineageHex}/${publicationHex}/`,
        'newChunkRecordKeys',
    );
    const obsoleteChunkRecordKeys = readRecordKeys(
        `checkpoint/chunk/${lineageHex}/`,
        'obsoleteChunkRecordKeys',
    );
    if (offset !== bytes.byteLength) {
        throw new AuthenticatedRuntimeRecordError(
            'AuthenticationFailed',
            'Checkpoint journal contains trailing bytes.',
        );
    }
    const record = Object.freeze({
        ...
        newChunkRecordKeys,
        obsoleteChunkRecordKeys,
        publicationIdentifier: publicationHex,
        recordVersion: checkpointRecordVersion,
    });
    const canonicalBytes = encodeStoredJournal(record, limits);
    try {
        if (!bytesEqual(bytes, canonicalBytes)) {
            throw new AuthenticatedRuntimeRecordError(
                'AuthenticationFailed',
                'Checkpoint journal is not canonically encoded.',
            );
        }
    } finally {
        canonicalBytes.fill(0);
        decodedLineage.fill(0);
        publicationIdentifier.fill(0);
    }
    return record;
};
...
export const deleteJournalOwnedChunkRecord = async (input: {
    logicalRecordKey: string;
    store: UntrustedStorageTransactionStore;
    transactionLifetimeMilliseconds: number;
}): Promise<void> => {
...
export const runCheckpointLineageExclusive = async <Result>(
    store: UntrustedStorageTransactionStore,
    checkpointLineageIdentifier: Uint8Array,
...
export const validateAuthenticatedCheckpointStoreLimits = (
    limits: AuthenticatedCheckpointStoreLimits,
    ...
    for (const [value, label] of [
        ...
        [limits.maximumCursorByteLength, 'maximumCursorByteLength'],
    ...
    if (
        limits.maximumRecordSealingCount > maximumRuntimeRecordDerivationCount
    ) {
        throw new AuthenticatedRuntimeRecordError(
            'InvalidConfiguration',
            'maximumRecordSealingCount exceeds the runtime-record root-key derivation ceiling.',
        );
    }
    if (
        limits.maximumActiveOperationIdentityCount >
