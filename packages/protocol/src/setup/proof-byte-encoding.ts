...
const standardBase64Character = (index: number): string => {
    const character = standardBase64Alphabet[index];
    if (character === undefined) {
        throw new Error('standard base64 index is outside the alphabet.');
    }

    return character;
};

export const bytesToStandardBase64 = (bytes: Uint8Array): string => {
    let encoded = '';
    for (let byteIndex = 0; byteIndex < bytes.byteLength; byteIndex += 3) {
        const first = bytes[byteIndex] ?? 0;
        const second = bytes[byteIndex + 1] ?? 0;
        const third = bytes[byteIndex + 2] ?? 0;
        const remaining = bytes.byteLength - byteIndex;

        encoded += standardBase64Character(first >> 2);
        encoded += standardBase64Character(
            ((first & 0x03) << 4) | (second >> 4),
        );
        encoded +=
            remaining >= 2
                ? standardBase64Character(((second & 0x0f) << 2) | (third >> 6))
                : '=';
        encoded += remaining >= 3 ? standardBase64Character(third & 0x3f) : '=';
    }
