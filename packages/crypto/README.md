...
This private package owns domain-separated cryptographic wrappers used by the
sealed-lattice foundation.
...
The package must fail closed when a provider cannot perform the exact
participant-worker operation. Generic hidden-randomness and remote-provider
interfaces are unsupported. Protocol-object provenance still comes from the
