...
} from './browser-compat/mobile-context-options';

type BrowserCompatibilityWindow = Window &
    typeof globalThis & {
        runBrowserCompatibilityCheck?: () => Promise<unknown>;
    };
...
type BrowserCompatibilityTarget = {
    ...
    readonly deviceName?: MobileDeviceName;
...
const createContextOptions = (
    ...
    return {
        ...getMobileContextOptions(target.deviceName),
        colorScheme: 'light',
    };
};
...
const runTarget = async (
    ...
    try {
        ...
        await page.waitForFunction(
            () =>
                typeof (window as BrowserCompatibilityWindow)
                    .runBrowserCompatibilityCheck === 'function',
        );

        const report = await page.evaluate(async () => {
            const compatibilityWindow = window as BrowserCompatibilityWindow;

            if (
                compatibilityWindow.runBrowserCompatibilityCheck === undefined
            ) {
                throw new Error(
            ...
            return compatibilityWindow.runBrowserCompatibilityCheck();
        });
...
const main = async (): Promise<void> => {
    ...
    try {
        ...
        console.log(
            `Verified browser compatibility for ${selectedTargets.length} targets.`,
        );
    } finally {
        await server.close();
    }
