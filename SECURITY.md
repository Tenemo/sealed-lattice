...
| `SEC-004` | The selected common-proof replacement is not production-integrated. Its exact extraction, binding, and transcript correspondence are incomplete, and no quantitatively usable result instantiates its ideal quantum random oracle with fixed SHAKE256 at the candidate query budget. | A passing development proof does not establish the intended proof-system assurance.                        |
| `SEC-005` | Per-family and ceremony-wide zero-knowledge and leakage composition are incomplete.                                                                                                                                                                                                  | Published development proofs must not be assumed to reveal only their public statements.                   |
