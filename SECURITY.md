...
| `SEC-004` | The row-code common-proof replacement has aggregate-wide masking and explicit-point verification, but its live 64-way geometry does not match the pinned theorem certificate and the exact typed-transcript, commitment, and verifier-equation correspondence remains conditional. | A passing development proof does not establish the intended proof-system assurance.                        |
| `SEC-005` | The current aggregate-wide pad's exact agreement bound is too weak. Its safer code-rate repair, real coefficient-to-view masking proof, nonlinear proof-view correspondence, and ceremony-wide leakage composition are unfinished.                                    | Published development proofs must be assumed to leak beyond their public statements.                       |
