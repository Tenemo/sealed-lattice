...
- `SEC-023`: No malicious tally-preparation realization satisfies the stated adversary. The reviewed independent-label compositions assume a statically correct base garbling while the removed component-contributed implementation allowed a corrupt participant to alter a row without pre-activation detection; their direct scalar field-MPC repair also exceeds the absolute transport-stream bound before framing. Four distributed full-row pads do not repair the cited adaptive theorem because the emitted pre-activation view is not one padded correct garbling and the pad openings do not realize delayed four-of-ten decoder custody. HSS/BOSS-style BMR publishes output masks too early, while the screened public-audit protocols either verify clear outputs, assign a sole evaluator, or rely on privileged computation nodes. The screened selected-row route likewise has one evaluator, reconstructs clear output immediately, permits selective abort, and leaves its preprocessing in classical idealized hybrids. A replacement must either verify every row before activation or prove that every error choice occurs only at a theorem-permitted uniformly masked-input boundary, leaks no protected input through failure, and is positively detected before accepted evaluation, finality, or release. No construction is selected or implemented; the remaining generic-proof branch has not passed its theorem or mobile-resource screen.
