...
- `SEC-002`: An exact ten-participant, ten-option preparation-to-release experiment passes functional development checks, but a fresh cryptographic review rejected its private-evaluation layer because public label metadata exposes intermediate circuit values and action-wide continuation secrets expose counterfactual branches. No complete replacement has passed cryptographic admission, release-browser lifecycle closure, or selected-phone qualification.
- `SEC-003`: The internal experiment implements persist-before-publish source, finality, activation, and terminal state with byte-identical replay and conflict refusal. Those state properties do not repair the evaluation privacy failure. Action-wide rollback reconciliation, eviction, quota failure, retirement, cleanup, and release-browser evidence also remain incomplete.
...
- `SEC-006`: An unexported development bridge carries state from preparation through source fixation, target finality, input activation, segmented evaluation, and result decoding with authenticated checkpoints. Its evaluation relation is rejected, so the bridge is functional evidence only and is not an admitted public capability chain.
...
- `SEC-023`: The exact emitted full tally publishes a zero-label semantic bit for each local label pair. Because the evaluator also receives the active label, it can decode every intermediate coordinate and the verifier can interpolate private conjunction values. This deterministic classical transcript attack invalidates the experiment's former conditional one-AND and full-circuit privacy arguments without breaking its PKE, hashes, AEAD, labels, or state. The exact evaluation instantiation is rejected, and no full private-evaluation construction is cryptographically admitted.
- `SEC-024`: The rejected experiment has a production-object static ledger and a development-Chromium ceremony, including transfer, refetch, checkpoint, protected-record, worker-copy, WebAssembly-memory, scalar-work, visit, and foreground measurements. Its activated browser result case used the shortest result width; maximum-width activation completed natively, while the maximum-width browser case was all abstention and did not evaluate the tally. The browser harness also emulated all participant stores with multiple workers in one process. Its accounted JavaScript/WebAssembly overlap is not total browser-process memory. Those values describe rejected bytes and cannot be applied mechanically to a replacement. A complete maximum-width scalar-browser result, production resources, external release-Chrome behavior, and selected-phone feasibility are open.
...
- `SEC-028`: The rejected evaluator reuses one affine continuation pair for every conjunction in an action. A gate exposes the selected key `A(0)` or `A(0)+B(0)`. Once the transcript contains both selector values, the evaluator knows both action-wide keys and can decrypt both continuation alternatives at every conjunction. Including the gate ordinal in the hash input separates ciphertext domains but does not make the underlying keys gate fresh. Any retained continuation design needs operation-fresh secret material and a complete serial, fan-out, multi-output, and full-tally privacy theorem.
- `SEC-029`: The rejected implementation derives all garbling labels from one 32-byte activation seed, while the working hidden-label analysis assumes independent labels. This is not a demonstrated attack on the already rejected transcript, but it is an independent theorem-to-bytes mismatch. A replacement must either sample and durably retain independent labels or prove an exact static-QPT keyed-generator and fixed-function result for the emitted correlated label corpus.
- `SEC-030`: The low-level finality-certificate command verifies signatures under a caller-supplied ordered action-key-set list but does not itself recompute that list's roster identity and compare it with the roster identity bound by the target. The current worker rederives the target and checks its retained roster, so no end-to-end substitution exploit is claimed. The owning verifier boundary must nevertheless become self-contained before admission.
