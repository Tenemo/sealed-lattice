...
- `SEC-023`: No malicious tally-preparation realization satisfies the stated adversary. The latest coded-share proposal robustly rejects three corrupt row coordinates but gives each participant enough pre-activation information to identify every selected row coordinate; one corrupt participant can then recover semantic wire masks and decode the result before finality. Any repair must keep selected coordinates hidden until active evaluation, prove privacy for the complete corrupt view, provide globally consistent row failure, and pass an exact static-quantum theorem and scalar-browser resource gate. `SEC-016` independently blocks delayed result release. Prior rejected garbling and proof routes are not fallbacks, and no construction is selected or implemented.
