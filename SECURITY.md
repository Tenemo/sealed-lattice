...
- `SEC-016`: No target-bound result-mask release construction is admitted. The bounded source screen found no applicable static-quantum theorem that simultaneously binds every real four-of-ten release to one preparation source and lets the simulator complete honest shares after the ideal result is known. Ordinary binding PVSS, noncommitting encryption, equivocal commitment, distributed PRF, and simulatable decryption results each miss part of that conjunction. Resource and scalar evidence cannot begin until it is closed.
...
- `SEC-023`: No malicious tally-preparation realization satisfies the stated adversary. The latest coded-share proposal robustly rejects three corrupt row coordinates but gives each participant enough pre-activation information to identify every selected row coordinate; one corrupt participant can then recover semantic wire masks and decode the result before finality. A share-hidden repair would still need complete-view privacy and globally consistent failure, but it is inactive because `SEC-016` fails first. Prior rejected garbling and proof routes are not fallbacks, and no construction is selected or implemented.
