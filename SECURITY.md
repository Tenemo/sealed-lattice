...
| `SEC-004` | `P1`     | The setup proof system does not carry a conventional 128-bit quantum soundness label.    | The setup proof families reach roughly 140 conjectured classical bits and about 70 quantum bits of soundness after the instance union, under the CS25 below-capacity FRI proximity-gap conjecture (with a proven BCIKS20 Johnson fallback at a higher query count). This remains below the conventional 128-bit quantum target and is scoped to proof publication time; the kernel enforces this floor, but it is not a 128-bit quantum claim. Ballot proofs and target-decryption share proofs need their own accepted accounting. |
