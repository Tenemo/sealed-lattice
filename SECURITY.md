...
- `SEC-006`: No participant bridge carries verifier-minted preparation, selected-set, input-activation, evaluation, finality, release-share, and result capabilities through the complete scalar ceremony with authenticated checkpoint custody.
- `SEC-007`: Direct threshold sharing is the retained ballot-custody family, but its exact availability predicate, sender-authenticated inconsistency outcome, service-censorship privacy, first-honest-release simulation, selected-set and input-activation binding, and production positive verifiers remain incomplete. Removed masked-ballot, omitted-input, encrypted-ballot, and per-ballot symmetric-bivariate routes do not supply those properties.
...
- `SEC-023`: No malicious tally-preparation realization satisfies the stated adversary. The reviewed independent-label compositions assume a statically correct base garbling while the removed component-contributed implementation allowed a corrupt participant to alter a row without pre-activation detection; their direct scalar field-MPC repair also exceeds the absolute transport-stream bound before framing. Four distributed full-row pads do not repair the cited adaptive theorem because the emitted pre-activation view is not one padded correct garbling and the pad openings do not realize delayed four-of-ten decoder custody. HSS/BOSS-style BMR publishes output masks too early, while the screened public-audit and selected-row protocols either expose clear outputs, assign a sole evaluator, or rely on privileged computation nodes. A reviewed honest-majority packed-sharing protocol reconstructs its output masks and clear result under a classical client-server theorem. Its delayed-output modification also fails: the source discloses its MAC keys before evaluation, binding an honest release share before the opaque target prevents simulator completion, and omitting that binding lets three corrupt holders substitute a polynomial with the first honest share. Direct transparent, lattice-toolkit, and streaming proof representations fail an exact theorem or scalar-browser memory gate. Every replacement must provide source-verifiable noncommitting four-of-ten release; a proof route must also provide distributed or per-contribution witness custody. Row correctness or succinct verification alone is insufficient. No construction is selected or implemented.
...
Identifiers are stable and are not reused. `SEC-005`, `SEC-009` through `SEC-015`, `SEC-018`, `SEC-022`, `SEC-025`, and `SEC-026` are retired because they describe removed constructions, duplicate active issues, or resolved authority conflicts.
