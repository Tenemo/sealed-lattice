...
| `SEC-004` | The row-code common-proof implementation has static geometry and masking evidence, but its deployed streaming leaf chain is cryptographically ineligible, complete WebAssembly prover liveness is not established, and no full-width production proof or release-WebAssembly browser verification has completed. | Static theorem and accounting evidence do not establish an eligible or operational proof path.             |
| `SEC-005` | Exact classical and quantum-random-oracle soundness are not established. Focused certificate code inventories the verifier-side deployed aggregate leaf calls and refuses the 256-bit intermediate outputs against the required uniform 512-bit oracle boundary. Family simulation, malicious-verifier zero knowledge, and quantum-random-oracle zero knowledge also remain unproved. | The prototype has no complete common-proof soundness, zero-knowledge, or ballot-privacy claim.              |
