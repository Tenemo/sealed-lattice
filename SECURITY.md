...
- `SEC-004`: No production proof system is selected. Production-derived evaluator-key quotient witnesses exceed the common-proof scratch ceiling, so the monolithic compact lowering is rejected. Its packet redesign gate also fails: a global shared lookup exhausts the scratch allowance before the remaining proof state, while local lookup packets lack a proved shared-witness relation, compound extraction and zero knowledge, and exact shared-transcript security. Replacement-backend selection must precede further proof-family or browser qualification work. The lower-level compact implementation is development material only, and the rejected previous implementation cannot be used as a fallback or as evidence.
