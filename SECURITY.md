...
| `SEC-004` | The row-code common-proof replacement has no eligible masking PCS and no completed theorem mapping for its exact typed transcript, explicit-point WHIR adapter, commitment layout, and verifier equations; its current soundness ledger is conditional.                          | A passing development proof does not establish the intended proof-system assurance.                        |
| `SEC-005` | The development proof path opens its aggregate commitment through a non-hiding argument, so query answers carry no masking component. Nonlinear proof-view simulation, persistent cross-consumer mask-image evidence, and ceremony-wide leakage composition are also unfinished. | Published development proofs must be assumed to leak beyond their public statements.                       |
