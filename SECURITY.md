...
- `SEC-023`: No malicious tally-preparation realization satisfies the stated adversary. The leading four-contributor structurally dealt degree-six zero-sharing source lacks its authoritative row-and-column verifier, constant-shift rejection, corrupt-view and selective-abort simulator, and production resource ledger; the subset-seeded fallback additionally lacks malicious key establishment and its multi-user fixed-Keccak theorem. Pairwise pseudorandom sharing remains in either route, while production integration of the selected collective challenge, hidden-bit certification, garbling provenance, and the complete emitted-protocol theorem also remain open.
