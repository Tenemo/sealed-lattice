...
| `SEC-004` | The row-code common-proof implementation has static geometry and masking evidence, but its deployed streaming leaf chain is not covered by the current soundness certificate and no full-width production proof or release-WebAssembly browser verification has completed. | Static theorem and accounting evidence do not establish an eligible or operational proof path.             |
| `SEC-005` | Exact quantum-random-oracle soundness accounting is not established: the current ledger uses a uniform 512-bit oracle model while the deployed aggregate leaf chain exposes 256-bit intermediate outputs. Family simulation, malicious-verifier zero knowledge, and quantum-random-oracle zero knowledge also remain unproved. | The prototype has no complete common-proof soundness, zero-knowledge, or ballot-privacy claim.              |
