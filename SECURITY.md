...
| `SEC-002` | Target-decryption verification is development-only. A release can start only from an in-kernel handle issued by successful accepted-setup verification, but the repository does not provide a persistent, non-forking authority that permits exactly one final result release across restarts or divergent clients. | Do not treat a released value as an authorized election result, expose raw decryption interfaces, or bypass proof-backed verification. |
