...
- `SEC-003`: The removed experiment implemented persist-before-publish source, finality, activation, and terminal state with byte-identical replay and conflict refusal. Those historical state properties do not repair the evaluation privacy failure. Construction-neutral durable preparation now retains the version-three directed-master vector before publication, and source fixation uses a new context-bound derivation version; both remain unadmitted research components alongside finality and direct all-abstain no-result state. No durable one-use activation owner consumes the leading reduced relation. Action-wide rollback reconciliation, eviction, quota failure, retirement, cleanup, and release-browser evidence also remain incomplete.
...
- `SEC-032`: The leading padded candidate masks each selected basis value with an independent pairwise-derived module value, derives continuation material separately for every gate and receiver, samples direct 320-bit labels, and uses KMAC256 row pads without publishing a label-corpus digest or refreshed-wire map. Its exact reduced relation emits through production Rust and an internal scalar-WebAssembly dispatcher, but remains stateless, unexported, and unreachable from the live worker. Independent hostile review found no deterministic selected-row extraction and did find proof-to-byte defects: an absent address ordinal encoded zero, the theorem named ML-DSA while the bytes used provisional WOTS, a classical wrong-key bound omitted coherent search, collision accounting omitted cross-participant and continuation-key events, and hostile coverage initially exercised only one selected row. The corrected bytes, exact games, and both-row native/scalar regressions passed the final reduced review. Generation now re-verifies the complete signed preparation and derives every reduced `B` and directed `P` value through exact KMAC256/AES domains; explicit affine input has been removed, old preparation and source versions refuse, and native plus signed scalar tests pin the weighted interpolation and inconsistent-pad failure. This is derivation correspondence only. Durable one-use state, complete-tally composition, standard ML-DSA/ML-KEM wrappers and their games, hostile relay execution, external browser, and mobile evidence remain open; the candidate is not cryptographically admitted.
