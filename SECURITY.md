...
- `SEC-032`: The leading padded candidate masks each selected basis value with an independent pairwise-derived module value, derives continuation material separately for every gate and receiver, samples direct 320-bit labels, and uses KMAC256 row pads without publishing a label-corpus digest or refreshed-wire map. Its exact reduced relation now emits through production Rust and an internal scalar-WebAssembly dispatcher, but remains stateless, unexported, and unreachable from the live worker. Independent hostile review found no deterministic selected-row extraction and did find proof-to-byte defects: an absent address ordinal encoded zero, the theorem named ML-DSA while the bytes used provisional WOTS, a classical wrong-key bound omitted coherent search, collision accounting omitted cross-participant and continuation-key events, and hostile coverage initially exercised only one selected row. The corrected bytes, exact games, and both-row native/scalar regressions passed the final reduced review. Derived streams, durable one-use state, complete-tally composition, standard ML-DSA/ML-KEM wrappers, hostile relay execution, external browser, and mobile evidence remain open; the candidate is not cryptographically admitted.
