...
| `SEC-004` | The row-code common-proof certificate now matches the live 64-way geometry and selected aggregate-wide mask, but no full-width production proof or release-WebAssembly browser verification has completed.                                                                 | Static theorem and accounting evidence do not establish an operational proof path.                         |
| `SEC-005` | The selected construction has exact joint-view masking correspondence and classical and quantum-random-oracle soundness accounting. Family simulation, malicious-verifier zero knowledge, and quantum-random-oracle zero knowledge remain unproved.                          | The prototype has no complete zero-knowledge or ballot-privacy claim.                                      |
