...
- `SEC-032`: The leading padded candidate masks each selected basis value with an independent pairwise-derived module value, derives continuation material separately for every gate and receiver, samples direct 320-bit labels, and uses KMAC256 row pads without publishing a label-corpus digest or refreshed-wire map. Its exact reduced relation emits through production Rust and an internal scalar-WebAssembly dispatcher, but remains stateless, unexported, and unreachable from the live worker. Independent hostile review found no deterministic selected-row extraction and did find proof-to-byte defects: an absent address ordinal encoded zero, a classical wrong-key bound omitted coherent search, collision accounting omitted cross-participant and continuation-key events, and hostile coverage initially exercised only one selected row. The corrected bytes, exact games, and both-row native/scalar regressions passed the final reduced review. Generation re-verifies complete signed preparation and derives every reduced `B` and directed `P` value through exact KMAC256/AES domains. The emitted development profile now uses canonical roster ML-DSA-65 signatures and fresh ML-KEM-768 encapsulation with KMAC256-derived AES-256-GCM-SIV for all ninety private deliveries; malformed roster keys, ciphertexts, signatures, domains, replays, and conflicting carriers have focused refusal coverage. This is wrapper and reduced-relation correspondence under direct multi-user primitive assumptions, not a reduction, composition theorem, or admission. Durable one-use activation state, complete-tally composition, hostile relay execution, production resources, external browser, and mobile evidence remain open.
