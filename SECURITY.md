...
- `SEC-023`: No malicious tally-preparation realization currently satisfies the security model. The leading candidate replaces the rejected dealer-supplied zero masks with subset-seeded degree-six zero sharing, uses one same-action preparation attempt, and derives its challenge from committed participant sources after the challenged roots are fixed. Local Rust verification now reaches canonical subset, pair, and collective-coin source leaves, a semantic inventory of all ten individually authorized seed-catalog roots, and an all-roster terminal over that inventory. Seed-delivery verification requires the typed terminal, but durable state and endorsement production, source-authorized pair-stream and collective-coin masters, authenticated private delivery, receipts, malicious key establishment, source-authorized pseudorandom streams, complete preparation verification, and a continuation capability remain absent. Its multi-user quantum-pseudorandom-function, salted-commitment, fixed-Keccak/KMAC, garbling, emitted-protocol, and selective-abort arguments remain open.
- `SEC-024`: Mobile feasibility is open for the active direction. The current one-attempt formula-level upload lower bound is `1,572,257,726` bytes, leaving `575,225,922` bytes before uncompiled durable state, mailbox encryption, signed receipts, ballot custody, checkpoints, storage overlap, repair, restart, and remaining terminal-burn retention. The current zero source also requires `8,403,192` independently framed field outputs per participant. A batched bit-validity candidate would reduce both figures materially, but it has no production graph, verifier, counters, or scalar-browser evidence and is not an admitted resource total. No complete production-derived ledger, matched scalar ceremony, external desktop-browser lifecycle, or physical-phone result exists.
