...
| `SEC-005`  | The native source-verified compact public-key terminal retains the exact canonical proof size and emitted-byte census only after transport, full algebraic verification, and independent source correspondence. Emitted-byte zero knowledge, salted-Merkle privacy, proof-to-random-oracle composition, the fixed-tape shared-quantum-random-oracle premise, the initial-transition lemma, and complete probability accounting remain open. No security-bit total is authorized. |
