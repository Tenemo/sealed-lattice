...
- `SEC-003`: The removed experiment implemented persist-before-publish source, finality, activation, and terminal state with byte-identical replay and conflict refusal. Those historical state properties do not repair the evaluation privacy failure. The current construction-neutral worker validates one retained ML-DSA-65 signing key and one retained ML-KEM-768 decapsulation key against the canonical roster on every load, retains the version-three directed-master vector before publication, and persists exact preparation, source, finality, and certificate-verified all-abstain bytes before release. The leading fixed reduced relation now has a new object-kind-nine one-body owner: it durably allocates independent labels and a nonce before generation, atomically binds one manifest and publication journal to the activation-purpose slot, persists the signed package before returning it, replays exact bytes, and refuses alternate requests, partial rollback, or lost activation state. This is conditional lifecycle evidence for a synthetic relation, not a complete tally or security admission. Coordinated rollback of the entire browser-local root and all protected records remains an environmental assumption; quota, eviction, physical-storage reclamation, release-browser behavior, and complete-tally retirement remain incomplete.
...
- `SEC-006`: The live worker reaches source fixation and self-contained target finality, with a separate certificate-verified all-abstain no-result terminal and an internal one-body path for the leading fixed reduced relation. Finality binds the exact action, compiler, circuit, output, source, activation, and direct-finality identities. Its owning verifier parses and validates the supplied canonical ML-DSA/ML-KEM roster, recomputes its identity, compares that identity with the target, and only then verifies the eight-of-ten carrier set and returns carrier-independent capability data. Both the earlier segmented evaluator and the attempted per-basis operation-fresh conjunction replacement are removed. The latter exposed degree-three affine evaluations through publicly decryptable high-basis translation rows, enabling same-gate counterfactual continuation-key recovery. Internal dispatcher commands and the reduced worker method exercise fixed synthetic relations only. They are not exported, authorized as tally results, or complete admitted protocols. There is no admitted public end-to-end capability chain.
...
- `SEC-032`: The leading padded candidate masks each selected basis value with an independent pairwise-derived module value, derives continuation material separately for every gate and receiver, samples direct 320-bit labels, and uses KMAC256 row pads without publishing a label-corpus digest or refreshed-wire map. Its exact reduced relation emits through production Rust and an internal scalar-WebAssembly dispatcher and now has an internal durable worker owner. Independent hostile review found no deterministic selected-row extraction and did find proof-to-byte defects: an absent address ordinal encoded zero, a classical wrong-key bound omitted coherent search, collision accounting omitted cross-participant and continuation-key events, and hostile coverage initially exercised only one selected row. The corrected bytes, exact games, and both-row native/scalar regressions passed the final reduced review. Generation re-verifies complete signed preparation and derives every reduced `B` and directed `P` value through exact KMAC256/AES domains. The worker samples canonical independent label pairs from platform randomness, persists them before generation, removes them from the live record when the exact body is bound, persists one activation signature, and passes cold-crash, exact-replay, alternate-body, partial-rollback, state-loss, corrupt-finality, and seven-signature refusal tests in development Chromium. The emitted development profile uses canonical roster ML-DSA-65 signatures and fresh ML-KEM-768 encapsulation with KMAC256-derived AES-256-GCM-SIV for all ninety private deliveries. This is wrapper, reduced-relation, and one-body lifecycle correspondence under direct multi-user primitive assumptions, not a reduction, composition theorem, or admission. Complete-tally composition, hostile relay execution, production resources and physical reclamation, external browser, and mobile evidence remain open.
