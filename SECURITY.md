...
- `SEC-022`: Governing product requirements still contain four direct threshold-homomorphic mechanism references that conflict with the active direction. The project owner must replace them with mechanism-neutral outcome and trust-boundary requirements before any suite activation or completion claim.
- `SEC-023`: No malicious tally-preparation realization currently satisfies both the leakage model and mobile resource plan. The active independent-label direction has reproducible circuit, pseudorandom-sharing, bivariate-sharing, masked-degree-reduction, garbling-geometry, and static-resource research results. It still lacks a malicious seed protocol, exact pseudorandom-function and quantum reductions, degree-reduction proof, commitment and sharing correspondence, public-verifiable-garbling theorem, state graph, complete resource ledger, and scalar-browser implementation. Superseded preparation models do not fill those gaps.
