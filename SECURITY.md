...
- `SEC-007`: Ballot-submission production, rollback-safe quorum authorization of one explicit selected-ballot-set body, activation, one-label opening, and complete positive evaluation are not connected through production byte interfaces.
...
- `SEC-023`: No malicious tally-preparation realization currently satisfies the security model. The latest reviewed dealer-based masked-degree-reduction route accepted an attacker-chosen nonzero constant from one corrupt dealer because codeword checking did not enforce zero at the origin. Its root-derived batch challenge also lacks fresh post-root entropy or a chosen-root grinding reduction. The attacked route is rejected. The broader independent-label direction still lacks a selected zero source, malicious seed protocol, exact byte and state graph, pseudorandom-function and quantum reductions, source correspondence, garbling theorem, emitted-protocol simulator, and complete positive verifier.
- `SEC-024`: Mobile feasibility is open for the active direction. The reviewed bivariate-dealer ledger undercounts field work, misclassifies stream and chunk sizes, omits the complete all-absent opening case and major protocol and browser owners, and already exceeds the all-participant upload planning target. The straightforward structurally zero-at-origin repair is larger still and requires architecture review. No complete production-derived ledger, matched scalar ceremony, external desktop-browser lifecycle, or physical-phone result exists.
