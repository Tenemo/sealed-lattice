...
- `SEC-023`: No malicious tally-preparation or private-evaluation realization satisfies the stated adversary. Direct compressed coded-share garbling robustly rejects three corrupt row coordinates but gives each participant enough pre-activation information to identify every selected row coordinate; one corrupt participant can then recover semantic wire masks and decode the result before finality. An unactivated minimum verifier establishes the pre-evaluation-finality chronology and construction-neutral correct-or-abort behavior under explicit preparation, source-privacy, signature, state, and fixed-Keccak assumptions. Three published garbling mappings are rejected at their static quantum theorem gates, and only the first reached executable work before removal. A crate-private direct honest-majority MPC candidate now has an information-theoretic arithmetic screen, canonical carriers, and a static resource and fixed-function ledger, but its dominant scalar execution, complete fixed-Keccak advantage, source and preparation verifiers, post-finality evaluation verifier, and composed theorem remain open. No construction is selected or suite-activated.
- `SEC-024`: Mobile feasibility is open. The direct-MPC candidate's static compiler accounts for its current transfer, storage, live-set, checkpoint, repair, restart, fixed-function, arithmetic, and hostile-path graph, but modeled allocations are not observed scalar or browser peaks. No current-candidate scalar WebAssembly timing, forced-restoration, external-Chrome, or selected-phone evidence exists.
