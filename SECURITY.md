...
| `SEC-002` | `P0`     | Target-bound decryption is not certified.                                            | The proof-backed target-decryption relation, smudging-hardened share generation with a seven-input smudging seed binding, and the staged one-shot release (begin/absorb/finish over an in-process verified-share session) are implemented as development evidence, and the proofless raw-share-generation and raw-recombination commands are feature-gated out of the default command surface. Two boundaries remain open and gate any certified use. First, the proof-backed reader currently binds the passive development setup package rather than the accepted, verifier-gated setup package, so the proof-backed release path is not yet wired to accepted setup; the kernel proof-backed tests are held pending that accepted-schema rebind, and a share must never be accepted against a package the accepted-setup verifier did not bless. Second, proof-backed release currently covers only the full-ranking `K_top = 20` target; top-k decryption for `1 <= K_top < 20` is unsupported, because the evaluator produces those targets below the canonical target-ciphertext level where CPAD-safe smudging headroom is infeasible, which is a scope reduction on the flagship tally-hiding output rather than a footnote. The C3 smudging statistical-distance certificate is not closed: bounded-leakage masks and smudging within certified support are not that certificate. One-shot release is enforced in-process only (an in-memory session store); persistent consumed-state across process restarts remains an open obligation. CPAD is mitigated by one-shot target-bound release plus mandatory smudging, never solved. WASM linear-memory pressure on this path is a known runtime constraint that has not been measured in this repository. Decryption-share proof accounting and certified recombination are unfinished. Current target-decryption code must be treated as development evidence only.                                                                                         |
