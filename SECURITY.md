...
- `SEC-006`: The unexported development bridge stops at source fixation and self-contained target finality, with a separate certificate-verified all-abstain no-result terminal. Finality binds the exact action, compiler, circuit, output, source, activation, and direct-finality identities, and its verifier recomputes the ordered signer roster before returning carrier-independent capability data. Both the earlier segmented evaluator and the attempted operation-fresh conjunction replacement are removed. The replacement exposed degree-three affine evaluations through publicly decryptable high-basis translation rows, enabling same-gate counterfactual continuation-key recovery. There is no admitted public end-to-end capability chain.
