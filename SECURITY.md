...
- `SEC-023`: A reduced one-bit/one-AND candidate has an internal conditional paper theorem, but its direct primitive assumptions and emitted implementation correspondence remain open. No full tally-preparation or private-evaluation construction is cryptographically admitted, and foundation verifiers do not yet compose into an admitted protocol.
