...
| `SEC-004` | `P1`     | The setup proof system does not carry a conventional 128-bit quantum soundness label.    | The setup proof families currently record roughly 140 conditional classical bits and about 70 quantum bits of soundness after the instance union, under the CS25 below-capacity FRI proximity-gap conjecture, which remains a recent conjecture rather than a proven bound; the stronger up-to-capacity proximity-gap conjecture was disproved in 2025 (Crites-Stewart, ePrint 2025/2046; BCHKS26). The fold/commit row is modelled by the current linear domain-size bound; the documented proven Johnson fallback may require a lower commit-phase number or a larger challenge field before it can replace that conditional row. The proven BCIKS20 Johnson fallback is tracked at a higher query count and is not the current byte-identical proof path. This remains below the conventional 128-bit quantum target and is scoped to proof publication time; the kernel enforces the implemented accounting floor, but it is not a 128-bit quantum claim. Ballot proofs and target-decryption share proofs need their own accepted accounting. |
| `SEC-005` | `P1`     | Setup proof zero knowledge is bounded leakage, not full 128-bit zero knowledge.          | Setup proofs disclose a family-aware bounded per-claim statistical distance rather than a full 128-bit zero-knowledge statement. The leakage-dominating recipient-private VSS family is about 2^-58 per claim and about 2^-40 across the first roster's adversary-visible claims; the other families are about 2^-68 per claim and about 2^-51 in total across the same view. The detailed derivations are maintainer evidence, not a public zero-knowledge approval. |
