...
| `SEC-004` | The row-code common-proof implementation has static geometry and masking evidence. Its aggregate commitment now uses bounded uniform 512-bit leaf-state stripes, but whole-prover WebAssembly liveness is not established and no full-width production proof or release-WebAssembly browser verification has completed.                                                                                                          | Static theorem and accounting evidence do not establish an operational proof path.                         |
| `SEC-005` | Exact classical and quantum-random-oracle soundness are not established. Focused certificate code inventories the deployed aggregate leaf calls and meets their uniform 512-bit collision allocation, but semantic state-transition correspondence and complete construction-level composition remain open. Family simulation, malicious-verifier zero knowledge, and quantum-random-oracle zero knowledge also remain unproved. | The prototype has no complete common-proof soundness, zero-knowledge, or ballot-privacy claim.             |
