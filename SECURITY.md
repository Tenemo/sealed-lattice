...
| `SEC-004` | The selected common-proof replacement has no completed theorem mapping for its exact typed transcript, explicit-point WHIR adapter, commitment layout, and verifier equations; its current soundness ledger is conditional.                                                      | A passing development proof does not establish the intended proof-system assurance.                        |
