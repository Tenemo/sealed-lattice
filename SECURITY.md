...
- `SEC-002`: The rejected public-semantic-map and action-wide-continuation experiment remains removed. A complete replacement now executes the ten-participant, ten-option tally for every `topCount` through Rust, scalar WebAssembly, the worker, segmented durable state, and verified result or no result. It uses no refreshed-wire semantic map or action-wide continuation pair. This is functional development and byte-correspondence evidence; the exact fixed-function, source, preparation, primitive, state, and static-quantum theorem remains incomplete, so no replacement has passed cryptographic admission, release-browser lifecycle closure, or selected-phone qualification.
- `SEC-003`: The current worker validates one retained ML-DSA-65 signing key and one retained ML-KEM-768 decapsulation key against the canonical roster on every load and persists exact preparation, source, finality, generation, evaluation, and terminal state before release. Segmented generation and evaluation have incompatible new object kinds and version-six action journals. Every chunk and checkpoint, the signed manifest, activation inventory, accepted chunk-set digest, and carrier-independent terminal are persist-before-return; exact replay, alternate requests, partial rollback, deleted records, and state loss are tested. Coordinated rollback of the entire browser-local root and all protected records remains an environmental assumption. Quota, eviction, cancellation, physical-storage reclamation, external release-browser behavior, and complete cleanup remain incomplete.
...
- `SEC-024`: The replacement now has emitted-object transfer, storage, checkpoint, worker-copy, WebAssembly-memory, scalar-work, visit, and foreground measurements for complete top-one and top-ten result ceremonies. The top-ten development run emitted a 304,336,370-byte activation-chunk corpus, used 319,304,184 bytes of measured origin storage and 15,990,784 bytes of WebAssembly memory, and completed all participants in five visits. Its whole guarded process tree peaked at 1,132,269,568 bytes, but that includes the test runner and child processes and is not the required browser-process private-memory increase over idle. The harness hosts all participant stores and the relay in one test page. True cold-page hostile-relay operation, quota, eviction, cancellation, cleanup, physical reclamation, external release Chrome, and selected-phone feasibility remain open.
...
- `SEC-029`: The removed implementation derived all garbling labels from one 32-byte activation seed while its working theorem assumed independent labels. The replacement closes that theorem-to-bytes mismatch by requesting every 320-bit label and point bit directly from browser platform randomness, rejecting equal pairs, and durably checkpointing the allocation before publication. No seed or label-corpus digest is emitted. Browser-CSPRNG security, durable generation chronology, collision conditioning, and the fixed-KMAC hidden-key games remain explicit admission terms; direct sampling does not prove them.
...
- `SEC-032`: The leading padded candidate masks each selected basis value with an independent pairwise-derived module value, derives continuation material separately for every gate and receiver, samples direct 320-bit labels, and uses KMAC256 row pads without publishing a label-corpus digest or refreshed-wire map. The same relation now emits the complete tally through production Rust, scalar WebAssembly, the worker, segmented durable generation and evaluation, and the terminal verifier for every `topCount`. An independent compiler and serialized-byte parser reproduces the full circuit, chunk plan, relation inventory, and result. Native and development-browser hostile tests cover serial multiplication, fan-out, reconvergence, multiple outputs, both selectors, corrupt rows and continuations, three malicious participants, forks, withholding, malformed encryption, replay, rollback, state loss, abstention, unusable ballots, and all abstention. This is implementation, correspondence, confluence-case, and resource evidence under direct primitive assumptions, not a reduction, independent cryptographic review, or admission.
- `SEC-033`: No applicable theorem proves the exact fixed-KMAC quantum interface. The reviewed outer-keyed-sponge theorem requires a uniformly random key longer than KMAC256's 1,048-bit rate, classical construction queries, and quantum ideal-permutation queries. The emitted interface uses 256- and 320-bit keys, coherent construction evaluation, and fixed public Keccak-f[1600], so the key-length premise already fails. The complete graph has 2,951,920 KMAC keys, 11,955,720 generation calls, 3,596,140 calls per verified full evaluation, maximum direct-label fan-out 332, and 5,977,860 hidden-key replacements. Standards vectors and passing tests fix bytes only. Row hiding, counterfactual-key hiding, adaptive continuation authentication, mailbox KDF, shared-Keccak composition, and fixed-AES terms remain direct assumptions, blocking static-quantum admission and every external/mobile qualification step.
