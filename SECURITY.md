...
- `SEC-023`: No malicious tally-preparation realization satisfies the stated adversary. The leading four-contributor structurally dealt degree-six zero-sharing source remains algebraically plausible but lacks its production verifier, simulator, and ledger; the subset-seeded fallback additionally lacks malicious key establishment and its multi-user fixed-Keccak theorem. More decisively, both reviewed adaptive-garbling compositions assume a statically correct base garbling while the removed component-contributed implementation allowed a corrupt participant to alter an unopened row without pre-activation detection. The direct scalar field-MPC repair also makes the required single contributor package exceed the absolute transport-stream bound before framing. No construction may mint verified preparation until a new full-garbling verifier satisfies the same theorem, chronology, and resource limits.
