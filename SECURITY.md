...
| `SEC-012` | `P1`     | The compact VSS commitment binding is a conditional Module-SIS assumption with no recorded lattice-estimator run. | The compact VSS setup path (compact coefficient, recipient-share, and aggregate-threshold commitments plus the share-linkage and same-secret bridge proofs) is the only accepted public VSS material in the kernel setup verifier; the terminal public-key-share and evaluation-key succinct proofs open the compact same-secret bridge's target constant commitments, so a setup package without the verified bridge is refused and full-material public VSS acceptance is unreachable. Acceptance is gated purely by recomputed roots and verified proof families. The compact commitment is computationally binding under Module-SIS on the short base-`3^17` digit witnesses: the kernel recomputes the no-wrap smallness precondition (per-digit difference bound `3^17 - 1 = 129,140,162`, below the commitment-modulus half-window `70,368,743,260,160`), but no `malb/lattice-estimator` run is recorded, so 128-bit binding is a target, not a measured value. The share-linkage and bridge proofs share the setup proof families' below-capacity CS25 FRI accounting (`SEC-004`) and bounded-leakage zero knowledge (`SEC-005`). A development check measures the compact coefficient commitment set as a small fraction of the former full-material coefficient baseline; the compact path is verified through the public SDK verifier. The compact share-linkage and same-secret bridge proof material streams through the setup-proof sidecar transport (the same chunked, stream-verified path the four other setup proof families use, bound by the per-record chunk hashes) rather than embedding in the package, so the canonical package stays encodable at production roster sizes; streaming changes how the proof bytes move, not their total volume (the share-linkage proof material is projected at roughly 1.1 GB at the production ring), which remains an open transport constraint that is not closed by streaming and must not be reported as transport-size closure. |
