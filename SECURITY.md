...
- `SEC-023`: No malicious tally-preparation realization satisfies the stated adversary. The reviewed independent-label compositions assume a statically correct base garbling while the removed component-contributed implementation allowed a corrupt participant to alter a row without pre-activation detection; their direct scalar field-MPC repair also exceeds the absolute transport-stream bound before framing. HSS/BOSS-style BMR is now a bounded research candidate because its published theorem permits evaluation-time error detection, but its post-quantum assumptions, preprocessing, public-transcript consistency, output masking, no-removal behavior, exact resources, and scalar implementation are unmapped. A replacement must either verify every row before activation or prove that every error choice occurs only at a theorem-permitted uniformly masked-input boundary, leaks no protected input through failure, and is positively detected before accepted evaluation, finality, or release. No such construction is selected or implemented.
