...
- `SEC-023`: A reduced one-bit/one-AND candidate has an internal conditional paper theorem. Its unexported action-key and private-preparation boundary now has Rust, scalar-WebAssembly, worker, and durable-state development evidence, but the direct primitive assumptions and complete source-to-result correspondence remain open. No full tally-preparation or private-evaluation construction is cryptographically admitted, and the implemented verifiers do not yet compose into an admitted protocol.
