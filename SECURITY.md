...
| `SEC-004`  | No production proof backend is selected. The replacement must close its complete hostile corpus, theorem and privacy composition, release WebAssembly path, browser lifecycle, and eleven remaining proof families. The rejected backend cannot satisfy any gate.    |
| `SEC-005`  | Emitted-byte zero knowledge, salted-Merkle privacy, proof-to-random-oracle composition, the fixed-tape shared-quantum-random-oracle premise, the initial-transition lemma, and complete probability accounting remain open. No security-bit total is authorized.     |
...
| `SEC-010`  | Homomorphic-encryption, verifiable-secret-sharing, and proof parameters remain provisional. The current topology is internally consistent but lacks a complete reviewed joint-exposure reduction.                                                                    |
...
| `SEC-019`  | Reproducible scalar WebAssembly and package bytes establish build identity only. The replacement lacks a complete release generation and verification pair and a frozen, non-authoritative evidence identity bound to every candidate input.                         |
