...
- full M9 encrypted aggregate bridge closure across the required `n=3..20`, `m=2..20` variant matrix; the public SDK exposes verification-only bridge checking, the cheap all-row shape/config matrix passes, and the focused subset passes implementation-evidence checks, but the full 342-row proof matrix, complete full-matrix negative suite, full benchmark report, and claim-bearing bridge closure remain pending
