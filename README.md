...
- Internal Rust owners now provide deterministic circuit semantics, the candidate scalar field and subset-stream arithmetic, salted subset, pair, and collective-coin source leaves, seed-catalog inclusion proofs, roster signatures, one-shot authorization verification, a complete semantic inventory of the ten authorized catalog roots, and an all-roster terminal verifier over that inventory. An unactivated sender-authenticated seed-mailbox slice uses roster-pinned ML-KEM-768 and ML-DSA-65 keys, fixed-output KMAC256 derivation, bounded AES-256-GCM-SIV chunks, and immediate root verification after decryption. A recipient verifier composes all nine authenticated streams and accepts one roster-pinned receipt signature over their ordered carrier identities and semantic inventory. Receipt production accepts only that typed complete authenticated inventory, checks the private signing key against the roster, requires fresh nonzero signing randomness, and positively verifies its own exact envelope. A further verifier requires one receipt at every roster position and all ten endorsements over one semantic receipt inventory. Before producing an endorsement, the endorser now verifies the complete signed receipt inventory, requires its exact public receipt body and carrier to match its retained authenticated local receipt, checks its signing key against the roster, and positively verifies the produced endorsement. The terminal proves common view only and grants no key-combination, coin-opening, burn, or continuation authority. A subsequent Rust-only join positively verifies all 94 local catalog openings against the terminal-selected local root, requires the participant's exact retained receipt body and randomized carrier to match the public terminal entry, consumes the nine authenticated remote inventories, and derives exactly 84 subset masters, nine pair masters, and one unopened local coin source. Those master types have no production raw constructor, subset-field generation accepts only the joined type, and the join can encode one zeroizing local-custody payload bound to both terminal carriers and the retained local receipt. The typed join itself remains an in-memory unactivated result with no burn, coin-opening, erasure, or preparation-continuation authority. An internal state-only owner samples every local catalog contribution and salt, anchors the complete raw inventory before kernel invocation, retains the exact root and canonical local openings, and checkpoints all nine canonical recipient plaintexts before returning root bytes. Other state-only owners reserve each sender stream before generation, reserve one recipient's exact local seed custody and receipt intent before receipt production, and reserve one participant-and-action terminal-endorsement slot over the complete receipt inventory, all ten exact receipt carriers, the retained-local receipt identities, and the terminal and authorization bodies before endorsement production. A further state-only owner accepts the exact completed source and recipient record bytes, requires its injected join validator to accept both terminal certificates and their provenance, and atomically replaces those two encrypted raw records with one encrypted joined payload. Cold resume revalidates that retained state, alternate terminal carriers conflict, and the returned metadata exposes no secret or continuation method. These owners are not connected to the Rust producers through WebAssembly, a production monotonic anchor, or the participant worker. Authenticated conflict-to-burn transitions, production joined-master restoration, physical reclamation, malicious key establishment, complete preparation verification, and the shared fixed-function reductions remain open.
- The source and joined-state owners establish persistence, atomic logical replacement, and byte-identical replay only; their current tests use injected model kernels and do not prove that production Rust derives the root, all nine plaintexts, or the retained joined payload from the authenticated state records. The collective-coin source and its commitment salt also lack their required verified degree-three sharing, authorized reconstruction, burn, checkpoint, and resource owners.
...
- The internal protocol runtime has authenticated copy-on-write storage, suite- and runtime-bound local record protection, checkpoint lineage and interrupted-publication repair, a strict-durability IndexedDB adapter, a fail-closed external-recency coordination interface, and the state-only seed-mailbox sender, recipient-receipt, and terminal-endorsement owners described above. No production monotonic anchor or Rust/WebAssembly adapter for those owners is selected, and persistence admission, quota and reclamation evidence, complete operation-specific cursors, the participant worker path, and physical-phone qualification remain missing. The public scalar WebAssembly package still exposes canonical foundation operations only.
