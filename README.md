...
- The organizer may close voting without waiting for every participant to cast a ballot. For `n` participants, let `f = floor((n - 1) / 3)`, the largest whole number below one third of `n`. Closing completes once `n-f` participants, including the organizer, respond, so up to `f` participants who never vote and leave cannot block it. Closing creates one verifiable inventory of on-time submissions, including invalid submissions with their deterministic classification and the exact accepted subset. Every accepted ballot is counted exactly once.
