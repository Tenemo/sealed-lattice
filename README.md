...
No complete preparation or evaluation construction is cryptographically admitted. An internal, unexported candidate cutout now implements action-key registration, private preparation delivery, complete-preparation verification, and one ballot-source fixation through Rust, scalar WebAssembly, a dedicated worker, and browser-local one-shot state. Development tests cover those boundaries only. Target finality, activation, evaluation, result verification, complete emitted-theorem correspondence, and mobile qualification remain absent. There is no end-to-end capability chain or active cryptographic suite. See the [open security issues](SECURITY.md#open-security-issues).

A test-only randomized direct-check reference flow was removed because complementary responses under one pad would reveal a protected source and it lacked a complete security theorem. A later exact-history audit found that its removed accepted-opening verifier did not itself make the two opposite responses reachable; this correction does not restore or validate that route. The replacement direction remains internal and conditional: its protocol, visit count, resource estimates, experimental primitives, and partial implementation are not part of the public package or evidence of end-to-end security.
