...
- Internal Rust owners now provide deterministic circuit semantics, the candidate scalar field and subset-stream arithmetic, salted subset, pair, and collective-coin source leaves, seed-catalog inclusion proofs, roster signatures, one-shot authorization verification, a complete semantic inventory of the ten authorized catalog roots, and an all-roster terminal verifier over that inventory. An unactivated sender-authenticated seed-mailbox slice uses roster-pinned ML-KEM-768 and ML-DSA-65 keys, fixed-output KMAC256 derivation, bounded AES-256-GCM-SIV chunks, and immediate root verification after decryption. Production KMAC paths use zeroizing absorbing, squeezing, padded-key, and output state while independent test implementations reproduce their bytes; persistent custody and physical browser-memory erasure remain open. A recipient verifier composes all nine authenticated streams and accepts one roster-pinned receipt signature over their ordered carrier identities and semantic inventory. Receipt production accepts only that typed complete authenticated inventory, checks the private signing key against the roster, requires fresh nonzero signing randomness, and positively verifies its own exact envelope. A further verifier requires one receipt at every roster position and all ten endorsements over one semantic receipt inventory. Before producing an endorsement, the endorser verifies the complete signed receipt inventory, requires its exact public receipt body and carrier to match its retained authenticated local receipt, checks its signing key against the roster, and positively verifies the produced endorsement. The terminal proves common view only and grants no key-combination, coin-opening, burn, or continuation authority. A subsequent Rust join positively verifies all 94 local catalog openings against the terminal-selected local root, requires the participant's exact retained receipt body and randomized carrier to match the public terminal entry, consumes the nine authenticated remote inventories, and derives exactly 84 subset masters, nine pair masters, and one unopened local coin source with its 64-byte commitment salt. Those master types have no production raw constructor, subset-field generation accepts only the joined type, and the join encodes one zeroizing local-custody payload bound to both terminal carriers and the retained local receipt. An internal state owner samples every local catalog contribution and salt, anchors the complete raw inventory before kernel invocation, retains the exact root and canonical local openings, and checkpoints all nine canonical recipient plaintexts before returning root bytes. Other state owners reserve each sender stream before generation, reserve one recipient's exact local seed custody and receipt intent before receipt production, and reserve one participant-and-action terminal-endorsement slot before endorsement production. A production scalar Rust/WebAssembly boundary now decodes the exact completed source and receipt records, re-verifies every roster root and receipt carrier plus both terminals, reconstructs each retained delivery against the selected root, checks local openings and byte-identical delivery-source correspondence, and emits the joined payload. The joined-state owner accepts only its module-branded adapter loaded against the package build's exact WebAssembly hash, atomically replaces both encrypted raw records, and revalidates retained public provenance on cold resume. A JavaScript object with matching method names is refused. After another recency-anchored read confirms raw-predecessor erasure, the owner may issue one opaque authorization for a Rust consumer to reverify and restore the typed inventory. The boundary remains unactivated and exposes no raw secret, source-consuming cursor, burn, coin-opening, erasure, or preparation-continuation capability.
- The pre-root source owner accepts only an integrity-pinned, module-branded scalar Rust/WebAssembly producer. Rust derives and revalidates the complete 94-leaf catalog and all nine recipient plaintexts from the exact durably retained contributions and salts; an exact completion-scale source production and validation run passes under Node/WebAssembly. The sender-stream owner likewise requires an integrity-pinned scalar adapter and a one-shot opaque authorization from that completed source owner. Rust redecodes the retained source record and public root terminal, selects each canonical recipient plaintext internally, produces and re-verifies the exact carrier, and accepts only a roster-key signature over the fixed manifest body. The recipient-receipt owner now requires an integrity-pinned scalar adapter and an opaque authorization minted only after Rust verifies the root terminal and all nine ordered sender-signed carriers, the browser-local signing and mailbox keys match the recipient's roster entry, and every decrypted plaintext matches its selected source root. Before private decapsulation, a separate owner recency-anchors the exact canonical open request under the context Rust actually verified. A genuine Rust-reported authenticated plaintext inconsistency or a conflicting durable receipt or terminal-endorsement intent replaces that selection with the same terminal action burn; malformed, unsigned, truncated, or publicly inconsistent transport leaves no burn, and the retained local record is not a public sender-misconduct certificate. Rust retains the exact authenticated inventory and receipt intent and verifies the fixed-context roster signature before returning one inert receipt envelope. Receipt custody accepts only a kernel opened through that exact authentication context and storage coordinator. The completed receipt owner issues a separate one-shot opaque authorization to the receipt-terminal endorsement adapter. That boundary redecodes the full local record, verifies all ten roots and all ten receipt signatures in canonical order, requires the endorser's exact public receipt carrier to match retained authenticated custody, retains the selected alternative in an opaque context, and verifies the fixed-context roster signature before returning one inert endorsement envelope. Both publication owners lock their exact intents before signing, suppress stale publication after a burn, and refuse shape-compatible JavaScript kernels. A successful joined-master transition atomically replaces the authentication selection with a compact joined marker while removing the raw source and receipt records, so the same recipient action cannot authenticate or publish again. Production codecs independently derive their Wasm request, response, cold-validation, custody, selection, burn, joined-marker, and transition lengths. Native tests exercise receipt and endorsement production from actual authenticated deliveries; packaged Node/WebAssembly coverage establishes their exact ABIs, integrity pins, and typed malformed-public-context refusals, while the joined adapter also exercises typed restoration validation. Successful completion-scale receipt, sender, endorsement, and join executions under WebAssembly remain absent. A complete action-wide index and head, later protocol burn owners, the sender action lifecycle, and a production recency anchor remain open. None of this is browser or phone evidence. The collective coin source and salt still lack verified degree-three sharing, authorized reconstruction, burn, checkpoint, and resource owners.
- Joined subset masters now feed a bounded scalar zero-sharing cursor whose workload is derived from the canonical completion circuit. It runs 504 canonical KMAC chunks for 8,403,192 field outputs, authenticates each inner checkpoint, requires output acknowledgement before advancing, and reproduces both output chunks byte-for-byte between native Rust and scalar Node/WebAssembly. This is development evidence only: the checkpoints lack durable encryption, authenticated predecessor ownership, rollback reconciliation, repair, and physical reclamation; the measured path excludes the all-roster codeword verifier; and no browser, phone, qPRF, fixed-Keccak, or malicious-preparation claim follows.
...
- The internal protocol runtime has authenticated copy-on-write storage, suite- and runtime-bound local record protection, checkpoint lineage and interrupted-publication repair, a strict-durability IndexedDB adapter, a fail-closed external-recency coordination interface, and the state owners described above. The recipient authenticated-inconsistency burn is durable under that abstract coordinator, but no production monotonic anchor connects the owners and the remaining burn transitions are absent. Persistence admission, quota and reclamation evidence, the remaining operation-specific cursors, the participant worker path, and physical-phone qualification remain missing. The public SDK still exposes canonical foundation operations only.
