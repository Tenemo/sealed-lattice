...
The leading reduced candidate uses fresh 320-bit labels, gate-specific continuation material, independent pairwise module pads on every selected basis row, and KMAC256 row masks. It emits no label-corpus digest or refreshed-wire semantic map. One exact four-input, seven-conjunction, three-output relation now runs through production Rust and the internal scalar-WebAssembly boundary with bounded direct entropy, canonical chunks and manifests, provisional one-time activation signatures, and terminal-only decoding. Independent hostile review found no deterministic row-level extraction but required corrections to one address sentinel, the exact signature game, quantum wrong-key accounting, global key-collision conditioning, and both-row hostile coverage. The corrected bytes, games, and native/scalar regressions passed the final reduced review. Body generation now re-verifies the complete signed preparation, derives receiver-and-gate-fresh degree-three material from the accepted subset keys, and derives every directed pairwise module pad from the new preparation masters through purpose-separated KMAC256 and AES addresses; it accepts no caller-supplied affine material. The version-three preparation plaintext and version-two context-bound source derivation are emitted and old versions fail closed. This closes only the reduced derivation correspondence. The internal emitter remains stateless, unexported, and unreachable from the live worker, which still stops at verified finality. Standard-primitive games and wrappers, durable one-use activation state, complete-tally composition, hostile relay execution, and browser evidence remain unfinished.
