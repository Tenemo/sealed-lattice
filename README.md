...
- The active sealed-lattice research direction uses a fixed tally circuit, independent wire labels, malicious all-roster preparation, publicly checkable garbling, certified evaluation, finality, and target-bound result-mask release. The latest reviewed dealer-based masked-degree-reduction realization was rejected because its codeword check did not enforce a required zero constant and its root-derived batch challenge lacked fresh entropy or a grinding reduction. The broader direction remains research; no exact preparation realization is selected, and its local circuit and algebra results do not form an emitted protocol or security theorem.
