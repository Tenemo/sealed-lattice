...
- The organizer may close voting without waiting for every participant to cast a ballot. Let `f` be the largest whole number below one third of the participant count: zero for three participants, three for ten, and six for twenty. Closing completes once the organizer and all but `f` of the other participants respond, so up to `f` participants who never vote and leave cannot block it. Closing creates one verifiable inventory of on-time submissions, including invalid submissions with their deterministic classification and the exact accepted subset. Every accepted ballot is counted exactly once.
- Neither the organizer nor a relay can choose which valid ballots count, with one bounded exception: a malicious relay, alone or with the organizer, can omit up to `f` ballots that it hid from the other participants before the close. Each affected voter is shown that its ballot was not included, and a ballot that reached at least `f+1` honest participants or an honest organizer is always counted. The [security policy](SECURITY.md#intended-security-model) describes the consequences.
