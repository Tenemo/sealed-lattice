...
- Internal Rust owners now provide deterministic circuit semantics, the candidate scalar field and subset-stream arithmetic, salted subset, pair, and collective-coin source leaves, seed-catalog inclusion proofs, roster signatures, one-shot authorization verification, a complete semantic inventory of the ten authorized catalog roots, and an all-roster terminal verifier over that inventory. An unactivated sender-authenticated seed-mailbox slice uses roster-pinned ML-KEM-768 and ML-DSA-65 keys, fixed-output KMAC256 derivation, bounded AES-256-GCM-SIV chunks, and immediate root verification after decryption. A recipient verifier composes all nine authenticated streams and accepts one roster-pinned receipt signature over their ordered carrier identities and semantic inventory. Receipt production now accepts only that typed complete authenticated inventory, checks the private signing key against the roster, requires fresh nonzero signing randomness, and positively verifies its own exact envelope. A further verifier requires one receipt at every roster position and all ten endorsements over one semantic receipt inventory. That terminal proves common view only and grants no key-combination, coin-opening, burn, or continuation authority. Internal state-only owners reserve each sender stream before generation and reserve one recipient's exact local seed custody, receipt intent, and signing seed before receipt production; both retain their exact public carrier before returning it through the authenticated recency coordinator. They are not connected to the Rust mailbox and receipt producers through WebAssembly, a production monotonic anchor, or the participant worker. Durable terminal-endorsement state, authenticated burn, malicious key establishment, the local/global seed join, source-authorized pair-stream and collective-coin masters, complete preparation verification, and the shared fixed-function reductions remain open.
