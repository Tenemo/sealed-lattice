...
- The one-slot submit-or-abstain lifecycle and verifier-driven preparation-to-result chain are not connected end to end. No malicious preparation or evaluation construction is admitted. Direct compressed coded-share garbling remains rejected because one corrupt participant can recover row selectors, wire masks, and the result before finality. The leading research chronology instead finalizes one deterministic computation target before any clear-output-capable online activation, then permits one clear-output protocol execution. Three published garbling mappings were rejected at their static quantum theorem gates. Only the first reached executable work, which was removed. A crate-private direct honest-majority MPC candidate now matches the independent tally semantics and derives its canonical carriers, interaction graph, fixed-function calls, checkpoints, hostile terminals, and static resource refusals. Its completion-scale scalar pseudorandom-sharing cursor has matched native and scalar WebAssembly outputs and typed refusals through authenticated checkpointing and forced-process cold restoration. Its positive seed-source and protocol verifiers, one-gate vertical slice, complete composed theorem, and runtime admission remain open. It is not an admitted construction.
- Focused scalar Node/WebAssembly development results now cover the current candidate's dominant pseudorandom-sharing kernel, but no matched full ceremony, browser lifecycle result, external-Chrome result, or selected-phone result exists.
