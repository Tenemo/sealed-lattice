...
The manual compact VSS measurement report separates implemented development artifact bytes: full-profile compact public commitment bodies remain `556,800` bytes versus `1,604,341,697` bytes of current full coefficient transport, a `2,881.360806393678x` public-body reduction. The static compact commitment work model remains `8,908,800` commitment residue multiply-adds plus `33,600` aggregate public-sum residue additions, for `8,942,400` modeled residue arithmetic operations. The latest isolated default runner measured the cached WASM compact commitment path at `6.56 s` warm generation and `6.60 s` warm verification extrapolated across `1,450` first-profile compact commitments, below the `30 s` development guards. The current default report verifies the compact share-linkage public statement and roots without transporting share-linkage proof bytes (`0` proof bytes in that section), and separately reports the reduced-ring same-secret bridge proof sample at `4,323,127` proof bytes and `5,767,439` proof-material JSON bytes. The opt-in full source-batch share-linkage proof-material measurement now emits `1,051,751` proof bytes for one source, `5,374,878` bytes for one source plus the bridge proof, `10,514,246` repeated all-source share-linkage proof bytes, `14,837,373` repeated all-source payload bytes with bridge proofs, and `20,591,104` proof-material JSON bytes. The one-source payload is under the `8 MiB` guard, and the repeated all-source payload is under the `64 MiB` setup-transport guard. The same run measured one-source proof generation at `4.52 s` warm median and verification at `0.96 s` warm median, plus `2.00 s` native material-set verification and `9.95 s` WASM material-set verification. Compact share-linkage proof-material transport and binary verifier paths remain development evidence because target-result release still requires target-ready compact commitment, proof-material, smudging, runtime, and release verification evidence.
...
The manual `pnpm run measure:compact-vss` CPU sanity runner replays one deterministic full-ring compact commitment through the TypeScript and Rust/WASM paths and prints the static byte accounting beside local wall-clock samples. It now fails if compact public commitment bodies miss the `2,800x` reduction floor, exceed the `64 MiB` public setup download budget, exceed compact public largest-object and WASM-copy budgets, exceed the measured target-decryption development artifact budget, exceed a `256 MiB` one-source private mailbox upload budget, exceed a `128 MiB` one-recipient private mailbox download budget, exceed a `16 MiB` one-recipient persistent local-state material budget, exceed a `10 s` private-state construction sample budget, exceed an `8 MiB` reduced-ring compact same-secret bridge proof payload budget, exceed a `16 MiB` compact bridge proof-material JSON budget, or if WASM warm full-profile generation or verification extrapolates above `30 s` on the local measurement host. When `SEALED_LATTICE_MEASURE_FULL_SHARE_LINKAGE_BATCH=1` is set, it also fails if the full source-batch one-source payload with the bridge proof exceeds `8 MiB` or if the repeated all-source payload with bridge proofs exceeds the `64 MiB` setup download budget. When `SEALED_LATTICE_MEASURE_TARGET_PROOF_MATERIAL=1` is set, it also fails if the heavy target proof-material run exceeds `16 MiB` of proof-material JSON, `8 MiB` of binary proof-material frame bytes, `12 MiB` of raw proof bytes, `180 s` generation, or `30 s` verification. The latest isolated local run measured `208.97 ms` for cold TypeScript seeded projection expansion plus commitment, then `15.00 ms` warm median commitment generation and `13.84 ms` warm median opening verification. Linear warm extrapolation across the `1,450` first-profile commitments is about `21.75 s` for commitment generation and `20.07 s` for opening verification in the TypeScript development path. The matching Rust/WASM command measured `42.87 ms` cold, `4.53 ms` warm median commitment generation, and `4.55 ms` warm median opening verification for full-ring compact commitment recomputation on the same host, with a `6.56 s` linear warm generation extrapolation and `6.60 s` linear warm verification extrapolation across `1,450` commitments, so the compact primitive and canonical body format remain comfortably within the local `30 s` WASM CPU guard. The same runner also records private-state TypeScript JSON construction samples; the latest mailbox sample is `1.83 s` for one full-ring source-recipient private mailbox delivery, and the one-source encrypted local-state sample builds in `4.94 s`. The same runner records the reduced-ring restricted compact same-secret bridge proof command: at ring degree `128` with seven target RNS limbs it emits a `4,323,127` byte proof, with `363.39 ms` warm median generation, `597.60 ms` warm median verification, and `665.99 ms` material-set verification. These proof measurements are restricted native/WASM command evidence only; they are not target-ready compact proof evidence and are not included in the static compact public commitment-body total.

Same-secret bridge evidence verification rejects embedded same-secret proof records whose `proofSizeBytes` or `proofBytesHash` do not match `proofBytesHex`, transported same-secret proof records whose proof-material root, full-object hash, chunk root, chunk hashes, size, or proof-byte hash do not match the supplied canonical base64 chunks, compact bridge statement sets whose `targetBasisHash` is not the kernel canonical target-basis hash, and compact bridge proof material records whose `proofBytesHash`, proof-record root, verifier-reconstructed lower-level proof statement, or material-set root does not match the supplied proof bytes and bound statement records. Compact proof-material verifiers compute proof-byte length from decoded proof bytes for reporting instead of accepting a stored record field. The accepted setup verifier now requires optional `compactSameSecretBridgeStatementSet` and `compactSameSecretBridgeProofMaterialSet` package fields to be supplied together and passes complete bridge material through restricted bridge proof verification with the bound same-secret evidence.

These measurements are development evidence, not a compact target-ready implementation. The lower-level native/WASM compact share-linkage proof command path is implemented only for the ternary-opening slice, including batched recipient/source-limb items inside one proof statement, and its proof material records have binding roots and per-record proof-byte hashes checked through the native/WASM material-set command. When compact share-linkage proof material carries matching packaged low-level proof statements, the same command verifies one restricted proof per proof record and requires coverage for every recipient and target limb under each source statement, counting both the primary proof item and additional batched items. Accepted setup refuses complete compact VSS public material as not binding under the current compact commitment profile, and rejects partial compact groups. The default compact same-secret bridge proof command and material-set command remain reduced-ring development paths; the opt-in accepted-ring same-secret bridge proof measurement proves the standalone bridge relation over the final ring with a `1,401,895` byte proof, `18.79 s` warm median generation, and `2.64 s` warm median verification, but accepted setup still refuses complete bridge material while compact VSS remains fail-closed. The active full source-batch proof material now fits the one-source proof-payload and all-source setup-transport guards, but that is still proof-byte and transport evidence for the restricted compact proof path, not a production compact-profile claim. Accepted setup bridge consumption under a replacement compact commitment, target-ready target-result release API, zero-knowledge coverage for released smudged decryption shares in the final proof profile, replacement compact commitment parameter security review, activation of a target-ready compact profile, and final target-profile native/WASM proof measurements remain unfinished.
