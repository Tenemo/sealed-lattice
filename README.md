...
A separate external release-Chrome resource proxy under mobile emulation stored, reread, and digested 305,130,327 high-entropy bytes, executed the exact generated 3,596,140-call scalar KMAC workload, reclaimed the same persistent browser profile to zero origin usage after restart, and measured a 500,867,072-byte browser-process private-memory increase over idle. These are development resource results for the unadmitted candidate. They are not the complete one-participant-at-a-time hostile-relay ceremony, mobile feasibility, physical-phone qualification, or activation.

The candidate now proceeds under three explicitly named fixed-KMAC256 games for fixed public Keccak-f[1600]. Direct-row hiding uses independently sampled labels. Counterfactual continuation hiding is separate and applies only after a gate-specific hidden stream proves that alternative key fresh while the complete selected transcript stays fixed. Continuation authentication is a known-key second-preimage game: the attacker knows the true selected key, ciphertext, context, and transcript before choosing a distinct reconstructed key. The accounting keeps adversarial coherent queries, honest protocol work, online checks, and offline candidate tests separate; it assigns no bare random-function probability or aggregate security-bit figure to fixed KMAC. These are assumptions, not established security claims. One conditional argument maps source privacy and extraction, malicious preparation, fixed AES and shorter KDFs, local state, losing forks, and straight-line static-quantum composition to exact lemmas or named direct assumptions. Independent byte-level and proof review, followed by the admission decision, remain incomplete.
