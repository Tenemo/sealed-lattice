...
No complete preparation or evaluation construction is cryptographically admitted. An internal, unexported candidate now exercises the full ten-participant, ten-option ceremony through Rust, scalar WebAssembly, a dedicated worker, and browser-local durable state. It covers submitted and abstaining participants, invalid but well-formed ballots, all structurally admitted result lengths, direct finality, one postfinality activation wave, segmented cold restore, and verified result or no result. This is conditional development evidence only: the experimental primitives and full argument still require genuinely independent review, release-browser lifecycle testing, reproducible exact-build freeze, and selected-phone qualification. There is no public end-to-end capability chain or active cryptographic suite. See the [open security issues](SECURITY.md#open-security-issues).

A test-only randomized direct-check reference flow was removed because complementary responses under one pad would reveal a protected source and it lacked a complete security theorem. A later exact-history audit found that its removed accepted-opening verifier did not itself make the two opposite responses reachable; this correction does not restore or validate that route. The replacement remains internal and conditional: its protocol, measured visit and resource profile, experimental primitives, and development ceremony are not part of the public package or evidence of production security.
