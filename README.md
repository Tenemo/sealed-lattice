...
- The former public masked-ballot candidate is rejected under a service-censorship attack. Direct sharing of the compiler-derived masked bundle is the only retained replacement candidate; encrypting the same bundle under a shared seed is rejected because it keeps the identical share-custody problem while adding KMAC, authenticated-encryption, ciphertext, nonce, and erasure obligations. The unactivated kernel derives the minimal masked-bundle width and bit order from the compiled circuit and rejects noncanonical padding across every configurable option count. A challenge-free symmetric-bivariate workbench now derives the roster-sized private rows, verifies local degree and pairwise crosspoint consistency, reconstructs from any `n - f(n)` mutually consistent released rows, and demonstrates that every completion-profile view of at most three rows is compatible with another bundle. This is algebraic development evidence conditional on future root binding and honest receipt checks. It authenticates no source and grants no release authority. No canonical commitment, cover-traffic, delivery, receipt, selected-set, release, or state object set and no positive release verifier has yet established private ballot presence and mutually exclusive included-payload or omitted-input release.
