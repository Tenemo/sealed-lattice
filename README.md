...
- The one-slot submit-or-abstain lifecycle and verifier-driven preparation-to-result chain are not connected end to end. No malicious preparation or garbling construction is selected or implemented. Direct compressed coded-share garbling remains rejected because one corrupt participant can recover row selectors, wire masks, and the result before finality. The leading research chronology instead finalizes one deterministic computation target before any clear-output-capable online activation, then permits one published clear-output BMR execution. The first BMR candidate has an unactivated production-derived compiler and matched scalar Rust/WebAssembly prime-field and PRF screen, but it still lacks a complete applicable theorem, ballot-source realization, one-gate positive verifier, browser evidence, and protocol admission.
