...
- Conditional independent-label preparation work includes scalar sharing, salted seed custody, authenticated private delivery, recipient inventories, pseudorandom zero-sharing workloads, hidden-bit checks, and direct ballot-custody models. These components are not a malicious-preparation theorem or `VerifiedTallyPreparation`, and construction-specific field-MPC work may be removed when a replacement construction is selected.
- The selected one-slot submit-or-abstain lifecycle, verifier-driven preparation-to-result chain, finality, and target-bound release are not connected end to end. The reviewed independent-label compositions fail the required malicious base-generation premise, their distributed full-pad repair does not supply a correct adaptive base or delayed threshold decoder, and HSS/BOSS-style garbling exposes output masks too early. The screened public-audit and selected-row protocols expose clear outputs too early or rely on privileged computation roles. A reviewed honest-majority packed-sharing protocol likewise reconstructs its output masks and clear result, while its delayed-output modification fails the exact four-share source-binding cutout: binding blocks simulator completion and absent binding permits corrupt release substitution. Direct transparent, lattice-toolkit, and streaming proof representations fail an exact theorem or scalar-browser memory gate. No preparation construction is selected or implemented.
