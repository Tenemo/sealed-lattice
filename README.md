...
The research target is at least 80 bits of modeled post-quantum attack work under the stated assumptions and limits. The exact internal candidate is conditionally admitted against that target under explicitly named direct assumptions; this is not an unconditional result, a production rating, an independent audit, or a certification.
...
The version-one operation-fresh padded transcript with 320-bit operation keys is a conditionally and internally cryptographically admitted candidate at identity `dfee7cd92b199269c24f2c87db2e6d021a721345e9b0274c53edcfb45632f1b5cfd4e41be5850eb68ec9f7b9e3f3b63aa74bb5299fb5e4dfd1f436613eba1382`. It exercises the real ten-participant, ten-option tally for every `topCount` from one through ten through production Rust, scalar WebAssembly, a dedicated worker, and browser-local durable state. It covers submitted and abstaining participants, unusable ballots, all abstention, direct eight-of-ten finality, one postfinality activation wave, segmented cold restore, corrupt and withheld messages, malformed cryptography, forks, rollback and state loss that leave local evidence, result uniqueness, and verified result or no result. The worker and finality verifier derive the roster, source set, circuit, output width, predecessor, and activation bindings from verified capabilities rather than trusting ambient agreement. The admission is conditional on the exact fixed-function, standardized-primitive, browser-randomness, and retained-state assumptions described below; it is not an independent human review or production approval.
...
The candidate proceeds under three explicitly named fixed-KMAC256 games for fixed public Keccak-f[1600]. Direct-row hiding uses independently sampled labels only after a compiler-topological producer step has hidden every earlier plaintext occurrence of the challenge label. Counterfactual continuation hiding is separate and applies only to honest receivers after a gate-specific hidden stream proves the alternative key fresh while the complete selected transcript stays fixed; corrupt receiver coordinates are adversarial inputs to confluence. Continuation authentication is a known-key second-preimage game: the attacker knows the true selected key, ciphertext, context, and transcript before choosing a distinct reconstructed key. Before those games, explicit local-HMAC and local-AEAD shadow assumptions replace storage-visible honest source and checkpoint records with equal-length zero-plaintext ciphertexts; exact retained plaintext is available only to honest restore after authenticating the complete protected-record inventory and accepting its state policy. The worker rejects any insertion, mixed epoch, or deletion or rollback that leaves an authenticated root or protected record before another protocol primitive or result. A coherent replay of a complete old root plus its matching inventory, and complete erasure of an initialized root plus all seven protected stores to the pristine empty state, are locally indistinguishable without an external continuity anchor and are explicitly excluded by the retained-state assumption. That assumption also treats the honest nonexportable root as an opaque capability that the storage adversary cannot synthesize, replace, or transplant with its own usable key; the current worker validates key type and usages but does not externally pin its identity. No lifecycle or qualification claim covers a stronger local-store adversary. The accounting keeps adversarial coherent queries, honest protocol work, online checks, offline candidate tests, and fixed-corruption challenge counts separate; it assigns no bare random-function probability or aggregate security-bit figure to fixed KMAC. Fixed AES is likewise an explicit real-versus-random-function assumption whose game absorbs permutation-to-function switching. These remain assumptions rather than published reductions. One conditional argument maps source privacy and extraction, malicious preparation, local state, losing forks, and compiler-topological static-quantum composition to exact lemmas or named direct assumptions. Separate fresh-session byte-attacker and proof/correspondence reviews found no unresolved accepted issue, and the row-by-row internal admission accepted those named premises at the emitted one-action multiplicities.

An independently calculated 192-byte operation-key projection remains rejected reopening evidence. Its ideal-permutation theorem does not establish security for fixed public Keccak-f[1600], while its maximum-width corpus would grow to 1,441,520,630 bytes, 4.74 times the current corpus. It is not a fallback, production direction, or emitted format. There is still no public end-to-end capability chain, active cryptographic suite, reproducible final candidate freeze, complete external release-browser qualification, or supported-phone result. See the [open security issues](SECURITY.md#open-security-issues).

A test-only randomized direct-check reference flow was removed because complementary responses under one pad would reveal a protected source and it lacked a complete security theorem. A later exact-history audit found that its removed accepted-opening verifier did not itself make the two opposite responses reachable; this correction does not restore or validate that route. The later semantic-map and reused-continuation-key experiment is also rejected and its activation commands, byte parser, worker API, durable activation records, and resource model have been removed. Its former command range and durable object kinds are reserved tombstones, and affine-bearing preparation and source records have new incompatible versions. A replacement may decode a complete masked multiplication word only after proving that word independent of private semantics, and may decode authorized terminal outputs; it must not expose a decoder for a refreshed private wire. The 320-bit padded vertical is conditionally internally admitted under the exact direct KMAC and other named assumptions, after fresh internal byte and proof review; this is not a published fixed-KMAC theorem or an independent human audit. Reproducible final freeze, complete external release-browser closure, and selected-phone qualification remain incomplete.
