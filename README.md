...
The attempted per-basis operation-fresh replacement is also rejected and has no executable surface. Although it rotated affine continuation material per gate and receiver, each selected high-basis translation row was decryptable from its public active label. Four exposed coordinates reconstruct the degree-three affine polynomial and therefore the opposite continuation key at the same gate. This recovers both refreshed-label alternatives without breaking a primitive. A materially different joint-aggregate relation now has one fixed, synthetic reduced experiment in production Rust and the internal scalar WebAssembly dispatcher. Its exact seven-gate conditional theorem and emitted-byte correspondence passed independent hostile review, but it is not exported, durable, authorized as a tally result, or a complete cryptographically admitted construction; the live worker still stops at verified finality. Full-tally expansion, source and state composition, and complete admission remain active work.
