...
- The pre-root source owner accepts only an integrity-pinned, module-branded scalar Rust/WebAssembly producer. Rust derives and revalidates the complete 94-leaf catalog and all nine recipient plaintexts from the exact durably retained contributions and salts; an exact completion-scale source production and validation run passes under Node/WebAssembly. The sender-stream owner likewise requires an integrity-pinned scalar adapter and a one-shot opaque authorization from that completed source owner. Rust redecodes the retained source record and public root terminal, selects each canonical recipient plaintext internally, produces and re-verifies the exact carrier, and accepts only a roster-key signature over the fixed manifest body. The completed recipient-receipt owner now issues a separate one-shot opaque authorization to an integrity-pinned receipt-terminal endorsement adapter. Rust redecodes that full local record, verifies all ten roots and all ten receipt signatures in canonical order, requires the endorser's exact public receipt carrier to match retained authenticated custody, retains the selected alternative in an opaque context, and verifies the fixed-context roster signature before returning one inert endorsement envelope. The durable endorsement owner locks that prepared alternative before signing and refuses shape-compatible JavaScript kernels. Production codecs independently derive its Wasm request, response, cold-validation, and custody lengths. Native tests exercise the positive path from actual authenticated deliveries; packaged Node/WebAssembly coverage establishes the exact ABI, integrity pin, and typed malformed-public-context refusal, not a successful completion-scale endorsement run. Receipt production, authenticated burn, the action-wide sender lifecycle, and a production recency anchor remain open. The joined-boundary fixture remains native; packaged Node/WebAssembly joined evidence remains refusal-only. None of this is browser or phone evidence. The collective coin and its salt still lack verified degree-three sharing, authorized reconstruction, burn, checkpoint, and resource owners.
