...
- direct encrypted ballot replacement path: the internal prototype can encrypt 20-score BGV ballots, privately check all data-limb BGV encryption equations against one shared score, encoding-carry, randomizer, and error witness, generate/verify a 13,374,704-byte internal binary proof per ballot for that all-limb encryption relation, score-linear constraints, one-hot Booleanity, and witness support, and aggregate supplied direct ballot ciphertexts with test-only aggregate score decryption. A native 10-ballot measurement passed with 133,747,040 total proof bytes and aggregate scores matching the plaintext oracle. It is still not replacement-ready because claim-bearing zero-knowledge and soundness accounting, practical evaluator replay, target projection correctness, and Node/WASM or mobile proof evidence remain unavailable;
...
- an internal direct encrypted ballot prototype command for feasibility discovery. It performs all-data-limb private encryption preflight, one shared-response validity proof assessment, and direct ciphertext aggregation with test-only aggregate score decryption. Native packed evaluator replay remains blocked by practical runtime and target correctness; an unpacked direct-pair evaluator candidate has only small all-options rank/order evidence and is not 20-option closure evidence. The command does not make Node/WASM verification or mobile evidence claim-bearing;
