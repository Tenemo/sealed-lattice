...
A fresh cryptographic review rejected that experiment's private-evaluation layer for two independent transcript failures. Public label metadata lets its evaluator decode private intermediate circuit values. Separately, one affine continuation pair is reused across all conjunctions: two distinct public masked gate polynomials recover the reused slope by exact polynomial division, and then recover both continuation keys. Observing both selector values is only a simpler special case. Per-gate hash framing does not rotate those underlying keys. The experiment also derives its label corpus from one activation seed while its working hidden-label argument assumed independent labels; any reuse needs a theorem matching the emitted generator. Successful end-to-end runs, visit counts, and resource measurements are therefore functional and historical evidence for rejected bytes, not a conditional security theorem or a basis for browser or phone qualification. The public foundation remains unchanged, and there is no public end-to-end capability chain or active cryptographic suite. See the [open security issues](SECURITY.md#open-security-issues).

The attempted per-basis operation-fresh replacement is also rejected and has no executable surface. Although it rotated affine continuation material per gate and receiver, each selected high-basis translation row was decryptable from its public active label. Four exposed coordinates reconstruct the degree-three affine polynomial and therefore the opposite continuation key at the same gate. This recovers both refreshed-label alternatives without breaking a primitive. A materially different joint-aggregate relation remains available only as one fixed, synthetic reduced experiment in production Rust and the internal scalar WebAssembly dispatcher. Its exact seven-gate conditional theorem and emitted-byte correspondence passed independent hostile review, but it is not exported, durable, authorized as a tally result, or a complete cryptographically admitted construction.

The leading reduced candidate uses fresh 320-bit labels, gate-specific continuation material, independent pairwise module pads on every selected basis row, and KMAC256 row masks. It emits no label-corpus digest or refreshed-wire semantic map. One exact four-input, seven-conjunction, three-output relation now runs through production Rust and the internal scalar-WebAssembly boundary with bounded direct entropy, canonical chunks and manifests, ML-DSA-65 activation signatures, and terminal-only decoding. Independent hostile review found no deterministic row-level extraction but required corrections to one address sentinel, quantum wrong-key accounting, global key-collision conditioning, and both-row hostile coverage. The corrected bytes, games, and native/scalar regressions passed the final reduced review. Body generation re-verifies the complete signed preparation, derives receiver-and-gate-fresh degree-three material from the accepted subset keys, and derives every directed pairwise module pad from preparation masters through purpose-separated KMAC256 and AES addresses; it accepts no caller-supplied affine material. The dedicated worker now samples and durably retains the relation's independent label pairs and allocation nonce before generation, atomically binds the sole manifest to the activation-signature slot, persists the exact signed package before returning it, and refuses alternate inputs, partial rollback, and lost activation state. Cold worker crashes at all three durable boundaries replay the same package.

The development vertical now uses one roster ML-DSA-65 key and one roster ML-KEM-768 mailbox key per participant. Each of the ninety preparation deliveries uses a fresh ML-KEM encapsulation and purpose-separated KMAC256-derived AES-256-GCM-SIV key and nonce. The worker verifies retained secret credentials against the canonical roster on every restore, co-publishes input-independent preparation with exact-roster confirmation, and retains byte-identical preparation, source, finality, and reduced activation publications. A source-controlled dependency enumeration covers all 120 seven-signer prefixes and 144,000 participant schedules; ordinary paths need at most five visits, all-abstain paths at most four, and the defined four-crash-plus-one-transport-repair bound reaches ten without concurrent participants. These are implementation, correspondence, lifecycle, and dependency-graph results under explicitly stated direct primitive assumptions, not cryptographic admission. The reduced activation method is internal and conditional: it consumes the fixed synthetic relation's local coordinates and masks and is neither a tally compiler nor an application capability. Complete-tally composition, the complete primitive and static-QPT argument, hostile relay execution, production resources and storage reclamation evidence, external release-browser evidence, and mobile qualification remain unfinished.
