...
| Common proof replacement    | Independent relation, matrix, source, and witness checks plus a compiler-derived unactivated construction inventory covering 12 families, 103 physical applications, and 159 logical relation instances; one exact native public-key-share proof candidate accepts, while a separately constructed equation-invalid proof refuses without minting a capability | Eleven compact family contracts, the complete scalar release-WebAssembly proof ABI, the final evidence identity, theorem and privacy composition, browser lifecycle, and full hostile corpus remain open; the candidate is not selected |
