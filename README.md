...
- the internal Rust/WASM kernel now has a local `VerifyPrivateVssShareEnvelope` command that rejects leaked coefficient messages, raw Shamir coefficients, per-coefficient randomness, and per-coefficient openings, then verifies each recipient share through carry-aware aggregate openings against public coefficient commitments and returns `PrivateVssShareEnvelopeHash` plus `PrivateVssLocalVerificationRoot` without exposing dealer coefficients;
- the internal crypto package now creates and decrypts private VSS mailbox envelopes using ML-KEM-768, HKDF-SHA-384, and AES-256-GCM, recomputes the AAD hash, KEM ciphertext hash, and AEAD ciphertext byte hash before key derivation or decryption, and the accepted setup verifier recomputes public encrypted-envelope byte hashes and binds recipient mailbox public-key hashes and public-key bytes hashes to setup-intent roster records; encrypted local trustee state now decodes through a positive typed sealed-payload schema with rejected unknown fields, setup epoch and device binding, aggregate threshold-share/opening roots, and `ThresholdShareCommitmentRecipientRoot` binding while keeping forbidden-field scanning as defense in depth; internal protocol helpers now sample per-dealer VSS coefficient opening state from a CSPRNG/random-byte source with one centered-ternary trustee secret polynomial shared across all `Q_share` limbs for Shamir coefficient zero, uniform non-constant per-RNS Shamir coefficients, and centered-ternary BDLOP/LNP opening randomness; those helpers produce signed setup phase participant objects and phase roots, per-dealer root-bound VSS coefficient contribution records and full public material, per-dealer encrypted private VSS mailbox delivery references, signed VSS share acceptance records, signed VSS complaint records with evidence roots derived from local private VSS verifier refusal details, root-bound same-secret statement sets from accepted VSS constant commitments, root-bound public-key share and proof statement sets from supplied coefficient hashes, root-bound evaluator-key schedule records from the verifier-exposed frozen schedule profile, roots-only local trustee state commitments, and encrypted storage envelopes bound to protocol-built roots-only local state commitments; the accepted-shaped Node/WASM setup fixture uses those helpers with deterministic test bytes to generate VSS opening state, build VSS commitment material, derive private envelope hashes, derive recipient local verification roots from aggregate openings, build same-secret statement sets, build public-key share/proof statement sets, and build evaluator-key schedule records before publishing only encrypted delivery objects and roots;
...
- accepted setup can optionally consume a root-bound `SameSecretProofSet` whose internal binary relation proof verifies that each trustee's VSS constant commitments open to one centered-ternary integer polynomial across all `Q_share` limbs, including the `-1 mod q_l = q_l - 1` wrap case, with fixed verifier-side response bounds; this binds proof bytes, proof byte hashes, statement hashes, challenges, proof roots, fixed setup-proof profile/challenge-domain/sampler record binding, the proof-set root, and the public VSS material root, can extract constant commitments from verifier-checked transported binary public VSS chunks when the package uses binary-rooted material, and can resolve same-secret proof bytes from root-bound binary proof-material chunks supplied with the verification request, but remains claim-pending until accepted LaZer/LNP algebraic verification, generated family parameters, soundness, zero-knowledge, and accepted proof-material review are complete;
- accepted setup public-key share records now require root-bound `PublicKeyShareSet` and `PublicKeyShareProofSet` objects that bind setup context, accepted public common-randomness/public-`a` roots, accepted same-secret statement roots, ordered per-limb share coefficient hashes, and proof statement roots; the verifier can optionally consume root-bound `PublicKeyShareMaterialSet` and `PublicKeyShareInternalProofSet` objects whose internal proof bytes check the VSS-bound trustee secret opening, centered-binomial error support, coefficient-vector hashes, fixed verifier-side response bounds, and the lifted public-key share relation with explicit no-wrap carry witnesses against derived public `a`; public-key internal proof material must now bind the verified `SameSecretProofSetRoot` and each per-trustee `SameSecretProofRoot`, and the claim-pending `CollectivePublicKey` aggregate also binds that same-secret proof-set root, but this remains claim-pending because accepted LaZer/LNP soundness, zero-knowledge, streamed proof material, and accepted aggregation from accepted proof-bearing shares are still open;
- accepted setup evaluator-key schedule records now require a root-bound `EvaluatorKeySchedule` object that binds setup context, accepted common-randomness relinearization and Galois CRP roots, same-secret and public-key share roots, frozen relinearization levels, frozen Galois rotations and levels, `RequiredGaloisSetHash`, explicit first-profile generic key-switch refusal, and `EvaluatorKeyScheduleRoot`; the verifier refuses premature raw or root-only collective public-key material, non-empty relinearization or Galois proof payloads, and evaluation keys until accepted same-secret LaZer/LNP, public-key, relinearization, and Galois proof verifiers exist;
...
- the Rust setup module now verifies carry-aware homomorphic commitment openings for VSS share vectors by combining coefficient commitments with trustee-point powers, checking the lifted integer share relation, and opening the combined commitment below the CRT commitment modulus product;
...
- the setup package verifier intentionally remains claim-pending until accepted same-secret LaZer/LNP proof verification, public-key share no-wrap proof verification, relinearization proofs, and Galois proofs are implemented.
...
- full-profile public VSS commitment material is now measured at 1,020 MiB, about 1.07 GB, of coefficient bytes before overhead; the setup verifier now enforces a binary/chunked certificate and manifest for that lower bound, package-level threshold-share verification plus same-secret proof verification can consume verifier-checked transported binary public VSS material, and same-secret proof verification can consume root-bound transported proof-byte chunks; production-grade non-JSON streaming/lazy loading and extension to public-key, relinearization, Galois, and evaluation-key material still remain before full-ring setup can be treated as mobile-viable;
- fixed accepted setup-proof family verifier implementation for public-key, relinearization, Galois, and any explicitly scheduled generic key-switch relations;
- accepted same-secret LaZer/LNP proof verification linking the accepted trustee secret commitments to public key, VSS, relinearization, Galois, generic key-switch material only if required, and future decryption shares; the current same-secret verifier is internal relation evidence with fixed setup-proof profile/challenge-domain/sampler binding, root-bound transported proof-byte support, and a strict tbox proof-byte decoder that retains decoded transcript fields, but without generated same-secret tbox parameters or algebraic AB-DLOP/LNP verification;
- accepted LNP public-key share proof-byte verification and collective public-key aggregation from accepted shares; the current optional internal public-key proof verifier and claim-pending aggregate verifier are lifted relation evidence only and do not close accepted proof-system or accepted aggregation requirements;
...
- setup and evaluation-key footprint reduction, package-level lazy loading from the enforced setup transport manifest, and extension of the transported binary path beyond public VSS material to proof and evaluation-key material;
