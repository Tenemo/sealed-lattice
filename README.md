...
- Internal Rust owners now provide deterministic circuit semantics, the candidate scalar field and subset-stream arithmetic, salted subset, pair, and collective-coin source leaves, seed-catalog inclusion proofs, roster signatures, one-shot authorization verification, and a complete semantic inventory of the ten authorized catalog roots. That inventory deliberately has no private-delivery or preparation authority. Durable state production, source-authorized pair-stream and collective-coin masters, authenticated delivery and receipts, malicious key establishment, complete preparation verification, and the shared fixed-function reductions remain open.
