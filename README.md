...
- Internal Rust owners now provide deterministic circuit semantics, the candidate scalar field and subset-stream arithmetic, salted subset, pair, and collective-coin source leaves, seed-catalog inclusion proofs, roster signatures, one-shot authorization verification, a complete semantic inventory of the ten authorized catalog roots, and an all-roster terminal verifier over that inventory. Seed-delivery verification requires the typed terminal, but neither result has authenticated-delivery or preparation authority. Durable state and endorsement production, source-authorized pair-stream and collective-coin masters, authenticated delivery and receipts, malicious key establishment, complete preparation verification, and the shared fixed-function reductions remain open.
