...
- The pre-root and publication state tests still use injected model kernels and establish persistence, atomic replacement, and byte-identical replay only. An exact native Rust completion fixture exercises the joined boundary from actual authenticated mailbox deliveries and retained local receipts; Node/WebAssembly evidence currently covers the exact ABI, integrity pin, and typed malformed-input refusals, not a successful completion join, browser execution, or phone feasibility. The collective-coin source and its commitment salt also lack their required verified degree-three sharing, authorized reconstruction, burn, checkpoint, and resource owners.
