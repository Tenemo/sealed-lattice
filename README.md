...
- Cryptographic implementation is incomplete. No exact-ten suite is frozen,
  ...
  backend, the compact successor remains test-only development code with only
  its bounded measurement owners available to the measurement feature, and
  no accepted vote or complete public ceremony API exists.
...
The ordinary scalar kernel and SDK bytes now reproduce at the hash recorded
below, but those release bytes omit compact proof generation and verification.
...
The compact successor's semantic, prover, transport, and hostile-verifier
workbench discussed below is compiled only for tests. The
`primitive-measurement-evidence` feature contains only the bounded measurement
owners and their operative ring-native arithmetic dependencies. The default
kernel includes neither scope. Both remain development evidence, not the active
ballot-validity or target-release proof path.
...
inventory union. None is a security result before the masking, emitted-byte,
...
matrices. The test-only compact verifier owners recompute canonical roots from
values, salts, and minimal frontiers. The simulator owner currently records
...
SHAKE256 call counts are `181,604`, `183,360`, `179,548`, and `183,288`.
The independently reconciled fixed-message candidate-slot counts are
`1,339,520`, `1,355,392`, `1,326,720`, and `1,358,464`; their seed-and-block
call counts are `181,522`, `183,280`, `179,470`, and `183,212` before adding
one prefix call per verifier move.
...
155.758 seconds in desktop Chromium and ended at
...
ledgers. The test-only factor-one source-level semantic owner binds all 82
transitions,
...
next work item. No factor is selected. After the production-shaped small-chain
and authenticated-restart gates pass, factor one is the first selected-size
execution target because it minimizes projected prover memory,
...
- The compact semantic, prover, transport, and hostile-verifier modules are
  test-only. The measurement feature contains only bounded owners and their
  ring-native arithmetic. No production packet compiler or generated-proof
  verifier can replace the default calls.
...
- The compact ring-vector candidate has no production packet compiler, exact
  ...
  CFW algebra and claim batching, the test-only factor-one source-level
  semantic owner,
...
- The latest retained complete closure graph is
  ...
  subsequent reproducibility gate rebuilt the scalar kernel and SDK
  twice and matched every package byte at WASM SHA-256
  `91e5a2c5d26f9f98a7e0afa797948769c1a648f84622a78b115e7b86edc47524`.
