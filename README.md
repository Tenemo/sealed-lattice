...
The kernel setup verifier accepts setup only through the VSS commitment path plus succinct share-linkage, same-secret bridge, public-key-share, and evaluation-key proof families, with acceptance gated purely on recomputed roots and verified proofs. This is development evidence only, verified through the public SDK setup verifier. The VSS commitment does not provide message binding: a concrete collision has been demonstrated against the deployed commitment map (two distinct canonical openings, identical commitment root), so the VSS path must not be treated as an accepted binding setup path until the commitment is redesigned. Setup proof material can stream through the setup-proof sidecar transport rather than embed in the package, so the canonical setup package stays encodable at production roster sizes; streaming changes how the bytes move, not their total volume, which remains a tracked transport and download constraint. See `SEC-008` and `SEC-012` in [SECURITY.md](SECURITY.md).
