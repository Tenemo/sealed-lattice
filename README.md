...
The leading paper candidate uses fresh 320-bit labels, gate-specific subset-derived continuation material, independent pairwise module pads on every selected basis row, and KMAC256 row masks. It emits no label-corpus digest or refreshed-wire semantic map. Exhaustive finite-field checks found no deterministic row-level extraction, and the real compiler's first-order maximum-output payload remains below the current absolute transfer and storage limits. This is a conditional design result only: its fixed-function assumptions, production parser, standard primitive wrappers, durable state, full-tally composition, hostile relay ceremony, and browser evidence are unfinished. The live worker still stops at verified finality.
