...
No complete preparation or evaluation construction is cryptographically admitted. An internal, unexported candidate cutout now implements action-key registration and private preparation delivery through Rust, scalar WebAssembly, a dedicated worker, and browser-local one-shot state. Development tests cover that boundary only. Ballot-source fixation, target finality, activation, evaluation, result verification, complete emitted-theorem correspondence, and mobile qualification remain absent. There is no end-to-end capability chain or active cryptographic suite. See the [open security issues](SECURITY.md#open-security-issues).
