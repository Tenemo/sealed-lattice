...
- The one-slot submit-or-abstain lifecycle and verifier-driven preparation-to-result chain are not connected end to end. No malicious preparation or garbling construction is admitted. Direct compressed coded-share garbling remains rejected because one corrupt participant can recover row selectors, wire masks, and the result before finality. The leading research chronology instead finalizes one deterministic computation target before any clear-output-capable online activation, then permits one published clear-output protocol execution. The first published BMR candidate was rejected because its classical simulator rewrites an extracted adversary view after online input exchange. The second published BMR family was also rejected at its exact theorem gate: its generic preprocessing proof reads a corrupt party's classical random tape, while its TinyOT proof programs a classical random oracle, and neither supplies the required retained-state QPT simulation. The first candidate's executable work was removed; no executable work was started for the second. The packed-sharing candidate is now under theorem review and is not an admitted construction.
