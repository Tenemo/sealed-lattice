...
- The one-slot submit-or-abstain lifecycle and verifier-driven preparation-to-result chain are not connected end to end. No malicious preparation or evaluation construction is admitted. Direct compressed coded-share garbling remains rejected because one corrupt participant can recover row selectors, wire masks, and the result before finality. The leading research chronology instead finalizes one deterministic computation target before any clear-output-capable online activation, then permits one clear-output protocol execution. Three published garbling mappings were rejected at their static quantum theorem gates. Only the first reached executable work, which was removed. A crate-private direct honest-majority MPC candidate now matches the independent tally semantics and derives its canonical carriers, interaction graph, fixed-function calls, checkpoints, hostile terminals, and static resource refusals. Its representative scalar WebAssembly execution, positive protocol verifiers, complete composed theorem, and runtime admission remain open. It is not an admitted construction.
- Focused scalar Node/WebAssembly results exist for other active foundations, but the current evaluation candidate has no matched scalar or browser ceremony, external-Chrome lifecycle result, or selected-phone result.
