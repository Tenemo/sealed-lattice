...
- The one-slot submit-or-abstain lifecycle, verifier-driven preparation-to-result chain, finality, and target-bound release are not connected end to end. No malicious preparation construction is selected or implemented. The latest coded-share proposal makes three corrupt row positions non-authoritative, but one corrupt participant can still recover row selectors, wire masks, and the result before finality. Its share-hidden repair is blocked at the independent delayed-release cutout: the bounded source screen found no applicable static-quantum theorem that both binds real four-of-ten releases to preparation and preserves the simulator's late honest-share completion freedom. No downstream garbling implementation is authorized, and prior rejected garbling and proof routes are not fallbacks.
