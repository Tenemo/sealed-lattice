...
The version-one operation-fresh padded transcript with 320-bit operation keys remains a functional full-tally vertical, but its former admitted identity `dfee7cd92b199269c24f2c87db2e6d021a721345e9b0274c53edcfb45632f1b5cfd4e41be5850eb68ec9f7b9e3f3b63aa74bb5299fb5e4dfd1f436613eba1382` is withdrawn. Those bytes use a finality quorum derived from an excluded complete-profile-rollback premise, allow all-abstain no result without every participant's required postfinality action, and lack an absorbing durable local-retirement state. The current internal repair derives seven-of-ten direct finality, requires ten purpose-separated acknowledgements of the semantic target before the kernel produces a no-result-release capability, and durably retires a detecting local action after attributable misconduct or observable partial state loss. Zero through nine acknowledgements remain pending; acknowledgement replay is persisted before publication; retirement erases live local authority and refuses every later participant operation; and a verified terminal is not revoked by later input. Production dispatch remains disabled while the new theorem mapping, hostile review, and runtime evidence are rebuilt.
