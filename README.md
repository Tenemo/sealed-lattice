...
- The one-slot submit-or-abstain lifecycle and verifier-driven preparation-to-result chain are not connected end to end. No malicious preparation or garbling construction is selected or implemented. Direct compressed coded-share garbling remains rejected because one corrupt participant can recover row selectors, wire masks, and the result before finality. The leading research chronology instead finalizes one deterministic computation target before any clear-output-capable online activation, then permits one published clear-output BMR execution. The first BMR candidate still requires an exact theorem mapping, production-derived message and resource graph, and scalar browser screen.
