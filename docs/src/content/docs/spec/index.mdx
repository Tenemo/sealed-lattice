...
- `@sealed-lattice/protocol`, `@sealed-lattice/crypto`, and `@sealed-lattice/wasm` are workspace-internal only.
...
- Reserved protocol digest derivation, ballot privacy proof verification, and `GF(65537)` field checks exposed through the internal WASM loader must remain covered by Node and browser tests.
