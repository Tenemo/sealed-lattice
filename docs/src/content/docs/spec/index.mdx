...
- The selected protocol direction is a mobile-first direct encrypted ballot workflow where every participant is also a trustee.
...
- The packaged verification path must keep working in both Node and browser tests.
- Active setup, direct ballot proof, encrypted aggregation, evaluator replay, target finality, and target-bound decryption checks must stay on the selected direct path as those surfaces are implemented.
