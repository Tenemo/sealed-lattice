...
- The current public runtime facade exposes transcript core fixture verification plus deterministic protocol shell helpers.
- No public subpaths are promised yet.
- The published package exists to stabilize packaging, docs, smoke checks, transcript core fixtures, protocol shell vectors, and release flow before wider APIs are introduced.
...
- Node tests, browser tests, SDK surface drift checks, vector manifest verification, docs verification, pack smoke checks, and release-smoke checks must all continue to pass.
