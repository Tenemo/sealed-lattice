...
type SetupProofFieldCertificate = Readonly<{
    basePrimeFactors: readonly bigint[];
    modulus: bigint;
    pocklingtonWitnesses: readonly bigint[];
    powerBase: bigint;
    powerExponent: bigint;
}>;

export const candidateSetupProofFieldCertificate: SetupProofFieldCertificate = {
    basePrimeFactors: [2n, 7n, 105_929n],
    modulus:
...
export const compileCandidateSetupProofFieldCensus = (
    certificate: SetupProofFieldCertificate = candidateSetupProofFieldCertificate,
): CandidateSetupProofFieldCensus => {
    const {
        basePrimeFactors,
        modulus,
        pocklingtonWitnesses,
        powerBase,
        powerExponent,
    } = certificate;
    if (powerBase ** powerExponent + 1n !== modulus) {
    ...
    const factoredModulusMinusOne = basePrimeFactors.reduce(
        (product, factor) => product * factor ** powerExponent,
        1n,
    );
    if (
        factoredModulusMinusOne !== modulus - 1n ||
        factoredModulusMinusOne ** 2n <= modulus
    ) {
        throw new Error('The Pocklington factored divisor is insufficient.');
    }
    for (let index = 0; index < basePrimeFactors.length; index += 1) {
        ...
        if (
            exponentiate(witness, modulus - 1n, modulus) !== 1n ||
            greatestCommonDivisor(
                exponentiate(witness, (modulus - 1n) / primeFactor, modulus) -
                    1n,
                modulus,
            ) !== 1n
        ) {
    ...
    if ((modulus - 1n) % transformOrder !== 0n) {
        throw new Error('The proof field lacks the required transform root.');
    }
    const modulusBitLength = BigInt(modulus.toString(2).length);
    if (
        modulus < compileFheKeyIntegerEmbeddingBounds().minimumProofFieldModulus
    ) {
        throw new Error('The proof field can wrap a bounded FHE key equation.');
    }
    return {
