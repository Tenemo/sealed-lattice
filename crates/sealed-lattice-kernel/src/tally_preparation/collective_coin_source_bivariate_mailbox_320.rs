...
use super::{
    collective_coin_source_bivariate_commitment_320::{
        ...
        COLLECTIVE_COIN_SOURCE_BIVARIATE_ML_DSA_65_SIGNATURE_BYTE_LENGTH,
...
pub(crate) const COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_ALGORITHM_IDENTIFIER: &str =
    "ml-kem-768+kmac256+aes-256-gcm-siv";
pub(crate) const COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_HEADER_DOMAIN: &str =
    "sealed-lattice/v1/preparation/collective-coin/bivariate-private-row-mailbox-header";
pub(crate) const COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_HEADER_IDENTITY_DOMAIN: &str =
    "sealed-lattice/v1/preparation/collective-coin/bivariate-private-row-mailbox-header-identity";
pub(crate) const COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_RECIPIENT_KEY_IDENTITY_DOMAIN: &str = "sealed-lattice/v1/preparation/collective-coin/bivariate-private-row-mailbox-recipient-key-identity";
pub(crate) const COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_ASSOCIATED_DATA_DOMAIN: &str =
    "sealed-lattice/v1/preparation/collective-coin/bivariate-private-row-mailbox-associated-data";
pub(crate) const COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_CARRIER_DIGEST_DOMAIN: &str =
    "sealed-lattice/v1/preparation/collective-coin/bivariate-private-row-mailbox-carrier-digest";
pub(crate) const COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_MANIFEST_DOMAIN: &str =
    "sealed-lattice/v1/preparation/collective-coin/bivariate-private-row-mailbox-manifest";
pub(crate) const COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_MANIFEST_IDENTITY_DOMAIN: &str =
    "sealed-lattice/v1/preparation/collective-coin/bivariate-private-row-mailbox-manifest-identity";
pub(crate) const COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_SIGNATURE_ENVELOPE_DOMAIN: &str = "sealed-lattice/v1/preparation/collective-coin/bivariate-private-row-mailbox-signature-envelope";
pub(crate) const COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_SIGNATURE_CONTEXT: &[u8] =
    b"sealed-lattice/v1/preparation/collective-coin/bivariate-private-row-mailbox-manifest";
pub(crate) const COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_KEY_DERIVATION_LABEL: &[u8] =
    b"sealed-lattice/v1/preparation/collective-coin/bivariate-private-row-mailbox-aead-key";
pub(crate) const COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_NONCE_DERIVATION_LABEL: &[u8] =
    b"sealed-lattice/v1/preparation/collective-coin/bivariate-private-row-mailbox-aead-nonce";

pub(crate) const COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_AUTHENTICATION_TAG_BYTE_LENGTH: usize =
    16;
pub(crate) const COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_ML_KEM_CIPHERTEXT_BYTE_LENGTH: usize =
    ml_kem_768::CT_LEN;
pub(crate) const COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_HEADER_BODY_BYTE_LENGTH: usize =
    CANONICAL_TUPLE_HEADER_BYTE_LENGTH
        ...
        + COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_HEADER_DOMAIN.len()
        + CANONICAL_VARIABLE_VALUE_LENGTH_PREFIX_BYTE_LENGTH
        + COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_ALGORITHM_IDENTIFIER.len()
        + 3 * Hash512::BYTE_LENGTH
        + 3 * size_of::<u16>()
        + COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_ML_KEM_CIPHERTEXT_BYTE_LENGTH
        + 2 * size_of::<u64>();
pub(crate) const COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_ASSOCIATED_DATA_BYTE_LENGTH: usize =
    CANONICAL_TUPLE_HEADER_BYTE_LENGTH
        + 6 * CANONICAL_ITEM_HEADER_BYTE_LENGTH
        + CANONICAL_VARIABLE_VALUE_LENGTH_PREFIX_BYTE_LENGTH
        + COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_ASSOCIATED_DATA_DOMAIN.len()
        + 3 * Hash512::BYTE_LENGTH
        + 2 * size_of::<u64>();
pub(crate) const COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_SIGNATURE_ENVELOPE_BYTE_LENGTH: usize =
    CANONICAL_TUPLE_HEADER_BYTE_LENGTH
        + MAILBOX_SIGNATURE_ENVELOPE_ITEM_COUNT * CANONICAL_ITEM_HEADER_BYTE_LENGTH
        + CANONICAL_VARIABLE_VALUE_LENGTH_PREFIX_BYTE_LENGTH
        + COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_SIGNATURE_ENVELOPE_DOMAIN.len()
        + Hash512::BYTE_LENGTH
        + COLLECTIVE_COIN_SOURCE_BIVARIATE_ML_DSA_65_SIGNATURE_BYTE_LENGTH;

pub(crate) fn collective_coin_source_bivariate_mailbox_manifest_body_byte_length(
    participant_count: u16,
) -> Result<usize, CollectiveCoinSourceBivariateMailboxError320> {
    ...
    let item_count = MAILBOX_MANIFEST_PREFIX_ITEM_COUNT
        .checked_add(
            participant_count
                .checked_mul(2)
                .ok_or(CollectiveCoinSourceBivariateMailboxError320::ArithmeticOverflow)?,
        )
        .ok_or(CollectiveCoinSourceBivariateMailboxError320::ArithmeticOverflow)?;
    CANONICAL_TUPLE_HEADER_BYTE_LENGTH
        .checked_add(
        ...
        .and_then(|length| {
            length.checked_add(COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_MANIFEST_DOMAIN.len())
        })
...
pub(crate) enum CollectiveCoinSourceBivariateMailboxError320 {
    Canonical(CanonicalCodecError),
    Commitment(CollectiveCoinSourceBivariateCommitmentError320),
    ObjectMismatch {
        field: &'static str,
    },
    RosterMismatch,
...
impl From<CanonicalCodecError> for CollectiveCoinSourceBivariateMailboxError320 {
    fn from(error: CanonicalCodecError) -> Self {
        Self::Canonical(error)
    }
}

impl From<CollectiveCoinSourceBivariateCommitmentError320>
    for CollectiveCoinSourceBivariateMailboxError320
{
    fn from(error: CollectiveCoinSourceBivariateCommitmentError320) -> Self {
        Self::Commitment(error)
    }
}

impl fmt::Display for CollectiveCoinSourceBivariateMailboxError320 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Canonical(error) => {
                write!(formatter, "canonical collective-coin source mailbox error: {error}")
            }
            Self::Commitment(error) => {
                write!(formatter, "collective-coin source mailbox source error: {error}")
            }
            Self::ObjectMismatch { field } => {
                write!(
                    formatter,
                    "collective-coin source mailbox object has a wrong {field}"
                )
            }
            Self::RosterMismatch => formatter
                .write_str("collective-coin source mailbox roster does not match the commitment layout"),
            Self::HolderPositionOutOfRange {
                holder_position,
                participant_count,
            } => write!(
                formatter,
                "collective-coin source mailbox holder {holder_position} is outside participant count {participant_count}"
            ),
            Self::HeaderCount { expected, actual } => write!(
                formatter,
                "collective-coin source mailbox manifest has {actual} headers; expected {expected}"
            ),
            Self::CarrierCount { expected, actual } => write!(
                formatter,
                "collective-coin source mailbox package has {actual} carriers; expected {expected}"
            ),
            Self::EncapsulationRandomnessCount { expected, actual } => write!(
                formatter,
                "collective-coin source mailbox has {actual} encapsulation-randomness values; expected {expected}"
            ),
            Self::CarrierByteLength { expected, actual } => write!(
                formatter,
                "collective-coin source mailbox carrier has {actual} bytes; expected {expected}"
            ),
            Self::CarrierDigestMismatch {
                holder_position,
            } => write!(
                formatter,
                "collective-coin source mailbox carrier for holder {holder_position} does not match the signed manifest"
            ),
            Self::MalformedEncapsulationKey => formatter
                .write_str("collective-coin source mailbox roster contains a malformed ML-KEM-768 key"),
            Self::MalformedEncapsulationCiphertext => formatter
                .write_str("collective-coin source mailbox contains a malformed ML-KEM-768 ciphertext"),
            Self::DecapsulationKeyMismatch => formatter.write_str(
                "collective-coin source mailbox decapsulation key does not match the roster holder key",
            ),
            Self::DecapsulationFailed => {
                formatter.write_str("collective-coin source mailbox ML-KEM-768 decapsulation failed")
            }
            Self::AuthenticatedEncryptionFailed => {
                formatter.write_str("collective-coin source mailbox AES-256-GCM-SIV encryption failed")
            }
            Self::AuthenticatedDecryptionFailed => {
                formatter.write_str("collective-coin source mailbox AES-256-GCM-SIV authentication failed")
            }
            Self::MalformedSigningVerificationKey => formatter
                .write_str("collective-coin source mailbox roster contains a malformed ML-DSA-65 key"),
            Self::InvalidContributorSignature => formatter
                .write_str("collective-coin source mailbox manifest has an invalid contributor signature"),
            Self::SignatureByteLength { expected, actual } => write!(
                formatter,
                "collective-coin source mailbox signature has {actual} bytes; expected {expected}"
            ),
            Self::IntegerConversion => formatter
                .write_str("collective-coin source mailbox integer does not fit its canonical width"),
            Self::ArithmeticOverflow => {
                formatter.write_str("collective-coin source mailbox arithmetic overflowed")
            }
            Self::StreamingHash => {
                formatter.write_str("collective-coin source mailbox streamed hash framing failed")
            }
        }
    }
}

impl std::error::Error for CollectiveCoinSourceBivariateMailboxError320 {}

/// Public root-bound control bytes for one fixed private-row carrier.
#[derive(Clone, PartialEq, Eq)]
pub(crate) struct CollectiveCoinSourceBivariateMailboxHeaderBody320 {
    ...
    encapsulation_ciphertext:
        [u8; COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_ML_KEM_CIPHERTEXT_BYTE_LENGTH],
    plaintext_byte_length: u64,
    carrier_byte_length: u64,
}

impl CollectiveCoinSourceBivariateMailboxHeaderBody320 {
    fn new(
        ...
        encapsulation_ciphertext: [u8;
            COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_ML_KEM_CIPHERTEXT_BYTE_LENGTH],
    ) -> Result<Self, CollectiveCoinSourceBivariateMailboxError320> {
        let layout = root_body.layout();
        validate_roster_for_layout(layout, roster)?;
        let recipient_entry = require_roster_holder(roster, layout, holder_position)?;
        let plaintext_byte_length = u64::try_from(
            collective_coin_source_bivariate_private_row_body_byte_length(
                layout.participant_count(),
            )?,
        )
        .map_err(|_| CollectiveCoinSourceBivariateMailboxError320::IntegerConversion)?;
        let carrier_byte_length = plaintext_byte_length
            .checked_add(
                u64::try_from(
                    COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_AUTHENTICATION_TAG_BYTE_LENGTH,
                )
                .map_err(|_| CollectiveCoinSourceBivariateMailboxError320::IntegerConversion)?,
            )
            .ok_or(CollectiveCoinSourceBivariateMailboxError320::ArithmeticOverflow)?;
    ...
    pub(crate) fn canonical_bytes(
        &self,
    ) -> Result<Vec<u8>, CollectiveCoinSourceBivariateMailboxError320> {
        Ok(CanonicalTuple::new(
            CANONICAL_TUPLE_SCHEMA_IDENTIFIER,
            CANONICAL_TUPLE_VERSION,
            vec![
                CanonicalItem::nonempty_ascii(
                    COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_HEADER_DOMAIN,
                )?,
                CanonicalItem::nonempty_ascii(
                    COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_ALGORITHM_IDENTIFIER,
                )?,
    ...
    fn from_canonical_bytes(
        expected_root: &AuthorAuthenticatedCollectiveCoinSourceBivariateCommitmentRoot320,
        roster: &Roster,
        expected_holder_position: u16,
        bytes: &[u8],
    ) -> Result<Self, CollectiveCoinSourceBivariateMailboxError320> {
        ...
        let recipient_entry = require_roster_holder(roster, layout, expected_holder_position)?;
        ...
        Ok(Self {
            ...
            participant_count: layout.participant_count(),
            contributor_position: layout.contributor_position(),
            holder_position: expected_holder_position,
    ...
    pub(crate) fn identity(&self) -> Result<Hash512, CollectiveCoinSourceBivariateMailboxError320> {
        Ok(hash_foundation_tuple_512(
            COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_HEADER_IDENTITY_DOMAIN,
            &[CanonicalItem::variable_bytes(self.canonical_bytes()?)?],
        )?)
    }
}

impl fmt::Debug for CollectiveCoinSourceBivariateMailboxHeaderBody320 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("CollectiveCoinSourceBivariateMailboxHeaderBody320")
...
/// One contributor-signed, roster-ordered inventory for every fixed-shape carrier.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct CollectiveCoinSourceBivariateMailboxManifestBody320 {
    layout_identity: Hash512,
    root_body_identity: Hash512,
    participant_count: u16,
    contributor_position: u16,
...
impl CollectiveCoinSourceBivariateMailboxManifestBody320 {
    fn new(
        root_body: &CollectiveCoinSourceBivariateCommitmentRootBody320,
        headers: &[CollectiveCoinSourceBivariateMailboxHeaderBody320],
        carrier_digests: Vec<Hash512>,
    ) -> Result<Self, CollectiveCoinSourceBivariateMailboxError320> {
        let layout = root_body.layout();
        let expected_count = usize::from(layout.participant_count());
        if headers.len() != expected_count {
            return Err(CollectiveCoinSourceBivariateMailboxError320::HeaderCount {
                expected: expected_count,
                actual: headers.len(),
            });
        }
        if carrier_digests.len() != expected_count {
            return Err(CollectiveCoinSourceBivariateMailboxError320::CarrierCount {
                expected: expected_count,
        ...
        for (holder_index, header) in headers.iter().enumerate() {
            let holder_position = u16::try_from(holder_index)
                .map_err(|_| CollectiveCoinSourceBivariateMailboxError320::IntegerConversion)?;
            if header.layout_identity != layout.identity()
                ...
                || header.contributor_position != layout.contributor_position()
                || header.holder_position != holder_position
            {
    ...
    pub(crate) fn canonical_bytes(
        ...
        items.extend([
            CanonicalItem::nonempty_ascii(
                COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_MANIFEST_DOMAIN,
            )?,
            CanonicalItem::hash512(self.layout_identity.into_bytes()),
            CanonicalItem::hash512(self.root_body_identity.into_bytes()),
            CanonicalItem::unsigned16(self.participant_count),
...
/// Detached contributor signature over the exact aggregate carrier manifest.
#[derive(Clone, PartialEq, Eq)]
pub(crate) struct CollectiveCoinSourceBivariateMailboxSignatureEnvelope320 {
    manifest_identity: Hash512,
    signature: [u8; COLLECTIVE_COIN_SOURCE_BIVARIATE_ML_DSA_65_SIGNATURE_BYTE_LENGTH],
}

impl CollectiveCoinSourceBivariateMailboxSignatureEnvelope320 {
    ...
    pub(crate) fn canonical_bytes(
        ...
        Ok(CanonicalTuple::new(
            ...
            vec![
                CanonicalItem::nonempty_ascii(
                    COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_SIGNATURE_ENVELOPE_DOMAIN,
                )?,
    ...
    fn from_canonical_bytes(
        ...
        let signature = read_fixed_bytes::<
            COLLECTIVE_COIN_SOURCE_BIVARIATE_ML_DSA_65_SIGNATURE_BYTE_LENGTH,
        >(&tuple.items[2], "signature")?;
        Ok(Self {
...
impl fmt::Debug for CollectiveCoinSourceBivariateMailboxSignatureEnvelope320 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("CollectiveCoinSourceBivariateMailboxSignatureEnvelope320")
...
pub(crate) struct SealedCollectiveCoinSourceBivariateMailboxPackage320 {
    headers: Box<[CollectiveCoinSourceBivariateMailboxHeaderBody320]>,
    encrypted_row_carriers: Box<[Zeroizing<Vec<u8>>]>,
    manifest: CollectiveCoinSourceBivariateMailboxManifestBody320,
}

impl SealedCollectiveCoinSourceBivariateMailboxPackage320 {
    pub(crate) fn headers(&self) -> &[CollectiveCoinSourceBivariateMailboxHeaderBody320] {
        &self.headers
    }
    ...
    pub(crate) const fn manifest(&self) -> &CollectiveCoinSourceBivariateMailboxManifestBody320 {
        &self.manifest
    }
}

impl fmt::Debug for SealedCollectiveCoinSourceBivariateMailboxPackage320 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("SealedCollectiveCoinSourceBivariateMailboxPackage320")
...
pub(crate) fn seal_collective_coin_source_bivariate_mailbox_package_320(
    inventory: &CollectiveCoinSourceBivariateCommitmentInventory320,
    roster: &Roster,
...
> {
    ...
    if encapsulation_randomness.len() != expected_count {
        return Err(
            CollectiveCoinSourceBivariateMailboxError320::EncapsulationRandomnessCount {
                expected: expected_count,
        ...
        encapsulation_randomness.iter().enumerate()
    {
        ...
        let encapsulation_key =
            ml_kem_768::EncapsKey::try_from_bytes(recipient_entry.mailbox_encapsulation_key)
                .map_err(|_| {
                    CollectiveCoinSourceBivariateMailboxError320::MalformedEncapsulationKey
                })?;
        ...
        let header = CollectiveCoinSourceBivariateMailboxHeaderBody320::new(
            root_body,
            roster,
            holder_position,
        ...
        let cipher =
            Aes256GcmSiv::new_from_slice(authenticated_encryption_key.as_ref()).map_err(|_| {
                CollectiveCoinSourceBivariateMailboxError320::AuthenticatedEncryptionFailed
            })?;
        let private_row = inventory.private_row_body(holder_position)?;
        let mut encrypted_row_carrier = private_row.canonical_bytes()?;
        ...
        if encrypted_row_carrier.len() != expected_carrier_byte_length {
            return Err(
                CollectiveCoinSourceBivariateMailboxError320::CarrierByteLength {
    ...
    let manifest = CollectiveCoinSourceBivariateMailboxManifestBody320::new(
        root_body,
        &headers,
        carrier_digests,
    )?;
    Ok(SealedCollectiveCoinSourceBivariateMailboxPackage320 {
...
/// Positive contributor signature over one aggregate carrier manifest.
///
...
/// continuation exists.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct AuthorAuthenticatedCollectiveCoinSourceBivariateMailboxManifest320 {
    manifest: CollectiveCoinSourceBivariateMailboxManifestBody320,
    manifest_identity: Hash512,
}

impl AuthorAuthenticatedCollectiveCoinSourceBivariateMailboxManifest320 {
    ...
    pub(crate) const fn contributor_position(&self) -> u16 {
        self.manifest.contributor_position
    }
...
pub(crate) fn verify_collective_coin_source_bivariate_mailbox_manifest_signature_320(
    authenticated_root: &AuthorAuthenticatedCollectiveCoinSourceBivariateCommitmentRoot320,
    roster: &Roster,
    manifest_bytes: &[u8],
    signature_envelope_bytes: &[u8],
) -> Result<
    AuthorAuthenticatedCollectiveCoinSourceBivariateMailboxManifest320,
    CollectiveCoinSourceBivariateMailboxError320,
> {
    let layout = authenticated_root.root_body().layout();
    validate_roster_for_layout(layout, roster)?;
    let manifest = CollectiveCoinSourceBivariateMailboxManifestBody320::from_canonical_bytes(
        authenticated_root,
        manifest_bytes,
    )?;
    let manifest_identity = manifest.identity()?;
    let signature_envelope =
        CollectiveCoinSourceBivariateMailboxSignatureEnvelope320::from_canonical_bytes(
            manifest_identity,
            signature_envelope_bytes,
        )?;
    let contributor_entry = require_roster_holder(roster, layout, layout.contributor_position())?;
    let verification_key = ml_dsa_65::PublicKey::try_from_bytes(
        contributor_entry.signing_verification_key,
    )
    .map_err(|_| CollectiveCoinSourceBivariateMailboxError320::MalformedSigningVerificationKey)?;
    if !verification_key.verify(
        manifest_bytes,
        &signature_envelope.signature,
        COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_SIGNATURE_CONTEXT,
    ) {
        return Err(CollectiveCoinSourceBivariateMailboxError320::InvalidContributorSignature);
    }
    Ok(
        AuthorAuthenticatedCollectiveCoinSourceBivariateMailboxManifest320 {
            manifest,
...
/// failure is attributable to the contributor-signed carrier package, except for a
/// locally mismatched decapsulation key.
pub(crate) struct AuthorAuthenticatedCollectiveCoinSourceBivariateMailboxCarrier320 {
    header: CollectiveCoinSourceBivariateMailboxHeaderBody320,
    manifest_identity: Hash512,
    carrier_digest: Hash512,
    encrypted_row_carrier: Vec<u8>,
}

impl fmt::Debug for AuthorAuthenticatedCollectiveCoinSourceBivariateMailboxCarrier320 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AuthorAuthenticatedCollectiveCoinSourceBivariateMailboxCarrier320")
...
pub(crate) fn verify_collective_coin_source_bivariate_mailbox_public_carrier_320(
    authenticated_root: &AuthorAuthenticatedCollectiveCoinSourceBivariateCommitmentRoot320,
    authenticated_manifest: &AuthorAuthenticatedCollectiveCoinSourceBivariateMailboxManifest320,
    roster: &Roster,
    expected_holder_position: u16,
    header_bytes: &[u8],
    encrypted_row_carrier: &[u8],
) -> Result<
    AuthorAuthenticatedCollectiveCoinSourceBivariateMailboxCarrier320,
    CollectiveCoinSourceBivariateMailboxError320,
> {
    require_manifest_matches_root(authenticated_root, authenticated_manifest)?;
    let layout = authenticated_root.root_body().layout();
    validate_roster_for_layout(layout, roster)?;
    let header = CollectiveCoinSourceBivariateMailboxHeaderBody320::from_canonical_bytes(
        authenticated_root,
        roster,
        expected_holder_position,
        header_bytes,
    )?;
    let holder_index = usize::from(expected_holder_position);
    let expected_header_identity = authenticated_manifest
        .manifest
        .ordered_header_identities
        .get(holder_index)
        .ok_or(
            CollectiveCoinSourceBivariateMailboxError320::HolderPositionOutOfRange {
                holder_position: expected_holder_position,
                participant_count: layout.participant_count(),
            },
        )?;
...
/// Positive contributor-authenticated, confidential, root-matched local custody.
///
...
/// authority.
pub(crate) struct AuthenticatedCollectiveCoinSourceBivariateMailboxDelivery320 {
    layout_identity: Hash512,
    root_body_identity: Hash512,
    manifest_identity: Hash512,
    ...
    contributor_position: u16,
    holder_position: u16,
    authenticated_private_row: AuthenticatedCollectiveCoinSourceBivariatePrivateRow320,
    retained_private_row_body_bytes: Zeroizing<Vec<u8>>,
}

impl AuthenticatedCollectiveCoinSourceBivariateMailboxDelivery320 {
    pub(crate) const fn layout_identity(&self) -> Hash512 {
...
impl fmt::Debug for AuthenticatedCollectiveCoinSourceBivariateMailboxDelivery320 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AuthenticatedCollectiveCoinSourceBivariateMailboxDelivery320")
            .field("layout_identity", &self.layout_identity)
            .field("root_body_identity", &self.root_body_identity)
            .field("manifest_identity", &self.manifest_identity)
...
pub(crate) fn complete_collective_coin_source_bivariate_mailbox_delivery_320(
    authenticated_root: &AuthorAuthenticatedCollectiveCoinSourceBivariateCommitmentRoot320,
    roster: &Roster,
    public_carrier: AuthorAuthenticatedCollectiveCoinSourceBivariateMailboxCarrier320,
    recipient_decapsulation_key: &ml_kem_768::DecapsKey,
) -> Result<
    AuthenticatedCollectiveCoinSourceBivariateMailboxDelivery320,
    CollectiveCoinSourceBivariateMailboxError320,
> {
    let layout = authenticated_root.root_body().layout();
    validate_roster_for_layout(layout, roster)?;
    if public_carrier.header.layout_identity != layout.identity()
        ...
        || public_carrier.header.contributor_position != layout.contributor_position()
    {
        return Err(mailbox_object_mismatch("public carrier root scope"));
    }
    let holder_position = public_carrier.header.holder_position;
    ...
    let encapsulation_ciphertext =
        ml_kem_768::CipherText::try_from_bytes(public_carrier.header.encapsulation_ciphertext)
            .map_err(|_| {
                CollectiveCoinSourceBivariateMailboxError320::MalformedEncapsulationCiphertext
            })?;
    let shared_secret = recipient_decapsulation_key
        .try_decaps(&encapsulation_ciphertext)
        .map_err(|_| CollectiveCoinSourceBivariateMailboxError320::DecapsulationFailed)?;
    let shared_secret_bytes = Zeroizing::new(shared_secret.into_bytes());
    ...
    let cipher = Aes256GcmSiv::new_from_slice(authenticated_encryption_key.as_ref())
        .map_err(|_| CollectiveCoinSourceBivariateMailboxError320::AuthenticatedDecryptionFailed)?;
    cipher
        ...
        .map_err(|_| CollectiveCoinSourceBivariateMailboxError320::AuthenticatedDecryptionFailed)?;
    let authenticated_private_row = verify_collective_coin_source_bivariate_private_row_320(
        authenticated_root,
        &private_row_body_bytes,
    )?;
    if authenticated_private_row.holder_position() != holder_position
        || authenticated_private_row.contributor_position() != layout.contributor_position()
...
fn require_manifest_matches_root(
    authenticated_root: &AuthorAuthenticatedCollectiveCoinSourceBivariateCommitmentRoot320,
    authenticated_manifest: &AuthorAuthenticatedCollectiveCoinSourceBivariateMailboxManifest320,
) -> Result<(), CollectiveCoinSourceBivariateMailboxError320> {
    let layout = authenticated_root.root_body().layout();
    if authenticated_manifest.manifest.layout_identity != layout.identity()
        ...
        || authenticated_manifest.manifest.contributor_position != layout.contributor_position()
    {
...
) -> Result<Hash512, CollectiveCoinSourceBivariateMailboxError320> {
    Ok(hash_foundation_tuple_512(
        COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_RECIPIENT_KEY_IDENTITY_DOMAIN,
        &[
...
fn derive_mailbox_associated_data(
    ...
    let associated_data = CanonicalTuple::new(
        ...
        vec![
            CanonicalItem::nonempty_ascii(
                COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_ASSOCIATED_DATA_DOMAIN,
            )?,
    ...
    .encode()?;
    if associated_data.len() != COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_ASSOCIATED_DATA_BYTE_LENGTH
    {
...
fn hash_mailbox_carrier(
    ...
    let mut hasher = StreamingFoundationTupleHash512::new_variable_bytes(
        COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_CARRIER_DIGEST_DOMAIN,
        &[
            CanonicalItem::hash512(header.layout_identity.into_bytes()),
            CanonicalItem::hash512(header.root_body_identity.into_bytes()),
            CanonicalItem::unsigned16(header.holder_position),
            CanonicalItem::hash512(header.identity()?.into_bytes()),
        ],
        encrypted_row_carrier.len(),
    )
    .map_err(|_| CollectiveCoinSourceBivariateMailboxError320::StreamingHash)?;
    hasher
...
#[cfg(test)]
pub(super) fn hash_collective_coin_source_bivariate_mailbox_carrier_for_test(
    header: &CollectiveCoinSourceBivariateMailboxHeaderBody320,
    encrypted_row_carrier: &[u8],
) -> Result<Hash512, CollectiveCoinSourceBivariateMailboxError320> {
...
fn validate_roster_for_layout(
    ...
    roster
        .validate()
        .map_err(|_| CollectiveCoinSourceBivariateMailboxError320::RosterMismatch)?;
    ...
    {
        return Err(CollectiveCoinSourceBivariateMailboxError320::RosterMismatch);
    }
    Ok(())
}
...
fn require_roster_holder(
    ...
    layout: CollectiveCoinSourceBivariateCommitmentLayout320,
    holder_position: u16,
) -> Result<&crate::foundation::RosterEntry, CollectiveCoinSourceBivariateMailboxError320> {
    roster
        .entries
        .get(usize::from(holder_position))
        .filter(|entry| entry.roster_position == holder_position)
        .ok_or(
            CollectiveCoinSourceBivariateMailboxError320::HolderPositionOutOfRange {
                holder_position,
                participant_count: layout.participant_count(),
...
fn require_recipient_decapsulation_key(
    ...
    if !bool::from(
        ...
        return Err(CollectiveCoinSourceBivariateMailboxError320::DecapsulationKeyMismatch);
    }
    Ok(())
}
...
fn read_fixed_bytes<const BYTE_LENGTH: usize>(
    item: &CanonicalItem,
    field: &'static str,
) -> Result<[u8; BYTE_LENGTH], CollectiveCoinSourceBivariateMailboxError320> {
    if item.item_type() != CanonicalItemType::RawBytes {
        return Err(mailbox_object_mismatch(field));
    }
    item.canonical_bytes().try_into().map_err(|_| {
        if field == "signature" {
            CollectiveCoinSourceBivariateMailboxError320::SignatureByteLength {
...
fn read_u16(
    item: &CanonicalItem,
    field: &'static str,
) -> Result<u16, CollectiveCoinSourceBivariateMailboxError320> {
...
fn read_u64(
    item: &CanonicalItem,
    field: &'static str,
) -> Result<u64, CollectiveCoinSourceBivariateMailboxError320> {
...
const fn mailbox_object_mismatch(
    ...
    CollectiveCoinSourceBivariateMailboxError320::ObjectMismatch { field }
}
...
const _: () =
    assert!(COLLECTIVE_COIN_SOURCE_BIVARIATE_MAILBOX_ML_KEM_CIPHERTEXT_BYTE_LENGTH == 1_088);
