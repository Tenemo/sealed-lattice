...
use super::{
    ...
    SetupPublicPolynomialRootRole, VerifiedEvaluatorAuxiliaryRoot,
...
impl VerifiedGaloisSourceMaterialBatch {
    pub(crate) fn from_consumed_common_proof(
                ...
                .enumerate()
        {
            ...
            let expected_root_source_ordinal = material
                .topology()
                .data_block_count()
                .checked_mul(material.topology().extended_limb_count())
                .and_then(|common_reference_count| common_reference_count.checked_add(2))
                .and_then(|source_stride| source_stride.checked_mul(entry_ordinal))
