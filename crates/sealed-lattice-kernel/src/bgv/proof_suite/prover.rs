...
pub(crate) struct CommonProofGenerationStateMachine<'input> {
    ...
    quotient_components: Vec<Vec<ProofChallengeExtensionElement>>,
...
impl<'input> CommonProofGenerationStateMachine<'input> {
        ...
        match &active.leaf_source {
            ...
            leaf_source => {
                ...
                let row_width = match leaf_source {
                    ...
                    CommonProofTreeLeafSource::QuotientComponent(_)
                ...
                match leaf_source {
                    ...
                    CommonProofTreeLeafSource::QuotientComponent(component_index) => {
    ...
    pub(crate) fn new(
        ...
        if maximum_prefetched_query_byte_length == 0
            || maximum_external_memory_chunk_byte_length == 0
        {
        ...
        Ok(Self {
            ...
            maximum_prefetched_query_byte_length,
            maximum_output_fragment_byte_length: usize::try_from(
                maximum_external_memory_chunk_byte_length,
            )
            .map_err(|_| {
                CommonProofGenerationInitializationError::Prover(
    ...
    pub(crate) fn checkpoint_boundary(&self) -> Option<CommonProofGenerationStage> {
        (self.active_tree_materialization.is_none()
            ...
            && !matches!(
                self.phase,
                CommonProofGenerationPhase::Complete | CommonProofGenerationPhase::Cancelled
            ))
        .then(|| self.stage())
    }
        ...
        BoundOpenings: CommonProofBoundOpeningProvider,
    {
        ...
        match self.phase {
            ...
            CommonProofGenerationPhase::DerivingApplicationColumns => {
                ...
                self.phase =
                    CommonProofGenerationPhase::MaterializingAuxiliaryTrees { next_tree_index: 0 };
                Ok(CommonProofGenerationPoll::ArithmeticStepCompleted)
            }
            CommonProofGenerationPhase::MaterializingAuxiliaryTrees { next_tree_index } => {
                let next_tree = self
            ...
            CommonProofGenerationPhase::ConstructingQuotient => {
                ...
                .map_err(CommonProofGenerationError::Prover)?;
                self.quotient_components = construct_quotient_components(
                    self.variant,
                    self.relation_context,
                    &quotient,
                    coins,
            ...
            CommonProofGenerationPhase::MaterializingQuotientTrees {
                ...
                if self.quotient_components.get(next_component_index).is_none() {
                ...
                self.prepare_tree_materialization(
                    ...
                    CommonProofTreeLeafSource::QuotientComponent(next_component_index),
                    CommonProofTreeContinuation::Quotient {
            ...
            CommonProofGenerationPhase::DerivingDeepOpenings => {
                ...
                self.deep_evaluations = deep_evaluations;
            ...
            CommonProofGenerationPhase::PreparingFri => {
                ...
                let initial_fri_polynomial = construct_initial_fri_polynomial(
                    ...
                    &self.quotient_components,
                    self.opening_batch_mask.as_deref(),
                    &self.opening_points,
                    &self.deep_evaluations,
                    &opening_batch_coefficients,
                )
                .map_err(CommonProofGenerationError::Prover)?;
                let initial_fri_evaluations = self
                    .evaluation_domain
                ...
                self.quotient_components.clear();
            ...
            CommonProofGenerationPhase::FoldingFri { next_fold_ordinal } => {
                ...
                let (next_domain, next_evaluations) =
                    construct_next_fri_layer(&current_evaluations, current_domain, challenge)
                        .map_err(CommonProofGenerationError::Prover)?;
...
/// Previous stack-local implementation retained temporarily as a differential
/// oracle while the persistent state machine is exercised by the complete
/// proof round-trip tests.  It is not called by production code.
fn generate_common_proof_stack_local_reference<Storage, Coins, Sink, BoundOpenings>(
    input: CommonProofGenerationInput<'_>,
    storage: &mut Storage,
    coins: &mut Coins,
    sink: &mut Sink,
