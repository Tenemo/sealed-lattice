...
pub(in crate::bgv::proof_suite) struct ExactPolynomialProtocolExtractorCertificate {
    ...
    prior_vss_bound_codeword_count: usize,
    direct_bound_codeword_count: usize,
...
impl ExactPolynomialProtocolExtractorCertificate {
    pub(in crate::bgv::proof_suite) fn is_complete(&self) -> bool {
        self.construction_plan_identity_hash != [0_u8; 64]
            ...
            && self.authenticated_source_polynomial_count == 2_018
            && self.raw_authenticated_source_coefficient_position_count == 33_128_448
            ...
            && self.synthetic_division_identity_count > 0
            && self.actual_pole_rejection_count == 1
    }

    pub(in crate::bgv::proof_suite) const fn construction_plan_identity_hash(&self) -> [u8; 64] {
...
pub(in crate::bgv::proof_suite) struct ExactPointConstraintExtractorCertificate {
    ...
    coordinate_authorities: BTreeSet<ExactPointCoordinateAuthority>,
...
impl ExactPointConstraintExtractorCertificate {
    pub(in crate::bgv::proof_suite) fn is_complete(&self) -> bool {
        self.construction_plan_identity_hash != [0_u8; 64]
            && self.relation_plan_variant_hash != [0_u8; 64]
            && self.explicit_point_count == 3
            && self.explicit_point_variable_count == EXACT_TABLE_VARIABLE_COUNT
            && self.fixed_aggregate_selector_coordinate_count == 1
            && self.row_selector_coordinate_count == EXACT_ROW_SELECTOR_VARIABLE_COUNT
            && self.coefficient_reduction_coordinate_count == EXACT_BOUND_POLYNOMIAL_VARIABLE_COUNT
            && self.coordinate_authorities
                == BTreeSet::from([
                    ExactPointCoordinateAuthority::TranscriptOpeningPoint,
                    ExactPointCoordinateAuthority::TranscriptRowSelector,
                    ExactPointCoordinateAuthority::PolynomialOpeningReduction,
                ])
            && self.proof_supplied_point_coordinate_count == 0
            && self.opening_point_aggregate_columns == [0, 1, 2]
            ...
            && self.proof_created_tree_opening_claim_count
                ...
                == self.relation_opening_claim_count
            && self.selector_equality_identity_count == 3 * LOGICAL_POLYNOMIALS_PER_PHYSICAL_ROW
    }
...
fn checked_trace_phase_polynomial_mapping(
    ...
    let mut expected_columns = BTreeSet::new();
    for tree in variant.ordered_trees() {
        ...
        for column_ordinal in ordered_column_ordinals {
            if !expected_columns.insert(*column_ordinal) {
                return Err("exact proof-created relation column is duplicated".to_owned());
            }
        }
    }
    ...
    for row in &phase.rows {
        for chunk in row.logical_polynomial_chunks.iter().flatten() {
            if chunk.coefficient_chunk_ordinal != 0
                || !observed_columns.insert(chunk.column_ordinal)
            {
                return Err("exact trace phase does not define a polynomial bijection".to_owned());
            }
            let mut expected_opening_points = variant
                .ordered_opening_claims()
                .iter()
                .filter(|claim| {
                    claim.source_class() == RelationOpeningSourceClass::TreeColumn
                        && claim.column_ordinal() == Some(chunk.column_ordinal)
                })
                .map(|claim| claim.opening_point_ordinal())
                .collect::<Vec<_>>();
            expected_opening_points.sort_unstable();
            expected_opening_points.dedup();
            if expected_opening_points != row.opening_point_ordinals {
                return Err("exact trace phase changes a column opening pattern".to_owned());
            }
        }
    }
    if drop_first_polynomial {
        let first = observed_columns
            .first()
            .copied()
            .ok_or_else(|| "exact trace phase has no polynomial to remove".to_owned())?;
        observed_columns.remove(&first);
    }
...
fn checked_quotient_phase_polynomial_mapping(
    ...
    let mut quotient_coordinates = BTreeSet::new();
    let mut mask_coordinates = BTreeSet::new();
        ...
        for chunk in row.logical_polynomial_chunks.iter().flatten() {
            ...
            match chunk.source {
                RowCodeWhirOpenedPolynomialSource::QuotientComponent { component_ordinal } => {
                    if !quotient_coordinates.insert((component_ordinal, coordinate.0, coordinate.1))
                    {
                        return Err("exact quotient component coordinate is duplicated".to_owned());
                    }
                }
                RowCodeWhirOpenedPolynomialSource::OpeningBatchMask { mask_ordinal } => {
                    if mask_ordinal != 0 || !mask_coordinates.insert((coordinate.0, coordinate.1)) {
                        return Err("exact opening-mask coordinate is duplicated".to_owned());
                    }
                }
            }
        }
    ...
    if quotient_coordinates != expected_quotient_coordinates
        || mask_coordinates != expected_mask_coordinates
    {
        return Err("exact quotient phase changes the polynomial-coordinate map".to_owned());
    }
...
fn checked_bound_polynomial_mapping(
    ...
    let mut expected_columns = BTreeSet::new();
    let mut expected_input_tree_count = 0_usize;
    let mut expected_output_tree_count = 0_usize;
    for tree in variant.ordered_trees() {
        let RelationTreeDescriptor::BoundPublic {
            root_use,
            ordered_column_ordinals,
        ...
        match root_use {
            BoundTreeRootUse::Input => expected_input_tree_count += 1,
            BoundTreeRootUse::Output => expected_output_tree_count += 1,
        }
        for column_ordinal in ordered_column_ordinals {
            if !expected_columns.insert(*column_ordinal) {
                return Err("exact bound relation column is duplicated".to_owned());
            }
        }
    }
    let mut observed_columns = BTreeSet::new();
    let mut observed_input_tree_count = 0_usize;
    let mut observed_output_tree_count = 0_usize;
    for tree in &plan.bound_trees {
        match (tree.root_use, tree.low_degree_mode, tree.query_count) {
            (
                BoundTreeRootUse::Input,
                RowCodeWhirBoundLowDegreeMode::PriorVssProofRequired,
                EXACT_INPUT_BOUND_QUERY_COUNT,
            ) => observed_input_tree_count += 1,
            (
                BoundTreeRootUse::Output,
                RowCodeWhirBoundLowDegreeMode::Direct,
                EXACT_OUTPUT_BOUND_QUERY_COUNT,
            ) => observed_output_tree_count += 1,
            _ => return Err("exact bound tree changes the verified/direct asymmetry".to_owned()),
        }
        for column in &tree.ordered_columns {
            if !observed_columns.insert(column.column_ordinal) {
                return Err("exact bound construction duplicates a polynomial".to_owned());
            }
        }
    }
    let reduction_tree_ordinals = plan
        .bound_reduction_blocks
        .iter()
        .flat_map(|block| block.ordered_bound_tree_ordinals.iter().copied())
        .collect::<BTreeSet<_>>();
    if observed_columns != expected_columns
        || observed_input_tree_count != expected_input_tree_count
        || observed_output_tree_count != expected_output_tree_count
        || reduction_tree_ordinals != (0_u32..EXACT_BOUND_TREE_COUNT as u32).collect()
    {
        return Err("exact bound codeword extraction is incomplete".to_owned());
    }
    Ok((
        observed_input_tree_count,
        observed_output_tree_count,
...
fn checked_opening_batch_mapping(
    ...
    if change_first_column {
        let first = observed
            ...
            .ok_or_else(|| "exact opening catalog is empty".to_owned())?;
        *first = 1;
    }
    let expected = requested_columns_by_point(shape)
        .into_iter()
        .map(|columns| {
            columns
                .into_iter()
                .map(|column| {
                    u32::try_from(column)
                        .map_err(|_| "exact aggregate column exceeds u32".to_owned())
                })
    ...
    let scalar_opening_count = observed.iter().try_fold(0_usize, |count, columns| {
        count
            ...
            .ok_or_else(|| "exact scalar opening count overflowed".to_owned())
    })?;
    let opening_point_aggregate_columns = observed
        .get(..3)
        .ok_or_else(|| "exact explicit opening batches are absent".to_owned())?
        .iter()
        .map(|columns| {
            columns
                .first()
                .copied()
                .filter(|_| columns.len() == 1)
                .ok_or_else(|| "exact explicit opening batch has the wrong width".to_owned())
        })
        .collect::<Result<Vec<_>, _>>()?;
    Ok((
        observed.len(),
...
fn checked_polynomial_basis_identities(change_first_identity: bool) -> Result<usize, String> {
    ...
    for (point_ordinal, evaluation_point) in points.into_iter().enumerate() {
        ...
        for coefficient_ordinal in 0..LOGICAL_POLYNOMIAL_COEFFICIENT_COUNT {
            ...
            if observed_power != expected_power {
                return Err("polynomial-to-multilinear basis identity changed".to_owned());
            }
            expected_power *= evaluation_point;
            identity_count = identity_count
                .checked_add(1)
                .ok_or_else(|| "polynomial basis identity count overflowed".to_owned())?;
        }
    }
    Ok(identity_count)
}
...
fn checked_selector_equality_identities() -> Result<usize, String> {
    let selector_cases = [
        core::array::from_fn(|selector_ordinal| {
            ChallengeField::from_u64(2 + 3 * selector_ordinal as u64)
        }),
        core::array::from_fn(|selector_ordinal| {
            ChallengeField::from_u64(7 + 6 * selector_ordinal as u64)
        }),
        core::array::from_fn(|selector_ordinal| {
            if selector_ordinal == 0 {
                ...
                ChallengeField::from_u64(31 + 6 * selector_ordinal as u64)
            }
        }),
    ];
    let mut identity_count = 0_usize;
    for selectors in selector_cases {
        let production_weights = selector_equality_weights(selectors);
        for (selector_index, production_weight) in production_weights.into_iter().enumerate() {
            let independent_weight =
                independent_multilinear_equality_weight(&selectors, selector_index);
            if production_weight != independent_weight {
                return Err("selector equality-weight ordering changed".to_owned());
            }
            identity_count += 1;
        }
    }
    ...
    for coefficient_count in [1_usize, 2, 17, 257] {
        ...
        for opening_point in opening_points {
            ...
            for claimed_value in [actual_evaluation, actual_evaluation + ChallengeField::ONE] {
                ...
                {
                    return Err("synthetic division changed its remainder identity".to_owned());
                }
                for coefficient_ordinal in 0..coefficients.len() {
                    ...
                    if reconstructed != expected {
                        return Err("synthetic division changed its polynomial identity".to_owned());
                    }
                }
                identity_count = identity_count
                    .checked_add(1)
                    .ok_or_else(|| "synthetic-division identity count overflowed".to_owned())?;
            }
        }
    }
    Ok(identity_count)
    ...
    for claim in variant.ordered_opening_claims() {
        match claim.source_class() {
            RelationOpeningSourceClass::TreeColumn => {
                let column = claim
                    .column_ordinal()
                    .ok_or_else(|| "tree opening claim has no column".to_owned())?;
                if proof_created_columns.contains(&column) {
                    proof_created_claim_count += 1;
                } else if bound_columns.contains(&column) {
                    bound_claim_count += 1;
                } else {
                    return Err("tree opening claim has no authenticated owner".to_owned());
                }
            }
...
pub(in crate::bgv::proof_suite) fn checked_exact_same_secret_extractor_correspondence_with_fault(
    ...
    expected_relation_plan_hash: [u8; 64],
    expected_relation_plan_variant_hash: [u8; 64],
    fault: ExactExtractorCorrespondenceFault,
) -> Result<
    (
        ExactPolynomialProtocolExtractorCertificate,
        ExactPointConstraintExtractorCertificate,
    ),
    String,
> {
    ...
    let construction_plan_identity_hash = construction_plan
        ...
        .map_err(|error| format!("derive exact construction identity: {error:?}"))?;
    let relation_plan_variant_hash = relation_variant
        .canonical_hash()
        .map_err(|error| format!("derive exact relation identity: {error:?}"))?;
    let source_manifest = SameSecretAuthenticatedSourceManifest::derive(
        construction_plan,
        relation_variant,
        relation_context,
    )
    .map_err(|error| format!("derive exact source manifest: {error:?}"))?;
    if source_manifest.construction_identity() != construction_plan_identity_hash {
        return Err("exact source manifest changes the construction identity".to_owned());
    }

    let base_phase = construction_plan
        .base_phase
        .as_ref()
        .ok_or_else(|| "exact base phase is absent".to_owned())?;
    let auxiliary_phase = construction_plan
        .auxiliary_phase
        .as_ref()
        .ok_or_else(|| "exact auxiliary phase is absent".to_owned())?;
    let base_polynomial_count = checked_trace_phase_polynomial_mapping(
        base_phase,
        relation_variant,
        ProofTreeRole::BaseOracle,
        fault == ExactExtractorCorrespondenceFault::DropFirstRelationPhasePolynomial,
    )?;
    let auxiliary_polynomial_count = checked_trace_phase_polynomial_mapping(
        auxiliary_phase,
        relation_variant,
        ProofTreeRole::AuxiliaryOracle,
        false,
    ...
    let (quotient_component_chunk_coordinate_count, opening_batch_mask_chunk_coordinate_count) =
        checked_quotient_phase_polynomial_mapping(construction_plan)?;
    let (prior_vss_bound_codeword_count, direct_bound_codeword_count, bound_polynomial_count) =
    ...
    let (opening_batch_count, mut scalar_opening_count, opening_point_aggregate_columns) =
        checked_opening_batch_mapping(
            construction_plan,
            shape,
            fault == ExactExtractorCorrespondenceFault::ChangeFirstAggregateOpeningColumn,
        )?;
    if fault == ExactExtractorCorrespondenceFault::ChangeScalarOpeningCount {
        scalar_opening_count = scalar_opening_count
            .checked_add(1)
            .ok_or_else(|| "faulted scalar opening count overflowed".to_owned())?;
    }
    ...
    let whir_fold_transition_count = whir_epoch_codeword_count
        .checked_mul(construction_plan.parameters.folding_factor)
        .ok_or_else(|| "exact WHIR fold count overflowed".to_owned())?;
    let whir_code_state_count = whir_epoch_codeword_count
        .checked_mul(construction_plan.parameters.folding_factor + 1)
        .ok_or_else(|| "exact WHIR code-state count overflowed".to_owned())?;
    let polynomial_basis_identity_count = checked_polynomial_basis_identities(
        fault == ExactExtractorCorrespondenceFault::ChangeFirstPolynomialBasisIdentity,
    )?;
    ...
    let proof_supplied_point_coordinate_count =
        usize::from(fault == ExactExtractorCorrespondenceFault::PermitProofSuppliedPoint);

    let polynomial_protocol = ExactPolynomialProtocolExtractorCertificate {
        ...
        authenticated_source_polynomial_count: source_manifest
            ...
            .map_err(|error| format!("count exact authenticated sources: {error:?}"))?,
        raw_authenticated_source_coefficient_position_count: source_manifest
            .raw_authenticated_source_coefficient_position_count()
            .map_err(|error| format!("count exact raw source positions: {error:?}"))?,
        persisted_pre_challenge_source_coefficient_position_count: source_manifest
            ...
            .map_err(|error| format!("count exact persisted source positions: {error:?}"))?,
        deterministic_reversed_column_count: source_manifest
            .deterministic_reversed_column_count()
            .map_err(|error| format!("count exact reversed columns: {error:?}"))?,
        stored_pre_challenge_column_count: source_manifest
            .stored_pre_challenge_column_count()
            .map_err(|error| format!("count exact stored columns: {error:?}"))?,
        relation_phase_row_counts: [
            base_phase.rows.len(),
            auxiliary_phase.rows.len(),
            construction_plan.quotient_phase.rows.len(),
        ],
        relation_phase_polynomial_counts: [
            base_polynomial_count,
            auxiliary_polynomial_count,
            quotient_component_chunk_coordinate_count + opening_batch_mask_chunk_coordinate_count,
        ],
        quotient_component_chunk_coordinate_count,
        opening_batch_mask_chunk_coordinate_count,
        prior_vss_bound_codeword_count,
        direct_bound_codeword_count,
    ...
    let point_constraints = ExactPointConstraintExtractorCertificate {
        construction_plan_identity_hash,
        relation_plan_variant_hash,
        explicit_point_count: relation_variant.ordered_opening_points().len(),
        explicit_point_variable_count: EXACT_TABLE_VARIABLE_COUNT,
        fixed_aggregate_selector_coordinate_count: 1,
        row_selector_coordinate_count: EXACT_ROW_SELECTOR_VARIABLE_COUNT,
        coefficient_reduction_coordinate_count: EXACT_BOUND_POLYNOMIAL_VARIABLE_COUNT,
    ...
    if !polynomial_protocol.is_complete() || !point_constraints.is_complete() {
        return Err("exact polynomial extractor correspondence is incomplete".to_owned());
    }
...
> {
    checked_exact_same_secret_extractor_correspondence_with_fault(
        ...
        expected_relation_plan_hash,
        expected_relation_plan_variant_hash,
        ExactExtractorCorrespondenceFault::None,
    )
}
...
fn requested_columns_by_point(shape: ExactProofShape) -> Vec<Vec<usize>> {
    ...
    requested.extend(
        std::iter::repeat_with(|| vec![EXACT_BOUND_REDUCTION_COLUMN_INDEX]).take(
            (shape.prior_proof_bound_query_count + shape.direct_bound_query_count) * 2
