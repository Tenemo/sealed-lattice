...
impl CompactCfwExternalStorageCatalog {
    ...
    pub(crate) const fn total_transaction_count(&self) -> u64 {
        self.plan.maximum_transaction_count()
    }
    ...
    pub(crate) const fn total_read_byte_length(&self) -> u64 {
        if self.read_extension_element_count == 0 {
            0
        } else {
            self.plan.maximum_total_read_byte_length()
        }
    }

    pub(crate) const fn peak_stored_byte_length(&self) -> u64 {
        self.plan.maximum_stored_byte_length()
    }
...
mod tests {
    ...
    #[test]
    fn selected_cfw_external_plan_matches_the_runtime_geometry() {
