...
impl VerifiedCommonProofApplicationSourceAuthority {
    ...
    pub(super) fn from_verified_accepted_setup_package(
    ...
    #[cfg(test)]
    pub(super) fn from_test_verified_public_key_share_source(
        verified_public_randomness: &VerifiedPublicRandomness,
        proof_stream_descriptor: StreamDescriptor,
        roster_position: u16,
    ) -> Result<(Self, u16), CommonProofRuntimeError> {
        let verified_context = verified_public_randomness.context();
        if verified_public_randomness
        ...
        Ok((
            Self {
                suite_identifier: verified_context.suite_identifier(),
                ceremony_context_hash: verified_context.ceremony_context_hash(),
                action_context_hash: verified_context.action_context_hash(),
                application_source_object_hash: verified_public_randomness
    ...
    pub(super) fn matches_proof_application_binding(
        &self,
        proof_application_binding: &ProofApplicationBinding,
    ) -> bool {
        let application_slot = proof_application_binding.application_slot();
