    ...
    ColumnEvaluator: VerifiedRelationColumnEvaluator + ?Sized,
{
    ...
    let decode_result = pending.decode_query_section(
        ...
        |opening| {
            if let Err(error) = workspace.consume_opening(opening) {
...
> {
    ...
    evaluation_domain: ProofEvaluationDomain,
    query_representatives: &'queries [u64],
...
impl<'relation, 'queries, 'evaluator, ColumnEvaluator>
    QueryVerificationWorkspace<'relation, 'queries, 'evaluator, ColumnEvaluator>
where
    ColumnEvaluator: VerifiedRelationColumnEvaluator + ?Sized,
{
    fn new(
    ...
    fn consume_fri_tree(
        ...
        let leaf_count = u64::try_from(self.evaluation_domain.size())
            ...
            .ok_or(CommonProofVerifierError::InvalidTreeLayout)?;
        for (state, representative) in states
            .iter_mut()
    ...
    fn ensure_fri_states(&mut self) -> Result<(), CommonProofVerifierError> {
    ...
    fn finish(&mut self) -> Result<(), CommonProofVerifierError> {
