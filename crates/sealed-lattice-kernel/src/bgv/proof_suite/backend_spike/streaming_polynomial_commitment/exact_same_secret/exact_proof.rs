...
use crate::bgv::proof_suite::{
    ...
    ValidatedRelationPlanArtifact, VerifiedKeyRelationColumnEvaluator,
...
pub(crate) struct ExactSameSecretPublicInput {
    ...
    public_relation_trees: Vec<StatementOwnedProofTreeInput>,
}
...
fn validate_public_input(
    ...
    if public_input.statement_schema_identifier
        ...
        || public_input.public_setup_seed == [0_u8; 64]
    {
    ...
    bound_column_ordinals.sort_unstable();
    ...
    for column_ordinal in bound_column_ordinals {
    ...
    let validated_plan =
        ValidatedRelationPlanArtifact::from_compiled_plan(&compiled_plan, &relation_context)
            .map_err(|error| format!("revalidate exact relation plan: {error:?}"))?;
    let verifier_sequence_evaluator =
        VerifiedKeyRelationColumnEvaluator::from_public_setup_seed_for_backend(
            public_input.public_setup_seed,
            &validated_plan,
            &variant,
            &relation_context,
        )
...
fn exact_bound_tree_catalog_entries(
    ...
    let schedule = verified_relation
        .variant
        .common_proof_transcript_schedule(&verified_relation.context)
        .map_err(|error| format!("derive exact tree-catalog schedule: {error:?}"))?;
    let canonical_header = exact_transcript_header(
        public_input.protocol_version,
        public_input.suite_identifier,
    ...
    let catalog = build_complete_proof_tree_catalog(
        ProofTreeCatalogInput {
            suite_identifier: public_input.suite_identifier,
            canonical_proof_object_header_bytes: canonical_header,
            application_statement_schema_identifier: public_input.statement_schema_identifier,
            proof_field_index: 0,
            evaluation_domain_size: verified_relation.variant.evaluation_domain_size(),
            relation_trees: verified_relation.relation_trees.clone(),
        },
        &schedule,
    )
    .map_err(|error| format!("build exact bound tree catalog: {error:?}"))?;
    if catalog.evaluation_domain_size()
        != u64::try_from(EXACT_BOUND_LEAF_COUNT * 2)
            .map_err(|_| "bound evaluation domain exceeds u64".to_owned())?
    {
        return Err("exact bound tree catalog has the wrong evaluation domain".to_owned());
    }
    let entries = catalog
        .entries()
        .iter()
...
struct ExactTranscriptPrefix {
    ...
    deep_points: Vec<ProofChallengeExtensionElement>,
...
fn exact_transcript_prefix(
    ...
    for component_ordinal in 0..verified_relation.context.quotient_component_count {
        transcript
            ...
            .map_err(|error| format!("absorb exact quotient root: {error:?}"))?;
    }
    let mut deep_points =
        Vec::with_capacity(usize::from(verified_relation.context.deep_point_count));
    for point_ordinal in 0..verified_relation.context.deep_point_count {
        let mut relation_error = None;
        let deep_point = transcript
            .sample_deep_point(point_ordinal, |candidate| {
                match verified_relation.variant.deep_point_candidate_is_forbidden(
            ...
            .map_err(|error| format!("sample exact deep point: {error:?}"))?;
        if let Some(error) = relation_error {
            return Err(format!("validate exact deep point: {error:?}"));
        }
        deep_points.push(deep_point);
    }
    let opening_points = verified_relation
        .variant
        .derive_opening_points(&verified_relation.context, &deep_points)
...
fn finish_exact_transcript(
    ...
    prefix
        ...
        .map_err(|error| format!("absorb exact deep evaluations: {error:?}"))?;
    prefix
        .transcript
    ...
    let mut opening_batch_challenges = Vec::with_capacity(deep_evaluations.len());
    for claim_ordinal in 0..deep_evaluations.len() {
        opening_batch_challenges.push(
            prefix
                .transcript
    ...
    Ok(opening_batch_challenges)
}

fn verify_production_deep_composition(
    ...
    verified_relation
        ...
        .map_err(|error| format!("verify exact production deep composition: {error:?}"))
}
...
fn aggregate_binding(
    ...
    binding.extend_from_slice(EXACT_AGGREGATE_BINDING_DOMAIN);
    binding.extend_from_slice(&public_input.digest()?);
    for root in proof_roots {
        binding.extend_from_slice(&column_digest_bytes(root));
    }
    for point in &transcript_prefix.deep_points {
        for coordinate in point.canonical_coordinates() {
            binding.extend_from_slice(&coordinate.to_le_bytes());
        }
    }
    for point in &transcript_prefix.opening_points {
        for coordinate in point.canonical_coordinates() {
            binding.extend_from_slice(&coordinate.to_le_bytes());
        }
    }
    ...
    for challenge in opening_batch_challenges {
...
fn derive_bound_opening_claims(
    ...
    if variant.ordered_opening_claims().len() != deep_evaluations.len() {
        return Err("bound opening reduction has the wrong claim count".to_owned());
    }
    let mut state = Shake256::default();
    state.update(&(EXACT_BOUND_BATCHING_CHALLENGE_DOMAIN.len() as u64).to_le_bytes());
    state.update(EXACT_BOUND_BATCHING_CHALLENGE_DOMAIN);
...
fn derive_distinct_indices(
    ...
    if upper_bound < count || !upper_bound.is_power_of_two() {
        return Err("exact query domain has the wrong fixed shape".to_owned());
    }
    let aggregate_root = aggregate_commitment
        .roots()
        .first()
        .copied()
...
fn derive_bound_query_indices(
    ...
    derive_distinct_indices(
        EXACT_BOUND_QUERY_DOMAIN,
        binding,
...
fn bound_degree_test_points(
    ...
    let aggregate_root = aggregate_commitment
        ...
        .ok_or_else(|| "exact aggregate commitment has no root".to_owned())?;
    let mut state = Shake256::default();
    state.update(&(EXACT_BOUND_DEGREE_POINT_DOMAIN.len() as u64).to_le_bytes());
    state.update(EXACT_BOUND_DEGREE_POINT_DOMAIN);
    state.update(&(binding.len() as u64).to_le_bytes());
    state.update(binding);
    state.update(&column_digest_bytes(aggregate_root));
    let mut reader = state.finalize_xof();
...
fn derive_exact_point_row_weights(
    ...
    let mut state = Shake256::default();
    state.update(&(EXACT_RANK_ONE_CHALLENGE_DOMAIN.len() as u64).to_le_bytes());
    state.update(EXACT_RANK_ONE_CHALLENGE_DOMAIN);
    state.update(&(binding.len() as u64).to_le_bytes());
    state.update(binding);
    let mut reader = state.finalize_xof();
...
fn deep_evaluation_catalog(
    ...
    if deep_evaluations.len() != variant.ordered_opening_claims().len() {
        return Err("deep evaluation count does not match the relation".to_owned());
    }
...
fn exact_whir_opening_points(
    ...
    for opening_point_ordinal in 0..3 {
        ...
        if coordinates.len() != EXACT_TABLE_VARIABLE_COUNT {
            return Err("exact deep WHIR point has the wrong variable count".to_owned());
        }
        points.push(Point::new(coordinates));
    }
...
fn expected_deep_whir_evaluations(
    variant: &RelationPlanVariant,
    base_layout: &ExactBasePhaseLayout,
    auxiliary_layout: &ExactBasePhaseLayout,
    opening_points: &[ProofChallengeExtensionElement],
    ...
    for opening_point_ordinal in 0..3 {
        ...
        let mut polynomial_value = expected_base_phase_deep_value(
            base_layout,
        ...
        polynomial_value += expected_base_phase_deep_value(
            auxiliary_layout,
        ...
        if opening_point_ordinal == 0 {
            polynomial_value += expected_quotient_phase_deep_value(
...
fn verify_whir_evaluation_claims(
    proof: &ExactSameSecretProof,
    expected_deep: [ChallengeField; 3],
    ...
    for (opening_ordinal, expected) in expected_deep.iter().enumerate() {
        let batch = &proof.aggregate_opening_proof.evals[opening_ordinal];
        if batch.current() != [*expected] || !batch.next().is_empty() {
            return Err(format!(
                "exact WHIR deep opening {opening_ordinal} does not authenticate the production claims"
            ));
        }
    }
...
pub(crate) fn verify_exact_same_secret_proof_bytes(
    ...
    let mut verified_relation = validate_public_input(&public_input)?;
    let bound_tree_entries = exact_bound_tree_catalog_entries(&public_input, &verified_relation)?;
    ...
    let bound_query_indices = derive_bound_query_indices(&binding, &proof.aggregate_commitment)?;
    let degree_test_points = bound_degree_test_points(&binding, &proof.aggregate_commitment)?;
    ...
    let expected_deep = expected_deep_whir_evaluations(
        &verified_relation.variant,
        &base_layout,
        &auxiliary_layout,
        &transcript_prefix.opening_points,
    ...
    let mut challenger = plain_aggregate_challenger(&pcs, &binding);
...
pub(crate) fn encode_exact_same_secret_public_input(
    ...
    writer.write_bytes(&public_input.public_setup_seed);
...
pub(crate) fn decode_exact_same_secret_public_input(
    ...
    let public_setup_seed = reader.read_exact::<64>()?;
...
fn encode_exact_same_secret_proof(
    ...
    writer.write_u32(checked_u32(
        proof.deep_evaluations.len(),
        "deep evaluation count",
    )?);
...
fn decode_exact_same_secret_proof(
    ...
    let deep_evaluation_count = reader.read_u32()? as usize;
    if deep_evaluation_count != shape.opening_claim_count {
        return Err(format!(
            "exact same-secret proof has {deep_evaluation_count} deep evaluations, expected {}",
            shape.opening_claim_count
        ));
    }
    let mut deep_evaluations = Vec::with_capacity(deep_evaluation_count);
...
fn validate_exact_proof_shape(
    ...
    if proof.deep_evaluations.len() != shape.opening_claim_count
...
mod construction_tests {
    ...
    #[test]
    fn exact_construction_has_a_machine_checked_soundness_ledger() {
        let pcs = plain_aggregate_pcs(EXACT_PCS_VARIABLE_COUNT)
        ...
        for round in &pcs.round_parameters {
            query_branches.push((current_log_inverse_rate, round.num_queries));
        ...
        // BCS/BT24 classical-ROM compilation at Q <= 2^128 and kappa = 512.
        let random_oracle_query_bound =
            BigUint::from(1_u8) << CLASSICAL_RANDOM_ORACLE_QUERY_BOUND_EXPONENT;
        let conservative_round_by_round_numerator = BigUint::from(1_u8)
            << (FIAT_SHAMIR_XOF_OUTPUT_BIT_LENGTH - STRICT_ROUND_BY_ROUND_SOUNDNESS_BITS);
        let compiler_numerator = BigUint::from(3_u8)
        ...
        assert!(
            random_oracle_query_bound * conservative_round_by_round_numerator + compiler_numerator
                < target_numerator
        );
    }
    ...
    fn exact_masking_budget_covers_the_complete_first_oracle_view() {
        ...
        let point_row_weights = derive_exact_point_row_weights(
            b"exact masking view certificate",
            &base_layout,
            &auxiliary_layout,
...
mod native_prover {
    ...
    fn exact_public_input_from_production_source(
        sources: &ProductionBackendSameSecretSources,
    ) -> Result<ExactSameSecretPublicInput, String> {
        let request_context = sources
            .source_polynomials
            .production_backend_request_context();
        ...
        let _ = validate_public_input(&public_input)?;
        Ok(public_input)
    }
    ...
    fn construct_bound_reduction_polynomial(
        ...
        for claim in bound_claims {
            ...
            if remainder != ChallengeField::ZERO {
                return Err(format!(
                    "bound relation column {} does not match its claimed deep evaluation",
                    claim.column_ordinal
                ));
            }
        ...
        for (bound_tree_ordinal, entry) in entries.iter().enumerate() {
            ...
            let persistent_source = if entry.requires_persistent_leaf_salt() {
                Some(
                    sources
                        .source_polynomials
                        .production_backend_bound_material_source(
    ...
    fn generate_exact_same_secret_artifacts() -> Result<(Vec<u8>, Vec<u8>), String> {
        ...
        let bound_tree_entries =
            exact_bound_tree_catalog_entries(&public_input, &verified_relation)?;
        let shape = ExactProofShape::from_variant(&verified_relation.variant)?;
        ...
        let deep_evaluations = evaluate_exact_deep_claims(
            &verified_relation.variant,
            &transcript_prefix.opening_points,
            &store,
        )?;
        ...
        let degree_test_points = bound_degree_test_points(&binding, &aggregate_commitment)?;
        let bound_evaluation_domain = ProofEvaluationDomain::new(
            usize::try_from(verified_relation.variant.evaluation_domain_size())
                .map_err(|_| "bound evaluation domain exceeds usize".to_owned())?,
            verified_relation.context.evaluation_coset_offset,
...
mod persisted_verifier_tests {
    ...
    fn mutate_proof(
        ...
        let verified_relation =
            validate_public_input(&public_input).expect("validate exact public input for mutation");
        let bound_tree_entries =
            exact_bound_tree_catalog_entries(&public_input, &verified_relation)
                .expect("derive exact bound tree entries for mutation");
        let shape = ExactProofShape::from_variant(&verified_relation.variant)
            .expect("derive exact proof shape for mutation");
        let mut proof = decode_exact_same_secret_proof(shape, &bound_tree_entries, canonical_proof)
    ...
    fn assert_refuses(public_input: &[u8], proof: &[u8], mutation_label: &str) {
        if let Ok(metrics) = verify_exact_same_secret_proof_bytes(public_input, proof) {
            panic!(
    ...
    fn heavy_rust_kernel_exact_same_secret_adversarial_verifier() {
        ...
        let changed_setup_seed = mutate_public_input(&public_input, |input| {
            input.public_setup_seed[0] ^= 1;
        });
        assert_refuses(&changed_setup_seed, &proof, "changed public setup seed");
        ...
        let changed_deep_evaluation = mutate_proof(&public_input, &proof, |decoded| {
            decoded.deep_evaluations[0] =
                decoded.deep_evaluations[0].add(ProofChallengeExtensionElement::ONE);
        });
        assert_refuses(
            &public_input,
        ...
        swap_distinct_eight_byte_chunks(&mut swapped_extension_limbs, first_deep_evaluation_offset);
        assert_refuses(
            &public_input,
        ...
        *changed_whir_tail.last_mut().expect("proof is nonempty") ^= 1;
        ...
        let oversized_proof = vec![0_u8; MAXIMUM_STREAMING_WIRE_BYTE_LENGTH + 1];
        ...
        for retry_ordinal in 0..3 {
            verify_exact_same_secret_proof_bytes(&public_input, &proof).unwrap_or_else(|error| {
                panic!(
