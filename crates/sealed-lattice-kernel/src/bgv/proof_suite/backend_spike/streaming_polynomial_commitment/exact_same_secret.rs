//! Exact production-authenticated same-secret backend construction.
//!
//! This module is restricted to the backend research feature. It enters the
//! ordinary browser-owned setup authority, compiles the selected same-secret
//! relation, binds a persistent proof attempt to the canonical witness, and
//! consumes the production source adapter and private proof coins. Synthetic
//! columns are not accepted by this path.

use crate::{
    bgv::{
        proof_suite::{
            CommonProofRelationPlanCapability, CommonProofTranscript, ProofTreeRole,
            SelectedApplicationStatementContext, compile_same_secret_relation_plan,
...
use crate::{
    bgv::{
        ...
        setup::{
            ProductionBackendSameSecretSources, SetupGenerationKeyRelationApplication,
            populate_production_backend_prototype_authority,
            resolve_setup_generation_key_relation_preparation_source,
            with_setup_generation_key_relation,
        },
    },
    foundation::{
        Hash512, ProofApplicationSlot, SELECTED_MAXIMUM_PRIVATE_SAMPLER_CANDIDATE_DRAWS_PER_OUTPUT,
        prepare_production_backend_prototype_attempt,
    },
};

use std::collections::BTreeMap;

use super::protocol::MAXIMUM_STREAMING_WIRE_BYTE_LENGTH;
#[cfg(all(test, not(target_arch = "wasm32")))]
...
const SOURCE_POLYNOMIAL_DIGEST_DOMAIN: &str =
    "sealed-lattice/backend-research/production-source-polynomial/v1";
#[cfg(all(test, not(target_arch = "wasm32")))]
...
fn production_same_secret_sources() -> Result<ProductionBackendSameSecretSources, String> {
    ...
    let limits = CommonProofRuntimeLimits::new(
        ...
        MAXIMUM_STREAMING_WIRE_BYTE_LENGTH as u64,
    )
    ...
    let prepared_attempt = prepare_production_backend_prototype_attempt(
    ...
    with_setup_generation_key_relation(&authority.authority_handle, &application, |source| {
        source.prepare_production_backend_same_secret_sources(relation_plan, limits)
    })
...
mod tests {
    ...
    fn ensure_exact_verifier_sequence_columns(
        sources: &mut ProductionBackendSameSecretSources,
        ...
        for column_ordinal in 0..sources.relation_plan_variant.ordered_columns().len() {
            ...
            let polynomial = sources
                .source_polynomials
                .production_backend_verifier_sequence_polynomial(column_ordinal)
