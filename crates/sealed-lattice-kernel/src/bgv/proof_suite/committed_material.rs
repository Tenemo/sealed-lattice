...
use super::{
    PROOF_BASE_FIELD_MODULUS, PROOF_EVALUATION_BLOWUP_FACTOR,
    PROOF_EVALUATION_COSET_OFFSET, PROOF_MAXIMUM_FIAT_SHAMIR_CANDIDATE_DRAWS_PER_OUTPUT,
    CommonProofProverError, CommonProofSourcePolynomial, ProofBaseFieldElement,
...
const MERKLE_NODE_HASH_DOMAIN: &str =
...
impl CommittedMaterialProfile {
    pub(crate) fn selected(ring_degree: usize) -> Result<Self, CommittedMaterialError> {
        ...
        let maximum_consuming_numerator_degree = material_column_degree_bound_exclusive
            ...
            .ok_or(CommittedMaterialError::CountOverflow)?;
        let committed_polynomial_degree_bound_exclusive = maximum_consuming_numerator_degree
            .checked_add(1)
            .and_then(usize::checked_next_power_of_two)
            .ok_or(CommittedMaterialError::CountOverflow)?;
        let evaluation_domain_size = committed_polynomial_degree_bound_exclusive
            .checked_mul(
...
impl CommittedMaterialTree {
    pub(crate) fn construct(
        ...
        let mask_coefficient_count = input
            ...
            .checked_add(1)
            .ok_or(CommittedMaterialError::CountOverflow)?;
        let mut masked_coefficients_by_physical_column =
            Vec::with_capacity(MATERIAL_COLUMN_COUNT);
        let mut extension_columns = Vec::with_capacity(MATERIAL_COLUMN_COUNT);
        for (digit_column_ordinal, digit_column) in
            input.message_digit_columns.iter().enumerate()
        {
            for half_ordinal in 0..2 {
                ...
                if masked_coefficients.len()
                    > input.profile.material_column_degree_bound_exclusive
                {
                    return Err(CommittedMaterialError::InvalidProfile);
                }
                extension_columns.push(
                    evaluation_domain.evaluate_base_polynomial(&masked_coefficients)?,
                );
    ...
    pub(crate) fn authentication_frontier(
        ...
        let mut current = sorted_unique_leaf_indices.iter().copied().collect::<BTreeSet<_>>();
...
impl<'seed> MaterialPrivateDerivation<'seed> {
    ...
    fn next_block(&mut self) -> Result<Zeroizing<[u8; DERIVED_BLOCK_BYTE_LENGTH]>, CommittedMaterialError> {
    ...
    fn sample_uniform_base_field(&mut self) -> Result<u64, CommittedMaterialError> {
        ...
        for _ in 0..PROOF_MAXIMUM_FIAT_SHAMIR_CANDIDATE_DRAWS_PER_OUTPUT {
            ...
            candidate_bytes[..sample_byte_length]
                .copy_from_slice(&block[..sample_byte_length]);
...
fn build_merkle_levels(
    ...
    while levels.last().is_some_and(|level| level.len() > 1) {
        let child_level = levels
            .last()
            .ok_or(CommittedMaterialError::InvalidInput)?;
        let level = u32::try_from(levels.len())
            .map_err(|_| CommittedMaterialError::CountOverflow)?;
