...
const LIMB_COUNT_BYTE_LENGTH: usize = 2;
...
///
/// The canonical bytes start with the little-endian active-limb count, followed
/// by each limb's `N` little-endian residues in increasing data-prime order.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
impl<'bytes> CanonicalTargetPartialDecryptionStream<'bytes> {
    pub(crate) fn decode(
        ...
        let encoded_limb_count = u16::from_le_bytes(
            canonical_bytes[..LIMB_COUNT_BYTE_LENGTH]
                .try_into()
                .map_err(|_| TargetPartialDecryptionStreamError::InvalidByteLength)?,
        );
        if usize::from(encoded_limb_count) != selected_target_data_prime_count() {
            return Err(TargetPartialDecryptionStreamError::InvalidLimbCount);
        }

        let mut residue_bytes = canonical_bytes[LIMB_COUNT_BYTE_LENGTH..].chunks_exact(8);
        for modulus in DATA_PRIMES.iter().take(selected_target_data_prime_count()) {
        ...
        if !residue_bytes.remainder().is_empty() || residue_bytes.next().is_some() {
            return Err(TargetPartialDecryptionStreamError::InvalidByteLength);
        }
...
pub(crate) fn encode_target_partial_decryption_stream(
    ...
    canonical_bytes.extend_from_slice(
        &u16::try_from(ordered_limbs.len())
            .map_err(|_| TargetPartialDecryptionStreamError::CountOverflow)?
            .to_le_bytes(),
    );
    for (limb, modulus) in ordered_limbs.iter().zip(DATA_PRIMES.iter()) {
...
-> Result<usize, TargetPartialDecryptionStreamError> {
    selected_target_data_prime_count()
        .checked_mul(POLYNOMIAL_DEGREE)
        .and_then(|count| count.checked_mul(CANONICAL_RESIDUE_BYTE_LENGTH))
        .and_then(|byte_length| byte_length.checked_add(LIMB_COUNT_BYTE_LENGTH))
...
mod tests {
    ...
    #[test]
    fn selected_target_partial_stream_round_trips_canonical_residues() {
        let limbs = selected_limbs();
        let bytes = encode_target_partial_decryption_stream(&limbs).expect("stream encodes");
        assert_eq!(
            bytes.len(),
    ...
    #[test]
    fn selected_target_partial_stream_rejects_truncation_and_extension() {
        let bytes = encode_target_partial_decryption_stream(&selected_limbs()).expect("stream");
    ...
    #[test]
    fn selected_target_partial_stream_rejects_wrong_limb_count() {
        let mut bytes =
            encode_target_partial_decryption_stream(&selected_limbs()).expect("stream encodes");
        ...
        assert_eq!(
            encode_target_partial_decryption_stream(&selected_limbs()[..1]),
            Err(TargetPartialDecryptionStreamError::InvalidLimbCount)
        );
    }

    #[test]
    fn selected_target_partial_stream_rejects_noncanonical_residue() {
        let mut bytes =
            encode_target_partial_decryption_stream(&selected_limbs()).expect("stream encodes");
        bytes[2..10].copy_from_slice(&DATA_PRIMES[0].to_le_bytes());
        assert_eq!(
            CanonicalTargetPartialDecryptionStream::decode(&bytes),
        ...
        limbs[1][POLYNOMIAL_DEGREE - 1] = DATA_PRIMES[1];
        assert_eq!(
            encode_target_partial_decryption_stream(&limbs),
            Err(TargetPartialDecryptionStreamError::NoncanonicalResidue)
        );
    }

    #[test]
    fn selected_target_partial_stream_rejects_wrong_coefficient_count() {
        let mut limbs = selected_limbs();
        limbs[0].pop();
        assert_eq!(
            encode_target_partial_decryption_stream(&limbs),
