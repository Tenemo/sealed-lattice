...
pub(in crate::bgv) struct ChallengeExtensionTower {
    pub(in crate::bgv) modulus: u64,
    // s^2 in the base field.
    pub(in crate::bgv) quadratic_non_residue: u64,
    // t^2 as an F_{p^2} element (constant term, s coefficient).
    pub(in crate::bgv) quartic_seed: [u64; 2],
}

// One F_{p^4} element as coefficients of {1, s, t, s*t}.
pub(in crate::bgv) type ChallengeExtensionElement = [u64; 4];

pub(in crate::bgv) const CHALLENGE_EXTENSION_DEGREE: usize = 4;
...
impl ChallengeExtensionTower {
    pub(in crate::bgv) fn for_modulus(modulus: u64) -> CanonicalResult<Self> {
    ...
    pub(in crate::bgv) fn zero() -> ChallengeExtensionElement {
        [0; 4]
    }

    pub(in crate::bgv) fn one() -> ChallengeExtensionElement {
        [1, 0, 0, 0]
    }

    pub(in crate::bgv) fn embed_base(&self, value: u64) -> ChallengeExtensionElement {
        [value % self.modulus, 0, 0, 0]
    }

    pub(in crate::bgv) fn is_zero(element: &ChallengeExtensionElement) -> bool {
        element.iter().all(|coefficient| *coefficient == 0)
    }

    pub(in crate::bgv) fn add(
        &self,
    ...
    pub(in crate::bgv) fn scale_base(
        &self,
