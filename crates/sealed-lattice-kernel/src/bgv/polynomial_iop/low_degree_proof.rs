...
pub(in crate::bgv) struct LowDegreeParameters {
    pub(in crate::bgv) modulus: u64,
    pub(in crate::bgv) initial_domain_size: usize,
    pub(in crate::bgv) initial_offset: u64,
    pub(in crate::bgv) initial_root: u64,
    ...
    pub(in crate::bgv) initial_degree_bound: usize,
}

pub(in crate::bgv) struct LowDegreeProof {
    pub(in crate::bgv) folded_layer_roots: Vec<[u8; 64]>,
    pub(in crate::bgv) final_coefficients: Vec<ChallengeExtensionElement>,
    pub(in crate::bgv) query_openings: Vec<LowDegreeQueryOpening>,
    // One batched authentication opening per committed folded layer, covering
    // every query's pair leaf in that layer at once.
    pub(in crate::bgv) layer_batch_openings: Vec<BatchedMerkleOpening>,
}

pub(in crate::bgv) struct LowDegreeQueryOpening {
    ...
    pub(in crate::bgv) folded_layer_pairs: Vec<LowDegreePairOpening>,
}

pub(in crate::bgv) struct LowDegreePairOpening {
    pub(in crate::bgv) pair: [ChallengeExtensionElement; 2],
}
...
pub(in crate::bgv) fn prove_low_degree(
...
pub(in crate::bgv) fn verify_low_degree(
    transcript: &mut FiatShamirTranscript,
    parameters: &LowDegreeParameters,
