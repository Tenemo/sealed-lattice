...
pub(super) fn verify_trustee_evaluation_key_proofs(
    ...
    if super::setup_package_uses_compact_vss_setup_path(setup_package)
        && verified_compact_same_secret_bridge.is_none()
    {
        return Ok(Some(evaluation_key_material_refusal(
            "trusteeEvaluationKeyCompactAnchorRebindingRequired",
            "compact setup packages require trustee evaluation-key proof records that bind compact same-secret evidence instead of full VSS constant commitments",
            "setupPackage.trusteeEvaluationKeyProofs",
        )?));
    }
...
pub(in crate::bgv::setup) fn trustee_evaluation_key_statement_from_package(
        ...
        inputs.verified_compact_same_secret_bridge
    {
        ...
        // key-switch limb (the working-level RNS limbs the relinearization
        // and Galois keys operate over), not every Q_share limb.
...
fn verify_trustee_evaluation_key_proof_transport_reference(
        ...
        || value_string(proof_record, "proofChunkRoot")? != transport_hashes.chunk_root
    {
        return Err(CanonicalError::new(
            CanonicalErrorCode::InvalidFixture,
            "trustee evaluation-key proof transport reference does not match transported chunks",
        ));
    }
    let chunk_hash_values = proof_record
        ...
        .ok_or_else(|| {
            CanonicalError::new(
                CanonicalErrorCode::InvalidFixture,
                "trustee evaluation-key proofChunkHashes must list every transported proof chunk",
            )
        })?;
    if chunk_hash_values.len() != transport_hashes.chunk_hashes.len() {
        return Err(CanonicalError::new(
            CanonicalErrorCode::InvalidFixture,
            "trustee evaluation-key proofChunkHashes length must match transported proof chunks",
        ));
    }
        ...
        .zip(transport_hashes.chunk_hashes.iter())
    {
        if chunk_hash_value.as_str() != Some(expected_chunk_hash.as_str()) {
            return Err(CanonicalError::new(
                CanonicalErrorCode::InvalidFixture,
                "trustee evaluation-key proofChunkHashes must match transported proof chunks",
            ));
        }
    }
...
mod tests {
    ...
    fn compact_setup_package_with_trustee_evaluation_key_proofs() -> Value {
        json!({
    ...
    #[test]
    fn compact_setup_refuses_old_trustee_evaluation_key_anchor_dependency() -> CanonicalResult<()> {
        let package = compact_setup_package_with_trustee_evaluation_key_proofs();

        let response = verify_trustee_evaluation_key_proofs(&package, &json!({}), None)?
            .expect("compact setup old evaluation-key proof anchor must refuse");

        assert_eq!(response["verifierStatus"], json!("refused"));
        assert_eq!(
            response["refusedObjects"][0]["reasonCode"],
            json!("trusteeEvaluationKeyCompactAnchorRebindingRequired")
        );
        Ok(())
    }
}
