...
pub(crate) fn vss_public_commitment_body_root(record: &Value) -> CanonicalResult<String> {
    let commitment = validate_standalone_vss_committed_material_commitment(
        value_at_path(record, &["commitment"])?,
        "VSS public commitment",
    )?;
    derive_canonical_object_hash(&commitment)
}

fn canonical_vss_public_source_coefficient_record(source_record: &Value) -> CanonicalResult<Value> {
    let source_trustee_identity = read_non_empty_string(source_record, "sourceTrusteeIdentity")?;
...
pub(crate) fn vss_public_coefficient_commitment_set_root(
    ...
    let source_trustee_records = array_at_path(coefficient_set, &["sourceTrusteeRecords"])?
        .iter()
        .map(canonical_vss_public_source_coefficient_record)
        .collect::<CanonicalResult<Vec<_>>>()?;
    derive_canonical_object_hash(&json!({
...
pub(crate) fn vss_public_source_recipient_share_record_root(
    ...
    let source_trustee_identity = read_non_empty_string(source_record, "sourceTrusteeIdentity")?;
    let recipient_share_commitments = array_at_path(source_record, &["recipientShareCommitments"])?
        .iter()
        .map(|record| {
            Ok(json!({
                "objectType": "VssPublicRecipientShareCommitment",
                "recipientIdentity": read_non_empty_string(record, "recipientIdentity")?,
                "commitment": validate_standalone_vss_committed_material_commitment(
                    value_at_path(record, &["commitment"])?,
