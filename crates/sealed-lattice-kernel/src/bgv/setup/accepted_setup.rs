...
fn verify_optional_same_secret_internal_proofs(
        ...
        != Some(same_secret_consistency_root.as_str())
    {
        return Ok(Some(same_secret_proof_refusal(
            ...
            "sameSecretProofs.sameSecretConsistencyRoot must match accepted same-secret statements",
            "setupPackage.sameSecretProofs.sameSecretConsistencyRoot",
        )?));
    }
...
fn verify_public_key_share_proof_record(
    ...
    for (field_name, expected_value) in [
        ...
        (
            "proofBytesStatus",
            "not-accepted-until-setup-proof-verifier",
        ),
    ] {
        if proof_record.get(field_name).and_then(Value::as_str) != Some(expected_value) {
            return Ok(Some(public_key_share_proof_refusal(
...
fn verify_pending_evaluation_key_material_boundary(
    ...
    let relinearization_rounds = setup_package
        .get("relinearizationKeyShareRounds")
        .ok_or_else(|| {
            CanonicalError::new(
                CanonicalErrorCode::InvalidFixture,
                "relinearizationKeyShareRounds was required before evaluation-key material boundary verification",
            )
        })?;
    let Some(relinearization_rounds) = relinearization_rounds.as_object() else {
        return Ok(Some(evaluation_key_material_refusal(
            "relinearizationKeyShareRoundsNotObject",
            "relinearizationKeyShareRounds must be an object while proof verification is pending",
            "setupPackage.relinearizationKeyShareRounds",
        )?));
    };
    if !relinearization_rounds.is_empty() {
        return Ok(Some(evaluation_key_material_refusal(
            "relinearizationProofMaterialBeforeVerifier",
            "relinearization proof material is refused until the frozen evaluator schedule has a proof verifier",
            "setupPackage.relinearizationKeyShareRounds",
        )?));
    }

    let galois_key_batches = setup_package
        .get("galoisKeyShareBatches")
        .ok_or_else(|| {
            CanonicalError::new(
                CanonicalErrorCode::InvalidFixture,
                "galoisKeyShareBatches was required before evaluation-key material boundary verification",
            )
        })?;
    let Some(galois_key_batches) = galois_key_batches.as_array() else {
        return Ok(Some(evaluation_key_material_refusal(
            "galoisKeyShareBatchesNotArray",
            "galoisKeyShareBatches must be an array while proof verification is pending",
            "setupPackage.galoisKeyShareBatches",
        )?));
    };
    if !galois_key_batches.is_empty() {
        return Ok(Some(evaluation_key_material_refusal(
            "galoisProofMaterialBeforeVerifier",
            "Galois key proof material is refused until the frozen evaluator schedule has a proof verifier",
            "setupPackage.galoisKeyShareBatches",
        )?));
    }
    ...
    if !evaluation_keys.is_empty() {
        return Ok(Some(evaluation_key_material_refusal(
            ...
            "evaluation keys are refused until relinearization and Galois proof verifiers pass against the frozen evaluator schedule",
            "setupPackage.evaluationKeys",
        )?));
    }
...
fn accepted_hashes_from_package(setup_package: &Value) -> Vec<String> {
