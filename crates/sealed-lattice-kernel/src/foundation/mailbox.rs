...
pub(crate) fn derive_mailbox_output_slot_hash(
    binding: &MailboxBindingExpectation,
) -> MailboxResult<Hash512> {
    let ordered_material_roots = binding
        .ordered_material_roots
        .iter()
        .map(|root| CanonicalItem::hash512(root.into_bytes()))
        .collect::<Vec<_>>();
    Ok(hash512(
        "sealed-lattice/mailbox/output-slot/v1",
        &[
            ...
            CanonicalItem::participant_identity(binding.source_participant_id.into_bytes()),
            CanonicalItem::participant_identity(binding.recipient_participant_id.into_bytes()),
            CanonicalItem::unsigned64(binding.producer_sequence),
            CanonicalItem::unsigned16(binding.payload_type.canonical_code()),
            CanonicalItem::hash512(binding.statement_hash.into_bytes()),
...
pub(crate) fn derive_sealed_mailbox_output_reference(
    output: &SealedMailboxPayload,
) -> MailboxResult<(Hash512, u64)> {
    let expected_descriptor = derive_canonical_stream_descriptor(
        CanonicalStreamDomain::PrivateMailboxCiphertext,
        &output.ciphertext,
    )
    .map_err(|reason| mailbox_error(reason, "mailbox output ciphertext is outside the profile"))?;
    if output.signed_envelope.ciphertext_descriptor != expected_descriptor {
        return Err(mailbox_error(
            RefusalReason::WrongHashOrRoot,
            "mailbox output ciphertext does not match its signed descriptor",
        ));
    }
    let canonical_envelope = output.signed_envelope.encode()?;
    let byte_length = canonical_envelope
        .len()
        .checked_add(output.ciphertext.len())
        .and_then(|length| u64::try_from(length).ok())
        .ok_or_else(|| {
            mailbox_error(
                RefusalReason::OutsideSupportedProfile,
                "mailbox output length does not fit u64",
            )
        })?;
    Ok((
        hash512(
            "sealed-lattice/mailbox/sealed-output/v1",
            &[
                CanonicalItem::variable_bytes(canonical_envelope)?,
                CanonicalItem::variable_bytes(&output.ciphertext)?,
            ],
        )?,
        byte_length,
    ))
