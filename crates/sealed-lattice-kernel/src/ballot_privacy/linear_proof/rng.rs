...
impl LinearProofAes256CtrCursor {
    ...
    fn read(&mut self, byte_count: usize) -> Vec<u8> {
        let required_total = self
            ...
            .expect("linear proof RNG cursor byte count overflowed");
        while self.buffered_bytes.len() < required_total {
            let mut encrypted_block = Block::<Aes256>::clone_from_slice(&self.counter_block);
            self.cipher.encrypt_block(&mut encrypted_block);
            self.buffered_bytes.extend_from_slice(&encrypted_block);
...
mod tests {
    use super::{
        LinearProofAes256CtrCursor, generate_linear_proof_aes256ctr_stream,
    ...
    #[test]
    fn aes256ctr_cursor_matches_single_stream_across_small_reads() {
        let seed = [5_u8; 32];
        let domain_separator = 77;
        let mut cursor = LinearProofAes256CtrCursor::new(&seed, domain_separator);
        let mut concatenated_reads = Vec::new();
        for byte_count in [1, 2, 0, 7, 16, 3, 64] {
            concatenated_reads.extend_from_slice(&cursor.read(byte_count));
        }
        let single_stream = generate_linear_proof_aes256ctr_stream(
            &seed,
            domain_separator,
            concatenated_reads.len(),
        );

        assert_eq!(concatenated_reads, single_stream);
    }
