...
#[cfg(test)]
use super::{
    public_parameters::DEFAULT_LINEAR_PROOF_COEFFICIENT_BIT_LENGTH,
    tbox_relations::{tbox_proof_ring, tbox_quadratic_many_dimension},
};
...
pub(crate) fn validate_many_quadratic_self_check() -> CanonicalResult<()> {
    ...
    let first_equation =
        LinearProofQuadraticEquation::zero(proof_ring, tbox_quadratic_many_dimension())?
    ...
    if fold.challenge_polynomials.len() != 2
        || fold.folded_equation.dimension() != tbox_quadratic_many_dimension()
...
mod tests {
    ...
    use crate::ballot_privacy::linear_proof::tbox_relations::{
        build_default_tbox_prefix_accumulators, tbox_proof_ring, tbox_quadratic_many_dimension,
    };
    use crate::ballot_privacy::polynomial_ring::PolynomialRing;

    fn zero_hash_mask_vector() -> Vec<Vec<u64>> {
        let proof_ring = tbox_proof_ring().expect("proof ring should validate");
    ...
    fn tbox_equations_expand_to_many_quadratic_dimension_and_bind_hash_mask() {
        ...
        assert!(
            equations
                .iter()
                .all(|equation| equation.dimension() == tbox_quadratic_many_dimension())
        );
        assert_eq!(
            equations[0]
                .linear_terms()
