...
const RECEIVER_SHARE_REPRESENTATIVE_BIT_LENGTH: u64 = 17;
const RECEIVER_OPENING_RANDOMNESS_BIT_LENGTH: u64 = 12;
...
fn validate_explicit_component_verification_summaries(
    case_object: &serde_json::Map<String, Value>,
) -> Result<(), String> {
...
fn validate_component_proof_readiness_manifests(
    case_object: &serde_json::Map<String, Value>,
) -> Result<(), String> {
...
fn validate_optional_unsigned_decimal_property(
...
fn validate_optional_u64_property(
    object: &serde_json::Map<String, Value>,
    field_name: &str,
    expected: Option<u64>,
) -> Result<(), String> {
...
fn validate_component_proof_statement_plans(
    case_object: &serde_json::Map<String, Value>,
) -> Result<(), String> {
    ...
    ) in plans.iter().zip(expected)
    {
        ...
        let expected_digest = derive_backend_digest(
            ...
            value_without_field(plan_value, "componentProofStatementDigest")?,
        )?;
...
fn validate_statement_dimensions(dimensions: EncodedRelationDimensions) -> Result<(), String> {
    ...
    let receiver_payload_ciphertext_chunk_count =
        receiver_payload_bit_count.div_ceil(RECEIVER_ENCRYPTION_MODULE_DEGREE);
    let receiver_encryption_witness_variables_per_receiver = receiver_payload_ciphertext_chunk_count
        * (2 * RECEIVER_ENCRYPTION_MODULE_RANK * RECEIVER_ENCRYPTION_MODULE_DEGREE
...
fn expected_receiver_encryption_explicit_rows(dimensions: EncodedRelationDimensions) -> u64 {
    let receiver_payload_bit_count = dimensions.encoded_coordinate_count
        * RECEIVER_SHARE_REPRESENTATIVE_BIT_LENGTH
        + OPENING_VARIABLES_PER_RECEIVER * RECEIVER_OPENING_RANDOMNESS_BIT_LENGTH;
    let receiver_payload_ciphertext_chunk_count =
...
fn expected_payload_plaintext_rows(dimensions: EncodedRelationDimensions) -> u64 {
    dimensions.roster_size * (dimensions.encoded_coordinate_count + OPENING_VARIABLES_PER_RECEIVER)
}

fn expected_score_and_shamir_rows(dimensions: EncodedRelationDimensions) -> u64 {
    dimensions.option_count * 2 + dimensions.roster_size * dimensions.encoded_coordinate_count
}
