...
pub(super) fn validate_hash_expanded_backend_row_batch(
    batch_object: &serde_json::Map<String, Value>,
    column_count: u64,
    dimensions: EncodedRelationDimensions,
) -> Result<(), String> {
    if string_property(batch_object, "batchKind")? != "HashExpandedRows"
        || batch_object.contains_key("rows")
    {
        return Err(
            "encoded relation backend hash-expanded row batch is not canonical".to_string(),
        );
    }
    ...
    let expected_row_count = match row_kind.as_str() {
        ...
        _ => {
            return Err("encoded relation backend hash-expanded row kind is invalid".to_string());
        }
    };
    if u64_property(batch_object, "rowCount")? != expected_row_count {
        return Err("encoded relation backend hash-expanded row count is invalid".to_string());
    }
    ...
    if receiver_roster_position == 0 || receiver_roster_position > dimensions.roster_size {
        return Err(
            "encoded relation backend hash-expanded receiver position is invalid".to_string(),
        );
    }
    ...
    {
        return Err("encoded relation backend hash-expanded labels are invalid".to_string());
    }
    validate_hash_string(&string_property(batch_object, "targetHash")?)?;
    validate_hash_map(object_property(batch_object, "publicInputHashes")?)?;
    ...
    validate_batch_hash_pair(
        batch_object,
        HASH_EXPANDED_BACKEND_MATRIX_HASH_PURPOSE,
        HASH_EXPANDED_BACKEND_TARGET_VECTOR_HASH_PURPOSE,
        hash_expanded_backend_payload(batch_object)?,
        hash_expanded_backend_payload(batch_object)?,
    )
}
...
pub(super) fn validate_backend_proof_components(
    ...
    for (component_index, expected_component_id) in expected_component_ids.iter().enumerate() {
        ...
        let component_hash = string_property(component_object, "componentHash")?;
        validate_hash_string(&component_hash)?;
        let component_value = Value::Object(component_object.clone());
        let component_payload = value_without_field(&component_value, "componentHash")?;
        let expected_component_hash =
            derive_backend_hash(BACKEND_PROOF_COMPONENTS_HASH_PURPOSE, component_payload)?;
        if component_hash != expected_component_hash {
            return Err("encoded relation backend proof-component hash is invalid".to_string());
        }
    }
...
pub(super) fn validate_batch_hash_pair(
    batch_object: &serde_json::Map<String, Value>,
    matrix_purpose: &str,
    target_purpose: &str,
    ...
    let matrix_hash = string_property(batch_object, "matrixHash")?;
    let target_vector_hash = string_property(batch_object, "targetVectorHash")?;
    validate_hash_string(&matrix_hash)?;
    validate_hash_string(&target_vector_hash)?;
    let expected_matrix_hash = derive_backend_hash(matrix_purpose, matrix_payload)?;
    let expected_target_vector_hash = derive_backend_hash(target_purpose, target_payload)?;
    if matrix_hash != expected_matrix_hash {
        return Err("encoded relation backend batch matrix hash is invalid".to_string());
    }
    if target_vector_hash != expected_target_vector_hash {
        return Err("encoded relation backend batch target-vector hash is invalid".to_string());
    }
...
pub(super) fn hash_expanded_backend_payload(
    batch_object: &serde_json::Map<String, Value>,
) -> Result<Value, String> {
    Ok(json!({
        ...
        "publicInputHashes": object_property(batch_object, "publicInputHashes")?,
        ...
        "targetHash": string_property(batch_object, "targetHash")?,
...
pub(super) fn backend_batch_matrix_summary(batch: &Value) -> Result<Value, String> {
    ...
    Ok(json!({
        ...
        "matrixHash": string_property(batch_object, "matrixHash")?,
        "rowCount": u64_property(batch_object, "rowCount")?,
        "rowKind": string_property(batch_object, "rowKind")?,
...
pub(super) fn backend_batch_target_summary(batch: &Value) -> Result<Value, String> {
    ...
    Ok(json!({
        ...
        "rowKind": string_property(batch_object, "rowKind")?,
        "rowOffset": u64_property(batch_object, "rowOffset")?,
        "targetVectorHash": string_property(batch_object, "targetVectorHash")?,
    }))
}

pub(super) fn validate_hash_map(object: &serde_json::Map<String, Value>) -> Result<(), String> {
    if object.is_empty() {
        return Err("encoded relation backend hash map must not be empty".to_string());
    }
    for value in object.values() {
        validate_hash_string(value.as_str().ok_or_else(|| {
            "encoded relation backend hash map value must be a string".to_string()
        })?)?;
    }
...
pub(super) fn validate_hash_string(value: &str) -> Result<(), String> {
    if value.len() != 128 || !value.bytes().all(|byte| byte.is_ascii_hexdigit()) {
        return Err("encoded relation hash must be 64 lowercase bytes".to_string());
    }
    if !value
        .bytes()
        .all(|byte| byte.is_ascii_digit() || (b'a'..=b'f').contains(&byte))
    {
        return Err("encoded relation hash must be lowercase hex".to_string());
    }
...
pub(super) fn derive_backend_hash(purpose: &str, payload: Value) -> Result<String, String> {
    ...
    .map_err(|error| format!("encoded relation backend hash could not be recomputed: {error}"))
}
    ...
    fn verifies_mini_encoded_relation_vector() {
        ...
        assert!(
            verification["statusLabels"]
                ...
                .contains(&serde_json::json!("EncodedRelationHashRecomputed"))
        );
    }
    ...
    fn rejects_proof_component_metadata_mutations() {
        ...
        expect_mini_case_mutation_rejected(|vector_case| {
            vector_case["loweredStatement"]["backendStatement"]["proofComponents"][0]["proofLoweringStatus"] =
                json!("HashExpandedRowsPending");
        });
        expect_mini_case_mutation_rejected(|vector_case| {
            vector_case["loweredStatement"]["backendStatement"]["proofComponents"][0]["componentHash"] =
                json!("0".repeat(128));
        });
        expect_mini_case_mutation_rejected(|vector_case| {
            vector_case["loweredStatement"]["backendStatement"]["proofComponentsHash"] =
                json!("0".repeat(128));
        });
        expect_mini_case_mutation_rejected(|vector_case| {
