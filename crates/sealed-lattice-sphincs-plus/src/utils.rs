...
pub fn bytes_to_ull(input: &[u8], inlen: usize) -> u64 {
    let mut retval = 0u64;
    for (i, input_byte) in input.iter().take(inlen).enumerate() {
        retval |= (*input_byte as u64) << (8 * (inlen - 1 - i));
    }
    retval
}
...
pub fn compute_root(
    root: &mut [u8],
    leaf: &[u8],
    leaf_position: (u32, u32),
    auth_path: &[u8],
    tree_height: u32,
    ctx: &SpxCtx,
    addr: &mut [u32; 8],
) {
    let (mut leaf_idx, mut idx_offset) = leaf_position;
