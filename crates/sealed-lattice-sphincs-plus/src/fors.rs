...
pub fn message_to_indices(indices: &mut [u32], m: &[u8]) {
    ...
    for index in indices.iter_mut().take(SPX_FORS_TREES) {
        *index = 0;
        for j in 0..SPX_FORS_HEIGHT {
            *index ^= (((m[offset >> 3] >> (offset & 0x7)) & 0x1) as u32) << j;
            offset += 1;
        }
    }
}
...
/// Assumes m contains at least SPX_FORS_HEIGHT * SPX_FORS_TREES bits.
pub fn fors_sign(sig: &mut [u8], pk: &mut [u8], m: &[u8], ctx: &SpxCtx, fors_addr: &mut [u32]) {
