...
pub fn wots_gen_leafx1(dest: &mut [u8], ctx: &SpxCtx, leaf_idx: u32, v_info: &mut LeafInfoX1) {
    let mut leaf_addr = v_info.leaf_addr;
    let mut pk_addr = v_info.pk_addr;

    let mut pk_buffer = [0u8; SPX_WOTS_BYTES];
    let wots_k_mask = if leaf_idx == v_info.wots_sign_leaf {
    ...
    for i in 0..SPX_WOTS_LEN {
        ...
        loop {
            ...
            if k == wots_k {
                let start = i * SPX_N;
                v_info.wots_sig[start..start + SPX_N].copy_from_slice(&pk_buffer[idx..idx + SPX_N]);
            }
