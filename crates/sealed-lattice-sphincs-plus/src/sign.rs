...
pub(crate) fn crypto_sign_seed_keypair(pk: &mut [u8], sk: &mut [u8], seed: &[u8]) {
...
pub(crate) fn crypto_sign_signature(sig: &mut [u8], m: &[u8], sk: &[u8], randomizer: &[u8]) {
    let mut ctx = SpxCtx::default();

    let mut sk_prf = Zeroizing::new([0u8; SPX_N]);
    sk_prf.copy_from_slice(&sk[SPX_N..SPX_N * 2]);
    let mut pk = [0u8; SPX_N * 2];
    pk[..SPX_N * 2].copy_from_slice(&sk[SPX_N * 2..SPX_N * 4]);

    let mut optrand = Zeroizing::new([0u8; SPX_N]);
    ...
    optrand.copy_from_slice(randomizer);
...
pub(crate) fn crypto_sign_verify(sig: &[u8], msg: &[u8], pk: &[u8]) -> bool {
    let mut ctx = SpxCtx::default();
    ...
    hash_message(&mut mhash, &mut tree, &mut idx_leaf, sig, pk, msg, &ctx);
    idx += SPX_N;
